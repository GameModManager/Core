#!/usr/bin/env bash
# build.sh — one-shot local build that mirrors the Linux CI runner
# (.github/workflows/build-linux.yml) minus the dependency-install step.
#
# What it does, in CI order:
#   1. dependency presence check   (the CI "Install dependencies" apt step —
#                                  assumed already installed locally)
#   2. git submodule update        (external/abi, external/plugins, themes)
#   3. cmake -B build -DCMAKE_BUILD_TYPE=Release
#      (auto-adds -DCMAKE_{C,CXX}_COMPILER_LAUNCHER=ccache when ccache is found —
#       caches compilation so incremental rebuilds only recompile changed files)
#   4. cmake --build build --parallel
#   5. ctest --test-dir build --output-on-failure (only with --tests)
#
# Usage:
#   ./build.sh                      configure + build (Release; tests off)
#   ./build.sh --tests              also run ctest after the build
#   ./build.sh --clean              rm -rf build/ and build_plugins/ first —
#                                   a genuine from-scratch build. build_plugins/
#                                   is the legacy standalone plugin build dir;
#                                   plugins now build inside build/plugins/.
#   ./build.sh --debug              CMAKE_BUILD_TYPE=Debug
#   ./build.sh --no-loot            -DGMM_WITH_LOOT=OFF (no cargo/rust needed)
#   ./build.sh --update-submodules  git submodule update --init --recursive
#                                   --remote — pulls the submodules to latest
#                                   main exactly like CI (default is the
#                                   pinned commit: --init only)
#   ./build.sh --skip-check         skip the dependency-presence check
...
for arg in "$@"; do
    case "$arg" in
        ...
        *) echo "Unknown option: $arg (see ./build.sh --help)" >&2; exit 1 ;;
    esac
done
