...
bool ProtonRuntime::launch(const std::filesystem::path& executable,
    ...
    auto proton = find_proton_binary(steam_appid);
    if (proton.empty()) return false;

    if (!prepare_proton_environment(game_dir, steam_appid)) return false;
...
std::filesystem::path ProtonRuntime::find_proton_binary(uint32_t steam_appid) {
    if (steam_appid > 0) {
...
bool ProtonRuntime::prepare_proton_environment(const std::filesystem::path& game_dir,
                                                uint32_t steam_appid) {
    auto steam_root = find_steam_root();
