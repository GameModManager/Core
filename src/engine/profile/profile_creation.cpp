...
ProfileCreationResult create_fresh_profile(const std::filesystem::path &profiles_dir,
  ...
  if (ec) {
    result.error =
        "failed to create profile directory " + target.string() + ": " + ec.message();
...
ProfileCreationResult copy_profile(const std::filesystem::path &profiles_dir,
  ...
  if (!std::filesystem::is_directory(source_dir, ec) || ec) {
    result.error =
        "source profile directory " + source_dir.string() + " does not exist";
    return result;
  }
  ...
  if (ec) {
    result.error = "failed to create profiles directory " + profiles_dir.string() +
                   ": " + ec.message();
    Logger::instance().error(result.error);
    return result;
  }
  ...
  std::filesystem::copy(source_dir, target,
                        std::filesystem::copy_options::recursive |
                            std::filesystem::copy_options::copy_symlinks,
                        ec);
  if (ec) {
    result.error = "failed to copy profile from " + source_dir.string() + " to " +
...
bool rename_profile(const std::filesystem::path &profiles_dir,
  ...
  if (!std::filesystem::is_directory(source, ec) || ec) {
    const std::string msg =
        "source profile directory " + source.string() + " does not exist";
    if (error) {
      *error = msg;
    }
    return false;
