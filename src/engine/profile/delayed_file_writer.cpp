...
DelayedFileWriter::DelayedFileWriter(WriteFn fn, std::chrono::milliseconds delay)
    : fn_(std::move(fn)), delay_(delay), thread_([this] {
        run();
      }) {}
...
DelayedFileWriter::~DelayedFileWriter() {
  ...
  if (flush) {
    try {
      ...
      Logger::instance().error(std::string("DelayedFileWriter flush failed: ") +
                               e.what());
    } catch (...) {
  ...
  if (do_flush) {
    try {
      ...
      Logger::instance().error(
          std::string("DelayedFileWriter immediate write failed: ") + e.what());
    } catch (...) {
      Logger::instance().error(
          "DelayedFileWriter immediate write failed: unknown exception");
    }
  }
}
...
void DelayedFileWriter::run() {
  ...
  while (!stop_) {
    ...
    try {
      ...
      Logger::instance().error(std::string("DelayedFileWriter callback failed: ") +
                               e.what());
    } catch (...) {
