...
ProfileManager::ProfileManager(std::filesystem::path directory,
                               std::chrono::milliseconds modlist_delay)
    : directory_(std::move(directory)), modlist_writer_(
                                            [this] {
                                              do_write_modlist();
                                            },
                                            modlist_delay) {
...
bool ProfileManager::local_saves() const {
  return get_setting_bool("LocalSaves");
}
void ProfileManager::set_local_saves(bool value) {
  set_setting_bool("LocalSaves", value);
}

bool ProfileManager::local_settings() const {
  return get_setting_bool("LocalSettings");
}
void ProfileManager::set_local_settings(bool value) {
  set_setting_bool("LocalSettings", value);
}
...
void ProfileManager::set_root_setting(const std::string &key,
...
bool ProfileManager::save_settings() {
  if (!safe_write_file(settings_path(), serialize_ini(ini_))) {
    Logger::instance().error("failed to write settings.ini: " +
...
void ProfileManager::refresh_mod_status(const std::vector<std::string> &known_mods,
  ...
  std::sort(mods_.begin(), mods_.end(),
            [](const ModListEntry &a, const ModListEntry &b) {
...
bool ProfileManager::set_mod_priority(const std::string &mod_id, int new_priority) {
  std::lock_guard lock(mods_mutex_);
  auto it = std::find_if(mods_.begin(), mods_.end(), [&](const ModListEntry &m) {
    return m.mod_id == mod_id;
  });
  if (it == mods_.end()) {
    return false;
  }
...
void ProfileManager::write_modlist() {
  modlist_writer_.write();
}
void ProfileManager::write_modlist_now() {
  modlist_writer_.write_immediately();
}
void ProfileManager::cancel_modlist_write() {
  modlist_writer_.cancel();
}
...
std::vector<std::string> ProfileManager::read_plugins() const {
  return read_lines(plugins_path());
}
bool ProfileManager::write_plugins(const std::vector<std::string> &plugins) {
  return write_lines(plugins_path(), plugins);
}

std::vector<std::string> ProfileManager::read_load_order() const {
  return read_lines(loadorder_path());
}
...
std::vector<std::string> ProfileManager::read_archives() const {
  return read_lines(archives_path());
}
