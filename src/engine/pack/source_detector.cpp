...
namespace {
  ...
  UrlParts split_url(std::string_view s) {
    ...
    parts.path = (path_begin == std::string_view::npos) ? std::string_view{}
  ...
  bool path_has_segment(std::string_view path_lower, std::string_view segment) {
    ...
    while (pos < path_lower.size()) {
      while (pos < path_lower.size() && path_lower[pos] == '/')
        ++pos;
      if (pos >= path_lower.size())
        break;
      auto end = path_lower.find('/', pos);
      if (end == std::string_view::npos)
        end = path_lower.size();
      if (path_lower.substr(pos, end - pos) == segment)
        return true;
      pos = end;
    }
    return false;
  ...
  std::uint16_t read_u16le(const std::vector<char> &buf, std::size_t pos) {
    return static_cast<std::uint16_t>(static_cast<unsigned char>(buf[pos]) |
  ...
  bool zip_has_entry(const std::filesystem::path &path,
    ...
    if (size < 22)
      return false;  // smaller than the EOCD itself
    ...
    if (!read_exact(f, tail_start, tail.data(), static_cast<std::size_t>(window))) {
      return false;
    }
    ...
    for (std::size_t i = tail.size() - 22;; --i) {
      if (tail[i] == '\x50' && tail[i + 1] == '\x4b' && tail[i + 2] == '\x05' &&
          tail[i + 3] == '\x06') {
        eocd = i;
        break;
      }
    ...
    const std::uint64_t cd_end    = static_cast<std::uint64_t>(cd_offset) + cd_size;
    if (cd_end > size)
      return false;
    ...
    for (std::uint16_t i = 0; i < total; ++i) {
      if (pos + 46 > cd_end)
        return false;
      if (!read_exact(f, pos, header.data(), 46))
        return false;
      if (!(header[0] == '\x50' && header[1] == '\x4b' && header[2] == '\x01' &&
            header[3] == '\x02')) {
      ...
      const std::uint64_t next        = name_pos + name_len + extra_len + comment_len;
      if (next > cd_end || next < pos)
        return false;
      if (name_len > 0 && name_len < 4096) {
        std::string name(name_len, '\0');
        if (!read_exact(f, name_pos, name.data(), name_len))
          return false;
        if (lower_ascii(name) == want_name_lower)
          return true;
      }
      pos = next;
    }
  ...
  std::optional<Detection> probe_nxm(std::string_view input) {
    if (!starts_with(lower_ascii(input), "nxm://"))
      return std::nullopt;
    return make_detection(PackFormat::NexusCollection,
  ...
  std::optional<Detection> probe_gmmpack_extension(std::string_view input) {
    ...
    if (!ends_with(lower_ascii(candidate), ".gmmpack"))
      return std::nullopt;
    ...
    if (std::filesystem::is_regular_file(path, ec)) {
      if (!file_starts_with_pk(path)) {
        return make_unknown("file has a .gmmpack extension but is not a zip archive");
      }
    }
  ...
  std::optional<Detection> probe_zip_sniff(std::string_view input) {
    ...
    if (!std::filesystem::is_regular_file(path, ec))
      return std::nullopt;
    if (!zip_has_entry(path, "manifest.json"))
      return std::nullopt;
    return make_detection(PackFormat::Gmmpack,
...
std::string_view format_id_of(PackFormat format) {
  switch (format) {
  case PackFormat::Gmmpack:
    return "gmmpack";
  case PackFormat::NexusCollection:
...
Detection detect_pack_source(const std::string &url_or_path) {
  ...
  if (input.empty())
    return make_unknown("empty pack reference");
  for (const Probe probe : kProbes) {
    if (auto decision = probe(input))
      return *decision;
  }
  return make_unknown("unsupported pack reference (expected a .gmmpack file, an nxm:// "
