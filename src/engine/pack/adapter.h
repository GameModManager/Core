...
public:
    ...
    [[nodiscard]] virtual bool can_handle_source(
        const std::string& url_or_path) const = 0;

    // Load and validate the pack, returning its manifest.
    [[nodiscard]] virtual Outcome<PackManifest> resolve_pack(
        const std::string& url_or_path) = 0;

    // Resolve one manifest entry to something downloadable.
    [[nodiscard]] virtual Outcome<ResolvedMod> resolve_mod(
        const ModEntry& entry) = 0;

    // Download details for a resolved mod (pure query over cached state).
    [[nodiscard]] virtual DownloadInfo get_download_info(
        const ResolvedMod& resolved) const = 0;
