...
std::vector<std::string> unmanaged_mods_for(const std::string &game_id) {
  if (auto feature =
          Game::Features::Registry::instance().resolve_feature<UnmanagedModsFeature>(
              game_id)) {
    return feature->mods();
  }
  return {};
...
namespace {
  ...
  const ModDataContentFeature::Content kMeshContent = {ModContentId::Mesh, "Meshes",
                                                       "mesh", false};
  const ModDataContentFeature::Content kBsaContent = {ModContentId::Bsa,
                                                      "Bethesda Archive", "bsa", false};
  ...
  const ModDataContentFeature::Content kTextureContent = {ModContentId::Texture,
                                                          "Textures", "texture", false};
  const ModDataContentFeature::Content kMcmContent     = {
      ModContentId::Mcm, "MCM Configuration", "mcm", false};
  const ModDataContentFeature::Content kIniContent = {ModContentId::Ini, "INI Files",
                                                      "ini", false};
...
bool register_game_feature_data(
  ...
  if (feature_type == "data_archives") {
    ...
    feature = std::make_shared<ScriptExtenderFeature>(
        kv_get(kv, "binary"), kv_get(kv, "plugin_path"), kv_get(kv, "loader_name"),
        kv_get(kv, "savegame_extension"));
    ...
    feature = std::make_shared<LocalSavegamesFeature>(kv_get(kv, "saves_subpath"),
                                                      kv_get(kv, "ini_file"));
    ...
    feature = std::make_shared<BSAInvalidationFeature>(kv_get(kv, "bsa_name"),
                                                       kv_get(kv, "bsa_version"));
    ...
    feature =
        std::make_shared<ModDataContentFeature>(std::move(enabled), std::move(custom));
  } else {
    Logger::instance().warn("register_game_feature_data: unknown feature type '" +
