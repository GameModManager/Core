...
std::vector<std::string> unmanaged_mods_for(const std::string& game_id) {
    if (auto feature =
            GameFeatureRegistry::instance().resolve_feature<UnmanagedModsFeature>(
                game_id)) {
        return feature->mods();
    }
    return {};
...
const ModDataContentFeature::Content kMeshContent =
    {ModContentId::Mesh, "Meshes", "mesh", false};
...
const ModDataContentFeature::Content kIniContent =
    {ModContentId::Ini, "INI Files", "ini", false};
...
std::vector<ModDataContentFeature::Content> mod_content_catalog() {
    return {kPluginContent, kOptionalContent, kInterfaceContent, kMeshContent,
            kBsaContent,    kScriptContent,   kSkseContent,      kSkseFilesContent,
            kSkyprocContent, kSoundContent,   kTextureContent,   kMcmContent,
...
int mod_content_id_from_string(const std::string& id) {
    ...
    if (lc == "plugin") return ModContentId::Plugin;
    if (lc == "optional") return ModContentId::Optional;
    if (lc == "interface") return ModContentId::Interface;
    if (lc == "mesh") return ModContentId::Mesh;
    if (lc == "bsa") return ModContentId::Bsa;
    if (lc == "script") return ModContentId::Script;
    if (lc == "skse") return ModContentId::Skse;
    if (lc == "skse_files") return ModContentId::SkseFiles;
    if (lc == "skyproc") return ModContentId::Skyproc;
    if (lc == "sound") return ModContentId::Sound;
    if (lc == "texture") return ModContentId::Texture;
    if (lc == "mcm") return ModContentId::Mcm;
    if (lc == "ini") return ModContentId::Ini;
    if (lc == "facegen") return ModContentId::Facegen;
    if (lc == "modgroup") return ModContentId::Modgroup;
    return -1;
}
...
std::vector<ModDataContentFeature::Content> ModDataContentFeature::all_contents() const {
    std::vector<Content> catalog = mod_content_catalog();
    std::vector<Content> out;
    for (auto& c : catalog) {
        for (const auto& cc : custom_contents_)
            if (cc.id == c.id) { c = cc; break; }
...
std::vector<int> ModDataContentFeature::contents_for(
    ...
    auto push = [&](int id) {
        if (content_enabled(enabled_ids_, id)) out.push_back(id);
    };
    ...
    for (const auto& entry : *tree) {
        if (entry->is_file()) {
            ...
            if (n == "textures" || n == "icons" || n == "bookart") {
                ...
                if (sub && sub->size() > 0) push(ModContentId::Optional);
            }
        }
    }
...
bool register_game_feature_data(
    ...
    if (feature_type == "data_archives") {
        ...
        feature = std::make_shared<ModDataContentFeature>(
            std::move(enabled), std::move(custom));
    } else {
        Logger::instance().warn(
    ...
    return GameFeatureRegistry::instance().register_feature(
