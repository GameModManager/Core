...
public:
  ...
  // count as real game data (MO2
  // possibleFolderNames()/possibleFileExtensions()).
...
public:
  ...
  std::vector<int>
  contents_for(const std::shared_ptr<const FileTree> &tree,
...
// Game::Features::Registry instead of hardcoding game-specific parsers.
...
public:
  ...
  explicit SaveParserFeature(ParserFn parser) : parser_(std::move(parser)) {}
