...
void GameCapabilities::register_capability(const CapabilityInfo &info) {
  ...
  Logger::instance().debug("Capability registered: " + info.game_id + "." +
                           info.capability + " (" + info.display_name + ")");
}
...
std::vector<CapabilityInfo>
GameCapabilities::capabilities_for(const std::string &game_id) const {
  auto game_it = caps_.find(game_id);
  if (game_it == caps_.end())
    return {};
...
std::vector<CapabilityInfo>
GameCapabilities::sorted_capabilities_for(const std::string &game_id) const {
  auto game_it = caps_.find(game_id);
  if (game_it == caps_.end())
    return {};
...
GameCapabilities::get_capability(const std::string &game_id,
                                 const std::string &capability) const {
  auto game_it = caps_.find(game_id);
  if (game_it == caps_.end())
    return nullptr;
...
GameCapabilities::visible_tabs_for(const std::string &game_id) const {
