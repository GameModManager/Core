...
struct CapabilityInfo {
  ...
  std::string data_path;     // where the game stores these (e.g. "Data/", "Documents/My
                             // Games/...")
...
public:
  ...
  [[nodiscard]] std::vector<CapabilityInfo>
  capabilities_for(const std::string &game_id) const;

  // Get capabilities sorted for tab display (respects insert_before/insert_after)
  [[nodiscard]] std::vector<CapabilityInfo>
  sorted_capabilities_for(const std::string &game_id) const;

  // Get a specific capability
  [[nodiscard]] const CapabilityInfo *
  get_capability(const std::string &game_id, const std::string &capability) const;
  ...
  visible_tabs_for(const std::string &game_id) const;

private:
  // game_id -> capability_name -> info
  std::unordered_map<std::string, std::unordered_map<std::string, CapabilityInfo>>
      caps_;
};
