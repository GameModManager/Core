...
namespace {
  ...
  // MO2-parity same-origin asset detection (PluginList, pluginlist.cpp:241-280):
...
bool Database::refresh(const std::filesystem::path &game_dir,
  ...
  if (std::filesystem::is_directory(mods_dir, ec)) {
    for (const auto &entry : std::filesystem::directory_iterator(mods_dir, ec)) {
      ...
      int priority = -1;
      if (!mods_dir.empty())
        priority = ModMeta::load(mods_dir, folder).priority();
  ...
  parse_headers();
  // Archive candidates for the whole virtual Data view (MO2 parity): every
  // top-level .bsa/.ba2 in game Data plus every enabled mod folder. Disabled
  // mod folders contribute nothing (they are skipped above, like plugins).
  std::vector<std::string> archive_candidates;
  {
    ...
    if (data_gf && data_gf->exists()) {
      for (const auto &entry :
        ...
        if (is_archive_file(to_lower(file)))
          archive_candidates.push_back(file);
      }
    }
    if (!mods_dir.empty()) {
      for (const auto &folder : folders) {
    ...
    std::sort(archive_candidates.begin(), archive_candidates.end());
    archive_candidates.erase(
...
void Database::load_creation_club(const std::filesystem::path &game_dir,
  ...
  if (data_gf && data_gf->exists() &&
    ...
    const auto data_ccc = resolve_regular_file_ci(data_gf->absolute(), ccc_filename);
    if (!data_ccc.empty())
      candidates.push_back(data_ccc);
  }
  if (candidates.empty())
    return;
...
void Database::sort_load_order() {
  ...
  for (size_t i = 0; i < n; ++i) {
    if (plugins_[i].is_game_native || plugins_[i].is_cc)
      continue;
  ...
  for (size_t i : mod_indices)
    if (indegree[to_lower(plugins_[i].name)] == 0)
      ready.insert(i);
  ...
  while (!ready.empty()) {
    ...
    for (size_t dep : dependents[to_lower(plugins_[cur].name)]) {
      if (--indegree[to_lower(plugins_[dep].name)] == 0)
        ready.insert(dep);
    }
  }
  ...
  for (size_t i : ordered)
    reordered.push_back(plugins_[i]);
...
bool Database::apply_load_order(const std::vector<std::string> &order,
  ...
  for (size_t i = 0; i < n; ++i)
    if (plugins_[i].is_cc && !done[i])
      emit(i);
  for (size_t idx : user_order)
    emit(idx);

  std::vector<GamePlugin> reordered;
  reordered.reserve(ordered.size());
  for (size_t idx : ordered)
    reordered.push_back(plugins_[idx]);
  plugins_ = std::move(reordered);
  rebuild_index();
  for (size_t i = 0; i < plugins_.size(); ++i)
    plugins_[i].priority = static_cast<int>(i);
...
void Database::set_missing_masters() {
  ...
  // Pass 2 (MO2 testMasters parity, pluginlist.cpp:1342-1361): for ENABLED
  // plugins only, masters absent from the list OR present-but-disabled.
  ...
  for (auto &p : plugins_) {
    ...
    for (const auto &master : p.masters) {
      ...
      if (enabled_names.find(lower) == enabled_names.end() &&
        ...
        p.master_unset.push_back(master);
      }
    }
...
void Database::generate_mod_indexes() {
  // MO2 generatePluginIndexes parity (pluginlist.cpp:1281-1326): disabled
  ...
  for (auto &p : plugins_) {
    ...
    if (p.is_medium()) {
      // "FD:00".."FC:FF" style: 2-hex high byte (253 + overflow) + 2-hex low.
      const uint32_t pos = 253 + num_eshs / 256;
      char buf[16];
      std::snprintf(buf, sizeof buf, "%02X:%02X", static_cast<unsigned>(pos),
                    static_cast<unsigned>(num_eshs % 256));
      p.mod_index      = 0xFD000000u | num_eshs;
      p.mod_index_text = buf;
      ++num_eshs;
    } else if (p.is_light()) {
      // "FE:000".."FF:FFF" style: 2-hex high (254 + overflow) + 3-hex low.
      const uint32_t pos = 254 + num_esls / 4096;
      char buf[16];
      std::snprintf(buf, sizeof buf, "%02X:%03X", static_cast<unsigned>(pos),
                    static_cast<unsigned>(num_esls % 4096));
      p.mod_index      = 0xFE000000u | num_esls;
      p.mod_index_text = buf;
      ++num_esls;
    } else {
      p.mod_index      = num_full;
      p.mod_index_text = hex2(num_full);
      ++num_full;
    }
  }
}
...
bool Database::set_enabled(const std::string &name, bool enabled, std::string *error) {
    ...
    while (!stack.empty()) {
      ...
      for (const auto &master : cur->masters) {
        ...
        if (mit == by_name_ci_.end()) {
          if (error)
            *error = "Cannot enable " + cur->name + ": missing master " + master;
          return false;
        }
  ...
  if (plugin.force_loaded) {
    if (error)
      *error = name + " is a core plugin and cannot be disabled";
    return false;
  }
  ...
  for (auto &other : plugins_) {
    ...
    if (std::find_if(other.masters.begin(), other.masters.end(), has_master) !=
      ...
      if (error)
        *error =
            "Cannot disable " + name + ": " + other.name + " requires it as a master";
      return false;
    }
  }
...
bool Database::move_plugin(int from_row, int to_row, std::string *error) {
  ...
  if (from_row < 0 || from_row >= n) {
    if (error)
      *error = "move_plugin: source row " + std::to_string(from_row) + " out of range";
    return false;
  }
  if (to_row < 0 || to_row >= n)
    to_row = n - 1;  // clamp to the last row

  if (plugins_[from_row].force_loaded) {
    if (error)
      *error = plugins_[from_row].name + " is a core plugin and cannot be moved";
    return false;
  }
  if (plugins_[from_row].locked) {
    if (error)
      *error = plugins_[from_row].name + " is locked and cannot be moved";
    return false;
  }
  ...
  if (plugins_[to_row].locked) {
    if (error)
      *error = plugins_[to_row].name + " is locked and cannot be displaced";
    return false;
  }
  if (from_row == to_row)
    return true;
  ...
  if (std::filesystem::is_regular_file(dir / "lockedorder.txt")) {
    ...
    while (std::getline(in, line)) {
      ...
      if (sep == std::string::npos)
        continue;  // malformed - skip (MO2 parity)
      int prio = 0;
      try {
      ...
      if (plugins_[it->second].force_loaded)
        continue;  // never lock core
...
std::filesystem::path Database::resolve_plugins_txt_target(
  ...
  if (subdir.empty())
    return {};  // game has no plugin support - skip
  if (!platform)
    return {};
...
bool Database::write_plugins_txt_for_launch(const std::filesystem::path &game_dir,
  ...
  if (inst.read_toml())
    override_path = inst.info().plugins_txt_path;

  const auto target = resolve_plugins_txt_target(knowledge, game_id, steam_appid,
                                                 platform, override_path);
  if (target.empty()) {
    Logger::instance().debug("Plugin::Database: no plugins.txt target for game " +
                             game_id + " - skipping");
    return false;
  }
  ...
  Database db;
  if (!db.refresh(game_dir, inst.path_for(InstanceKind::Mods), disable_mechanism,
                  game_native)) {
  ...
  if (ok) {
    Logger::instance().debug("Plugin::Database: wrote " +
                             std::to_string(db.plugins().size()) + " plugins to " +
                             target.string());
  } else {
