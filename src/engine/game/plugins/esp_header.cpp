...
EspHeaderInfo read_esp_header(const std::filesystem::path &file_path) {
  ...
  if (data_size > 16u * 1024 * 1024)
    return info;

  std::vector<unsigned char> data(data_size);
  if (data_size > 0) {
    in.read(reinterpret_cast<char *>(data.data()), data_size);
    if (static_cast<uint32_t>(in.gcount()) < data_size)
      return info;
  }
  ...
  while (pos + 6 <= data.size()) {
    ...
    if (std::strcmp(type, "MAST") == 0 && size > 0) {
      ...
      while (!master.empty() && (master.back() == '\0' || master.back() == '\r' ||
                                 master.back() == '\n' || master.back() == ' ')) {
        master.pop_back();
      }
      if (!master.empty())
        info.masters.push_back(master);
    } else if (std::strcmp(type, "HEDR") == 0 && size >= 8) {
      // HEDR: float header version, uint32 numRecords, uint32 nextObjectId.
      std::memcpy(&info.header_version, data.data() + pos, sizeof(info.header_version));
      ...
      while (!author.empty() && author.back() == '\0')
        author.pop_back();
      ...
      if (data_flags & 0x0200u)
        info.is_light = true;
      if (data_flags & 0x0400u)
        info.is_medium = true;
      if (data_flags & 0x0080u)
        info.localized = true;
    }
    pos += size;
  }
