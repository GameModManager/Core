...
namespace {
  ...
  void ini_set(const fs::path &path, const std::string &section, const std::string &key,
    ...
    int section_end    = -1;  // index of last line in section (inclusive)
    {
      ...
      for (size_t i = 0; i < lines.size(); ++i) {
        ...
        if (t[0] == '[') {
          ...
          auto name = trim(t.substr(1, close - 1));
          if (in_section) {
            section_end = static_cast<int>(i) - 1;
            break;
          }
          in_section = iequals(name, section);
          if (in_section)
            section_header = static_cast<int>(i);
          continue;
        }
        if (in_section)
      ...
      if (section.empty()) {
        ...
        for (size_t i = 0; i < lines.size(); ++i) {
          ...
          if (!t.empty() && t[0] == '[')
            break;
          auto eq = t.find('=');
          if (eq != std::string::npos && iequals(trim(t.substr(0, eq)), key)) {
            found = true;
            continue;
          }
...
LocalSavesConfig resolve_local_saves(const fs::path &my_games_dir,
                                     const fs::path &instance_root,
                                     const std::string &profile_sub_name,
                                     const std::string &ini_file_name, bool enabled) {
...
bool apply_local_saves(const LocalSavesConfig &cfg) {
  ...
  std::string current_path =
      ini_get(cfg.ini_path, kLocalSaveSection, kLocalSavesPathKey);
  std::string current_mygames =
      ini_get(cfg.ini_path, kLocalSaveSection, kLocalUseMyGamesKey);
  ...
  if (cfg.enabled) {
    ...
    if (!already_enabled) {
      ...
      Logger::instance().debug("local saves: redirected game saves to " +
                               cfg.local_saves_dir.string());
      return true;
    }
