...
SaveReader::SaveReader(const std::filesystem::path &path,
  ...
  if (max_prefix_bytes > 0 && static_cast<std::uint64_t>(want) > max_prefix_bytes) {
    // Prefix read (Workspace-69xt fast scan): buffer only the file head.
    // Reads past the prefix throw SaveParseError (EOF). Sized up front so
    // the allocation itself stays small (a later shrink would keep the
    // full capacity).
    want = static_cast<std::size_t>(max_prefix_bytes);
  }
  file_.resize(want);
...
void SaveReader::skip(std::size_t count) {
...
std::string SaveReader::read_bytes(std::size_t count) {
  if (pos_ + count > buf_.size()) {
    throw_eof();
  }
...
std::string SaveReader::wstring() {
  ...
  for (std::size_t i = 0; i < bytes.size();) {
    ...
    if (c < 0x80) {
      ...
      // Invalid UTF-8: keep the byte (fromUtf8 replaces with U+FFFD; a
      // lone byte is closer to the source than a replacement for our
      // display purposes).
      out.push_back(static_cast<char>(c));
      ++i;
    }
...
void SaveReader::begin_compressed(std::uint16_t type, std::uint64_t max_decompressed) {
  if (type == 0) {
  ...
  if (type == 1) {
    ...
    buf_ = inflate_chunks(chunk_start, total, file_, max_decompressed);
    pos_ = 0;
...
SaveReader::inflate_chunks(std::uint64_t start, std::uint64_t total_uncompressed,
  ...
  out.reserve(static_cast<std::size_t>(
      std::min<std::uint64_t>({total_uncompressed, file.size(), cap_bytes})));
  std::uint64_t pos = start;
  std::array<std::uint8_t, kChunk> inbuf{};
  std::array<std::uint8_t, kChunk> outbuf{};

  while (out.size() < total_uncompressed && out.size() < cap_bytes &&
         pos < file.size()) {
    z_stream zs{};
