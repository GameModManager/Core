...
public:
  ...
  // magic mismatch.
  //
  // `max_prefix_bytes` caps the initial read (Workspace-69xt fast scan):
  // only the first N bytes are buffered, so a caller that needs the file
  ...
  //
  // `max_decompressed` caps the type-1 inflate (Workspace-69xt fast scan):
  // inflation stops once the buffer holds that many bytes, so a caller
  // that only needs the region head (plugin lists) never pays for the
  // Papyrus/change-flag tail. Reads past the cap throw SaveParseError
  // (EOF). Defaults to unlimited (full-parse behavior). Type 2 (single
  // LZ4 block) always decompresses fully - a block cannot be truncated.
  void begin_compressed(std::uint16_t type,
                        std::uint64_t max_decompressed = UINT64_MAX);
  ...
  void release_raw_file() { std::vector<std::uint8_t>().swap(file_); }
