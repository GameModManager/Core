...
GameDetector::parse_library_folders(const std::filesystem::path &libraryfolders_vdf) {
...
GameDetector::find_app_install(const std::filesystem::path &library_folder,
                               uint32_t appid) {
  auto acf = library_folder / ("appmanifest_" + std::to_string(appid) + ".acf");
  if (!std::filesystem::exists(acf))
    return std::nullopt;
...
std::optional<std::string> GameDetector::parse_acf_value(const std::string &acf_content,
...
GameDetector::detect_steam_games(uint32_t steam_appid, const std::string &game_id,
...
std::vector<DetectedGame> GameDetector::detect_steam_games_multi(
    const std::vector<std::pair<uint32_t, std::pair<std::string, std::string>>>
        &games) {
