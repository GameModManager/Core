...
static bool content_looks_valid(const ScanConfig& cfg,
  ...
  static const std::vector<std::string> kPluginExts = {"esp", "esm", "esl", "bsa",
                                                       "ba2"};
...
static ScanConfig make_scan_config(const GameKnowledge& knowledge,
  ...
  cfg.metadata_tag_element = knowledge.get(game_id, "metadata_tag_element", "");
  ...
  auto checker = Game::Features::Registry::instance().resolve_mod_data_checker(game_id);
  if (checker) {
...
scan_entry(const std::filesystem::path& entry_path, const ScanConfig& cfg,
  ...
  if (!cfg.separator_suffix.empty() &&
      ...
      folder_name.compare(folder_name.size() - cfg.separator_suffix.size(),
                          cfg.separator_suffix.size(), cfg.separator_suffix) == 0) {
  ...
  if (cfg.use_xml_meta) {
    ...
    if (!std::filesystem::exists(metadata_path)) {
      ...
      if (raw_name.empty()) {
        ...
        if (!cfg.metadata_tag_element.empty() && !cfg.workshop_tag_categories.empty()) {
          const std::string needle = "<" + cfg.metadata_tag_element + " id=\"";
          ...
          if (!xml_tags.empty()) {
            mod.category_ids =
                map_workshop_tags_to_categories(xml_tags, cfg.workshop_tag_categories);
          }
        }
      }
    }
  ...
  if (!mod.validated) {
    ...
    if (!meta_content.empty()) {
      engine::ModMeta meta;
      if (meta.parse(meta_content) && meta.get("General", "validated") == "true") {
        mod.validated = true;
      }
    }
  }
  ...
  mod.is_empty = !found_real_file && !mod.is_separator && !mod.is_overwrite;
    ...
    if (!meta_content.empty()) {
      ...
      if (meta.parse(meta_content)) {
        ...
        if (!tags_csv.empty()) {
          ...
          mod.category_ids =
              map_workshop_tags_to_categories(tags, cfg.workshop_tag_categories);
        }
      }
    }
  }
...
ModScanner::prune_orphaned_empty_mods(const std::vector<ScannedMod>& scanned,
  ...
  for (const auto& mod : scanned) {
    ...
    std::error_code ec;
    if (!std::filesystem::is_directory(mods_dir / mod.folder_name, ec))
      continue;
