...
namespace {
  ...
  SemVer parse_version(const std::string &raw) {
    ...
    std::string numeric =
        (prerelease_pos != std::string::npos) ? s.substr(0, prerelease_pos) : s;
  ...
  int compare_prerelease(const std::string &a, const std::string &b) {
    ...
    for (size_t i = 0; i < len; ++i) {
      bool a_numeric = std::all_of(ids_a[i].begin(), ids_a[i].end(), [](char c) {
        return std::isdigit(static_cast<unsigned char>(c));
      });
      bool b_numeric = std::all_of(ids_b[i].begin(), ids_b[i].end(), [](char c) {
        return std::isdigit(static_cast<unsigned char>(c));
      });
    ...
    return ids_a.size() < ids_b.size() ? -1 : ids_a.size() > ids_b.size() ? 1 : 0;
  }
...
// GitHub::download
// ---------------------------------------------------------------------------
bool GitHub::download(const std::string &url, const std::filesystem::path &dest,
  ...
  req.headers.push_back("User-Agent: GameModManager/0.1");
  if (progress_cb) {
    req.progress.on = [cb = std::move(progress_cb)](
                          std::int64_t current, std::int64_t total, double /*bps*/) {
      if (total > 0)
        cb(static_cast<float>(current) / static_cast<float>(total));
    };
  }
