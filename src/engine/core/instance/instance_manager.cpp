...
InstanceManager::rename(const std::string &current_name,
  ...
  Logger::instance().info("Instance renamed: " + current_name + " -> " + unique_name);
...
InstanceManager::DeleteResult InstanceManager::remove(const std::string &name,
  ...
  if (was_active_flag && !force) {
    return {false, "Cannot delete active instance. Use force=true to override.", true};
  }
  ...
  if (ec) {
    return {false, "Failed to remove instance: " + ec.message(), was_active_flag};
  }
  ...
  // fields)
  {
    ...
    if (source_tbl) {
      ...
      if (out) {
        ...
        out.flush();
        invalidate_instance_toml_cache(dest.toml_path());
      }
    }
  }
...
InstanceManager::import_from_path(const fs::path &external_root,
  ...
  Logger::instance().info("Instance imported from path: " + external_root.string());
  return {true, "", inst, mods_imported, profiles_imported};
}

fs::path InstanceManager::instances_root() const {
  return root_;
}
