...
DeployConfig deploy_config_for(const fs::path &instance_root,
  ...
  cfg.case_sensitive =
      knowledge.get(game_id, "case_sensitive", "true") != "false";
...
LaunchParams prepare_launch_params(const std::filesystem::path &instance_root,
                                   ...
                                   const GameKnowledge &knowledge,
                                   const std::string &game_id,
                                   uint32_t steam_appid, bool is_windows_exe) {
...
LaunchParams prepare_launch_params(const LaunchPrepRequest &req,
  ...
  if (use_overlay) {
    ...
    Logger::instance().debug("Launch: OverlayFS staging at " +
                             staging_dir.string());
  } else if (use_direct) {
    ...
    deployed = deploy_all_enabled_mods_direct(
        ...
        deploy_cfg.deploy_prefix, deploy_cfg.deploy_include_mod_id,
        deploy_cfg.disable_mechanism, deploy_cfg.case_sensitive,
        deploy_cfg.ledger_file, deploy_cfg.backup_root, 0, progress);
    if (!deployed) {
      Logger::instance().warn(
          "Some mods failed to deploy into game_dir - continuing anyway");
    }
  ...
  if (use_overlay && req.local_saves_enabled && req.is_windows_exe &&
      req.platform) {
    ...
    if (feature && !ini_file.empty() && !sub.empty() && !appid_str.empty()) {
      ...
      if (!documents.empty()) {
        ...
        auto cfg = resolve_local_saves(mygames, req.instance_root, "Default",
                                       ini_file, true);
        ...
        if (!m.first.empty())
          Logger::instance().debug("Launch: local saves bind " +
                                   m.first.string() + " -> " +
                                   m.second.string());
      } else {
        Logger::instance().warn(
