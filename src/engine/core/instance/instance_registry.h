...
public:
  ...
  [[nodiscard]] std::optional<Entry> find_by_name(const std::string &name) const;
  ...
  // --- Mutations ---
  void register_instance(const std::string &name, const std::filesystem::path &root,
                         const std::string &type, const std::string &game_id,
                         const std::string &display_name);
  void unregister_instance(const std::string &name);
  void set_active(const std::string &name);  // marks one active, all others inactive
  void update_last_used(const std::string &name);  // bump timestamp
  void update_root(const std::string &name, const std::filesystem::path &new_root);
