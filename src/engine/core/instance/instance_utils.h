...
unique_instance_name(const std::string &display_name,
...
[[nodiscard]] std::string
instance_display_name(const std::filesystem::path &instance_root);
...
resolve_instance_path(const std::string &name_or_path);
...
// empty game_id).
[[nodiscard]] Instance
create_instance_for_game(const DetectedGame &game,
                         const std::filesystem::path &instances_root,
...
[[nodiscard]] inline Instance create_instance_for_game(const DetectedGame &game) {
...
[[nodiscard]] DeployConfig deploy_config_for(const std::filesystem::path &instance_root,
...
[[nodiscard]] std::string
effective_deploy_strategy(const std::filesystem::path &instance_root,
...
[[nodiscard]] LaunchParams prepare_launch_params(const LaunchPrepRequest &req,
...
[[nodiscard]] LaunchParams prepare_launch_params(
    const std::filesystem::path &instance_root, const std::filesystem::path &game_dir,
    const std::filesystem::path &executable, const GameKnowledge &knowledge,
    const std::string &game_id, uint32_t steam_appid, bool is_windows_exe);
