...
namespace {
  ...
  bool copy_directory_recursive(const fs::path &src, const fs::path &dst) {
    ...
    if (ec) {
      Logger::instance().warn("Failed to copy " + src.string() + " -> " + dst.string() +
                              ": " + ec.message());
      return false;
    }
    return true;
...
Mo2ImportResult import_mo2_instance(const fs::path &mo2_dir, const fs::path &gmm_root,
  ...
  if (!mo2_steam_appid_str.empty()) {
    try {
      ...
      Logger::instance().warn("Failed to parse steamAppID: " + mo2_steam_appid_str);
    }
  }
  ...
  Logger::instance().info("MO2 instance imported: " + inst_name + " (game=" + game_id +
