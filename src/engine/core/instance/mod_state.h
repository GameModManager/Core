...
// - list position, nesting depth, parent separator, separator color/collapsed
// (Workspace-1spd)
...
struct ModTrackingEntry {
  ...
  std::string
      parent_separator;         // folder_name of enclosing separator, empty = top-level
  ...
  std::vector<std::string>
      hidden_files;  // relative paths of hidden files within the mod
...
public:
  ...
  void set_nesting(const std::string &folder_name, const std::string &parent_separator,
                   int32_t depth);

  // Set separator visual properties.
  void set_separator_visual(const std::string &folder_name, const std::string &color,
                            bool collapsed);
  ...
  void set_hidden_files(const std::string &folder_name, std::vector<std::string> files);
