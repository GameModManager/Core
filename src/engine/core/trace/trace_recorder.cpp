...
void TraceRecorder::declare_flow(const std::string &flow_id, const std::string &title,
  ...
  for (size_t i = 0; i < flow.stages.size(); ++i) {
    ...
    flow.stages[i].status =
        flow.stages[i].implemented ? TraceStatus::Pending : TraceStatus::NotImplemented;
  }
  flow.running = false;
  flow.success = false;
...
void TraceRecorder::begin_flow(const std::string &flow_id) {
  ...
  for (auto &stage : flow.stages) {
    stage.status =
        stage.implemented ? TraceStatus::Pending : TraceStatus::NotImplemented;
    stage.reason.clear();
    stage.duration_ms  = 0;
...
std::optional<TraceRecorder::FlowSnapshot>
TraceRecorder::snapshot(const std::string &flow_id) const {
  std::lock_guard lock(mutex_);
  auto it = flows_.find(flow_id);
  if (it == flows_.end())
    return std::nullopt;
