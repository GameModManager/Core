...
void EventBus::dispatch(const std::string &event_id,
  ...
  {
    std::lock_guard<std::mutex> lock(mutex_);
    history_.push_back({event_id, payload_json, std::chrono::system_clock::now()});
  ...
  std::vector<Handler> to_call;
  {
    ...
    for (const auto &s : subs_)
      if (s.event_id == event_id)
        to_call.push_back(s.handler);
  }
  if (to_call.empty())
    return;
  for (auto &h : to_call)
    h(event_id, payload_json);
}
