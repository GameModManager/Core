...
public:
  using Handler =
      std::function<void(const std::string &event_id, const std::string &payload_json)>;
  ...
  [[nodiscard]] std::vector<EventRecord> recent_events(size_t max = kMaxHistory) const;
...
// -> {"mod":"SkyUI","enabled":"1"}
[[nodiscard]] std::string json_obj(std::vector<std::pair<std::string, std::string>> kv);
