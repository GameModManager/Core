  ...
  namespace Loot {
    ...
    namespace {
      ...
      bool fetch_file(const std::string &url, const fs::path &dest) {
        ...
        fs::remove(tmp, ec);
        long http_code = 0;
        const bool ok  = engine::download::curl_download(
            url, tmp, http_code, {}, nullptr, 0, nullptr, NET_CALLER);
        if (!ok) {
      ...
      std::optional<std::string> download_from_branches(const std::string &repo,
        ...
        for (const auto &branch : MasterlistManager::branch_candidates()) {
          if (fetch_file(raw_url(repo, branch, filename), dest))
            return branch;
        }
        return std::nullopt;
      }
    ...
    MasterlistManager::Masterlists MasterlistManager::ensure(const std::string &game_id,
                                                             const std::string &repo,
                                                             std::string *error) {
      return fetch_or_cache(game_id, repo, /*force=*/false, error);
    }

    MasterlistManager::Masterlists MasterlistManager::update(const std::string &game_id,
                                                             const std::string &repo,
                                                             std::string *error) {
      return fetch_or_cache(game_id, repo, /*force=*/true, error);
    }

    MasterlistManager::Masterlists
    MasterlistManager::fetch_or_cache(const std::string &game_id,
                                      const std::string &repo, bool force,
                                      std::string *error) {
      Masterlists out;
      ...
      if (force || !fresh_masterlist) {
        branch = download_from_branches(repo, kMasterlistFilename, masterlist_path);
      }
      ...
      if (force || !fresh_prelude) {
        if (branch && fetch_file(raw_url(kPreludeRepo, *branch, kPreludeFilename),
                                 prelude_path)) {
          prelude_ok = true;
        }
        if (!prelude_ok &&
            download_from_branches(kPreludeRepo, kPreludeFilename, prelude_path)) {
          prelude_ok = true;
        }
      }
      if (prelude_ok) {
