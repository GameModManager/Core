...
namespace Sorter {
  namespace Loot {
    ...
    public:
      ...
      Masterlists ensure(const std::string &game_id, const std::string &repo,
                         std::string *error = nullptr);

      // Force a re-download regardless of TTL (explicit "update masterlists").
      // Falls back to cache when offline, same as ensure().
      Masterlists update(const std::string &game_id, const std::string &repo,
                         std::string *error = nullptr);
    ...
    private:
      Masterlists fetch_or_cache(const std::string &game_id, const std::string &repo,
