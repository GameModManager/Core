...
namespace {
  ...
  std::string read_json_string(const std::filesystem::path &file,
...
theme_search_dirs(const std::filesystem::path &app_dir) {
  ...
  if (xdg && *xdg) {
    ...
    user_dir =
        std::filesystem::path(home) / ".local" / "share" / "GameModManager" / "themes";
  }
  if (!user_dir.empty())
...
void ThemeManager::scan_dir(const std::filesystem::path &themes_dir) {
  ...
  std::sort(themes_.begin(), themes_.end(), [](const ThemeInfo &a, const ThemeInfo &b) {
...
bool ThemeManager::load_tokens(const std::filesystem::path &tokens_file) {
  ...
  Logger::instance().debug("Loaded " + std::to_string(tokens_.size()) +
                           " tokens from " + tokens_file.string());
  return true;
}
...
std::string ThemeManager::apply_template(const std::string &qss_template) const {
  ...
  std::vector<std::pair<std::string, std::string>> sorted_tokens(tokens_.begin(),
                                                                 tokens_.end());
