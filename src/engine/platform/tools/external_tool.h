...
// §7: two categories - advisory (output feeds pipeline) and workshop (user launches
// directly).
enum class ToolKind {
...
public:
  ...
  [[nodiscard]] std::vector<ExternalTool>
  tools_for_game(const std::string &game_id) const;
  ...
  // Filtered by kind
  [[nodiscard]] std::vector<ExternalTool>
  advisory_tools_for(const std::string &game_id) const;
  [[nodiscard]] std::vector<ExternalTool>
  workshop_tools_for(const std::string &game_id) const;
...
private:
  ...
  std::unordered_map<std::string, std::unordered_map<std::string, ExternalTool>> tools_;
};
