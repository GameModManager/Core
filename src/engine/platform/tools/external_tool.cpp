...
ToolRegistry::tools_for_game(const std::string &game_id) const {
...
const ExternalTool *ToolRegistry::get_tool(const std::string &game_id,
  ...
  if (global_it != tools_.end()) {
    ...
    if (tool_it != global_it->second.end())
      return &tool_it->second;
  }
...
std::vector<ExternalTool>
ToolRegistry::advisory_tools_for(const std::string &game_id) const {
  auto all = tools_for_game(game_id);
  std::vector<ExternalTool> result;
  for (const auto &t : all) {
    if (t.kind == ToolKind::Advisory)
      result.push_back(t);
  }
  return result;
}
...
std::vector<ExternalTool>
ToolRegistry::workshop_tools_for(const std::string &game_id) const {
  auto all = tools_for_game(game_id);
  std::vector<ExternalTool> result;
  for (const auto &t : all) {
    if (t.kind == ToolKind::Workshop)
      result.push_back(t);
  }
  return result;
}
