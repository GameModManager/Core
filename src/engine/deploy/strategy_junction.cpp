#include "engine/deploy/junction.h"
...
bool create_junction(const std::filesystem::path &target,
  ...
  USHORT reparse_data_len =
      path_buffer_size + sizeof(MOUNT_POINT_REPARSE_BUFFER) - sizeof(WCHAR);
  ...
  HANDLE h = CreateFileW(
      target.wstring().c_str(), GENERIC_READ | GENERIC_WRITE, 0, nullptr,
  ...
  BOOL ok = DeviceIoControl(h, FSCTL_SET_REPARSE_POINT, buffer.data(),
...
bool Junction::deploy(const std::filesystem::path &source,
...
bool Junction::remove(const std::filesystem::path &target) {
  auto attrs = GetFileAttributesW(target.wstring().c_str());
  if (attrs != INVALID_FILE_ATTRIBUTES &&
...
bool Junction::is_available() { return true; }
...
bool Junction::deploy(const std::filesystem::path &,
                      const std::filesystem::path &) {
  return false;
}
bool Junction::remove(const std::filesystem::path &) { return false; }
bool Junction::is_available() { return false; }
