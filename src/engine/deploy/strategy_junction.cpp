...
namespace {
  ...
  bool create_junction(const std::filesystem::path &target,
    ...
    HANDLE h =
        CreateFileW(target.wstring().c_str(), GENERIC_READ | GENERIC_WRITE, 0, nullptr,
                    OPEN_EXISTING,
...
bool Junction::deploy(const std::filesystem::path &, const std::filesystem::path &) {
  return false;
}
bool Junction::remove(const std::filesystem::path &) {
  return false;
}
bool Junction::is_available() {
  return false;
