...
__attribute__((constructor)) void gmm_overlay_init(void) {
  ...
  while (gmm_overwrite_dir_len > 0 &&
         gmm_overwrite_dir[gmm_overwrite_dir_len - 1] == '/')
...
static int is_under_game_dir(const char *path) {
  if (!path || !gmm_game_dir)
    return 0;
  if (strncmp(path, gmm_game_dir, gmm_game_dir_len) != 0)
    return 0;
...
static char *rewrite_path(const char *path) {
  if (!path || !gmm_game_dir || !is_under_game_dir(path))
    return NULL;
...
int renameat(int olddirfd, const char *oldpath, int newdirfd,
  ...
  if (old_rw || new_rw) {
    if (new_rw)
      ensure_parent_dir(new_rw);
    int ret = real_renameat(AT_FDCWD, old_rw ? old_rw : oldpath, AT_FDCWD,
                            new_rw ? new_rw : newpath);
...
int mknod(const char *path, mode_t mode, dev_t dev) {
  if (!real_mknod)
    real_mknod = (real_mknod_t)dlsym(RTLD_NEXT, "mknod");
  char *rw = intercept_path(path);
  if (rw) {
    ensure_parent_dir(rw);
...
int mknodat(int dirfd, const char *path, mode_t mode, dev_t dev) {
  if (!real_mknodat)
    real_mknodat = (real_mknodat_t)dlsym(RTLD_NEXT, "mknodat");
  if (dirfd != AT_FDCWD)
