...
__attribute__((constructor)) static void gmm_ci_ctor(void) {
  ...
  if (realpath(root, gmm_ci_root) == NULL) {
    gmm_ci_enabled = 0;
    return;
  }
  size_t l = strlen(gmm_ci_root);
  while (l > 1 && gmm_ci_root[l - 1] == '/')
    gmm_ci_root[--l] = '\0';
  if (gmm_ci_debug)
    gmm_ci_log("enabled root=%s", gmm_ci_root);
}
...
static int under_root(const char *canon_path, const char **rel_out) {
  ...
  if (strncmp(canon_path, gmm_ci_root, rl) != 0)
    return 0;
...
static int ci_resolve(const char *path, char *out, size_t cap) {
  char canon[PATH_MAX];
  if (!gmm_canonicalize(path, canon, sizeof(canon)))
    return 0;
...
static statfn_t real_lstat_fn(void) {
  if (!F_lstat_real)
    F_lstat_real = (statfn_t)dlsym(RTLD_NEXT, "lstat");
  return F_lstat_real;
}
...
static int open_with_ci(const char *path, int flags, mode_t mode,
  ...
  if (ci_resolve(path, buf, sizeof(buf))) {
    ...
    if (gmm_ci_debug)
      gmm_ci_log("%s ci-open %s failed (%s)", what, buf, strerror(errno));
  }
  errno = saved;
  return -1;
}
...
int open(const char *path, int flags, ...) {
  if (!F_open)
    F_open = (openfn_t)dlsym(RTLD_NEXT, "open");
  va_list ap;
  va_start(ap, flags);
...
int openat(int dirfd, const char *path, int flags, ...) {
  if (!F_openat)
    F_openat = (openatfn_t)dlsym(RTLD_NEXT, "openat");
  va_list ap;
  va_start(ap, flags);
  mode_t mode = (flags & O_CREAT) ? (mode_t)va_arg(ap, int) : 0;
  va_end(ap);
...
int stat(const char *path, struct stat *sb) {
  if (!F_stat)
    F_stat = (statfn_t)dlsym(RTLD_NEXT, "stat");
...
int fstatat(int dirfd, const char *path, struct stat *sb, int flags) {
  if (!F_fstatat)
    F_fstatat = (fstatatfn_t)dlsym(RTLD_NEXT, "fstatat");
...
int __xstat(int ver, const char *path, struct stat *sb) {
  if (!F_xstat)
    F_xstat = (xstatfn_t)dlsym(RTLD_NEXT, "__xstat");
  int ret = F_xstat(ver, path, sb);
  if (!gmm_ci_enabled || ret == 0)
    return ret;
  int saved = errno;
  if (saved != ENOENT)
    return ret;
...
int __lxstat(int ver, const char *path, struct stat *sb) {
  if (!F_lxstat)
    F_lxstat = (xstatfn_t)dlsym(RTLD_NEXT, "__lxstat");
...
int __xstat64(int ver, const char *path, struct stat64 *sb) {
  if (!F_xstat64)
    F_xstat64 = (xstat64fn_t)dlsym(RTLD_NEXT, "__xstat64");
...
int __lxstat64(int ver, const char *path, struct stat64 *sb) {
  if (!F_lxstat64)
    F_lxstat64 = (xstat64fn_t)dlsym(RTLD_NEXT, "__lxstat64");
...
int access(const char *path, int mode) {
  if (!F_access)
    F_access = (accessfn_t)dlsym(RTLD_NEXT, "access");
  return access_with_ci(path, mode, F_access, "access");
}

int faccessat(int dirfd, const char *path, int mode, int flags) {
  if (!F_faccessat)
    F_faccessat = (faccessatfn_t)dlsym(RTLD_NEXT, "faccessat");
  if (!gmm_ci_enabled || dirfd != AT_FDCWD)
