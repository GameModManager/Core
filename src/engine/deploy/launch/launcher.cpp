...
static bool game_chain_alive_in_cgroup() {
  ...
  if (rel.find("/gmm-") == std::string::npos)
    return false;
  ...
  if (supervisor == 0) {
    ...
    if (game == 0) {
      ...
      if (!cgroup.path.empty()) {
        std::ofstream(cgroup.path + "/cgroup.procs") << getpid();
      }
  ...
  if (params.use_overlay &&
    ...
    if (game_chain_alive_in_cgroup()) {
      Logger::instance().debug(
          ...
          "cgroup - treating as launched (no fallback, avoiding "
          "double-launch)");
      return {getpid(), true};
    }
  ...
  // Fallback: standard runtime launch + post-hoc capture
  {
    ...
    if (!params.is_windows_exe && !fs::exists(exec_path, phys_ec) &&
      ...
      Logger::instance().error(
          ...
          "the OverlayFS launcher is required: " +
          exec_path.string());
    }
