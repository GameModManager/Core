...
bool OverlayFsLauncher::is_supported(const std::filesystem::path &upper_dir) {
  ...
  if (major == 5) {
    ...
    if (minor < 11) {
      Logger::instance().warn("Overlay: kernel " + std::to_string(major) + "." +
                              std::to_string(minor) + " < 5.11");
      return false;
    }
  }
  ...
  if (!upper_dir.empty()) {
    ...
    if (ret != 0) {
      Logger::instance().warn(
          "Overlay: filesystem at " + upper_dir.string() +
...
int64_t OverlayFsLauncher::launch(
    const std::filesystem::path &executable, const std::filesystem::path &game_dir,
    const std::filesystem::path &upper_dir, const std::vector<std::string> &args,
    const std::vector<std::filesystem::path> &extra_lowerdirs,
    const std::filesystem::path &bind_mount_source,
    const std::filesystem::path &bind_mount_target, const std::filesystem::path &cwd) {
  Logger::instance().debug(
      "OverlayFsLauncher::launch() executable=" + executable.string() +
  ...
  std::filesystem::create_directories(upper_dir, ec);
  if (ec) {
    Logger::instance().error("Overlay: failed to create upper_dir " +
  ...
  CloneArgs ca{executable,        game_dir, mount_point,     overlay_work,
               upper_dir,         args,     extra_lowerdirs, bind_mount_source,
               bind_mount_target, cwd,      stderr_fd,       outer_uid,
               outer_gid};
  ...
  pid_t pid = clone(
      [](void *arg) -> int {
        ...
        if (!ca->extra_lowerdirs.empty()) {
          ...
          for (const auto &p : ca->extra_lowerdirs) {
            ...
            if (access(s.c_str(), R_OK) == 0) {
              ...
              (void)write(STDERR_FILENO, "Overlay: warning: skipping extra_lowerdir ",
                          42);
          ...
          if (!valid.empty()) {
            ...
            if (mount(ca->game_dir.c_str(), orig_dir.c_str(), "", MS_BIND, NULL) != 0) {
              const char *e = strerror(errno);
              (void)write(STDERR_FILENO,
                          "Overlay: mount(bind orig game_dir) failed: ", 44);
              (void)write(STDERR_FILENO, e, strlen(e));
              (void)write(STDERR_FILENO, "\n", 1);
              _exit(16);
            }

            // Build lowerdir: valid extra layers first, then original game_dir as
            // bottom
        ...
        std::string data = "lowerdir=" + lowerdir_str +
                           ...
                           ",workdir=" + ca->overlay_work.string() + ",userxattr";

        if (mount("overlay", ca->mount_point.c_str(), "overlay", 0, data.c_str()) !=
            0) {
          const char *e = strerror(errno);
        ...
        if (mount(ca->mount_point.c_str(), ca->game_dir.c_str(), "", MS_BIND, NULL) !=
            0) {
          const char *e = strerror(errno);
          (void)write(STDERR_FILENO,
                      "Overlay: mount(bind overlay mount over game_dir) failed: ", 58);
          (void)write(STDERR_FILENO, e, strlen(e));
          (void)write(STDERR_FILENO, "\n", 1);
          _exit(10);
        }
        ...
        if (!ca->bind_mount_source.empty() && !ca->bind_mount_target.empty()) {
          if (access(ca->bind_mount_source.c_str(), R_OK | W_OK) == 0) {
            if (mount(ca->bind_mount_source.c_str(), ca->bind_mount_target.c_str(), "",
              ...
              const char *e = strerror(errno);
              (void)write(STDERR_FILENO,
                          "Overlay: bind mount (local saves) failed: ", 42);
              (void)write(STDERR_FILENO, e, strlen(e));
              (void)write(STDERR_FILENO, "\n", 1);
            } else {
              (void)write(STDERR_FILENO,
                          "Overlay: bind mount (local saves) installed\n", 43);
            }
          } else {
            (void)write(STDERR_FILENO,
                        "Overlay: bind mount (local saves) source missing, skipping\n",
                        60);
          }
        }
      ...
      stack + STACK_SIZE, CLONE_VFORK | CLONE_NEWUSER | CLONE_NEWNS | SIGCHLD, &ca);

  if (pid < 0) {
    int saved_errno = errno;
    delete[] stack;
    Logger::instance().error("Overlay: clone() failed: " +
  ...
  if (result == pid) {
    ...
    if (WIFEXITED(status)) {
      ...
      switch (code) {
      case 6:
        reason = "open(/proc/self/uid_map) failed";
        break;
      case 7:
        reason = "open(/proc/self/gid_map) failed";
        break;
      case 8:
        reason = "mount(MS_PRIVATE) failed";
        break;
      case 12:
        reason = "write(/proc/self/setgroups) failed";
        break;
      case 13:
        reason = "write(/proc/self/uid_map) failed";
        break;
      case 14:
        reason = "write(/proc/self/gid_map) failed";
        break;
      case 9:
        reason = "mount(overlay) failed";
        break;
      case 10:
        reason = "mount(bind overlay over game_dir) failed";
        break;
      case 11:
        reason = "execv + execl both failed";
        break;
      case 15:
        reason = "mkdtemp(/tmp/gmm_orig_XXXXXX) failed";
        break;
      case 16:
        reason = "mount(bind original game_dir to temp) failed";
        break;
      default:
    ...
    for (int i = 0; i < 20; i++) {
      ...
      if (result == pid) {
        ...
        if (WIFEXITED(status)) {
          ...
          Logger::instance().warn("Overlay: child PID " + std::to_string(pid) + " " +
                                  reason + " during grace window");
        } else if (WIFSIGNALED(status)) {
          reason = "killed by signal " + std::to_string(WTERMSIG(status));
          Logger::instance().warn("Overlay: child PID " + std::to_string(pid) + " " +
                                  reason + " during grace window");
        ...
        if (!log_ec && log_size > 0) {
          Logger::instance().warn("Overlay: stderr captured in " + log_path.string() +
                                  " (" + std::to_string(log_size) + " bytes)");
        }
        delete[] stack;
        return -1;
      }
