...
DirectDeployStrategy::DirectDeployStrategy(Config config)
    : config_(std::move(config))
    , file_strategy_(config_.case_sensitive) {}

// Per-file deploy: delegates to SymlinkStrategy (Liskov-compatible).
bool DirectDeployStrategy::deploy(const std::filesystem::path& source,
                                  const std::filesystem::path& target) {
    return file_strategy_.deploy(source, target);
}

// Per-file remove: delegates to SymlinkStrategy.
bool DirectDeployStrategy::remove(const std::filesystem::path& target) {
    return file_strategy_.remove(target);
}

// Full deploy: wraps deploy_all_enabled_mods_direct with owned config.
bool DirectDeployStrategy::deploy_all(const DeployProgressFn& progress) {
    return deploy_all_enabled_mods_direct(
...
bool DirectDeployStrategy::undeploy(const DeployProgressFn& progress) {
    return remove_deployed_files(
        config_.game_dir, config_.backup_root, config_.ledger_file,
        /*num_threads=*/0, progress);
}
...
SyncResult DirectDeployStrategy::sync(const DeployProgressFn& progress) {
    SyncResult result;
    const auto old_ledger = load_deploy_ledger(config_.ledger_file);
    const bool ok = deploy_all(progress);
    const auto new_ledger = load_deploy_ledger(config_.ledger_file);
    ...
    if (!ok) result.files_failed = -1;  // sentinel: partial failure
    return result;
}

bool DirectDeployStrategy::is_deployed(
    const std::filesystem::path& target) const {
    auto ledger = load_deploy_ledger(config_.ledger_file);
    return ledger.count(target) > 0;
}

const DeployedFileInfo* DirectDeployStrategy::find(
    const std::filesystem::path& target) const {
...
std::vector<DeployedFileInfo> DirectDeployStrategy::list_deployed() const {
    auto ledger = load_deploy_ledger(config_.ledger_file);
...
DirectDeployStrategy::current_ledger() const {
    return load_deploy_ledger(config_.ledger_file);
}
