...
VfsStrategy::VfsStrategy() = default;
...
bool VfsStrategy::deploy(const std::filesystem::path& source,
                          const std::filesystem::path& target) {
    std::lock_guard lock(mutex_);

    if (!std::filesystem::exists(source)) {
        Logger::instance().error("VFS deploy: source does not exist: " + source.string());
        return false;
    }
...
bool VfsStrategy::remove(const std::filesystem::path& target) {
    std::lock_guard lock(mutex_);
    file_map_.erase(target.string());
    return true;
}

bool VfsStrategy::mount(const std::filesystem::path& mount_point) {
    std::lock_guard lock(mutex_);

    if (mounted_) {
        Logger::instance().warn("VFS already mounted at " + mount_point.string());
        return false;
    }

    // Check if fuse3 is available
    if (!is_available()) {
        Logger::instance().error("FUSE not available on this system");
        return false;
    }
...
void VfsStrategy::add_file(const std::string& virtual_path, const std::string& source_path) {
    std::lock_guard lock(mutex_);
    file_map_[virtual_path] = source_path;
}

void VfsStrategy::remove_file(const std::string& virtual_path) {
    std::lock_guard lock(mutex_);
    file_map_.erase(virtual_path);
}

bool VfsStrategy::is_available() {
