...
// (via SymlinkStrategy internally), with a persistent ledger for O(Δ)
// incremental redeploys and original-file backup/restore.
//
// This is a Strategy that owns the full lifecycle:
//   - deploy_all(): full deploy of all enabled mods (replaces
//     deploy_all_enabled_mods_direct)
//   - undeploy():   remove all deployed files + restore originals (replaces
//     remove_deployed_files)
//   - sync():       incremental re-deploy of only changed files
//   - deploy() [inherited]: per-file symlink deploy (delegates to
//     SymlinkStrategy)
//   - remove() [inherited]: per-file symlink removal (delegates to
//     SymlinkStrategy)
//
...
class DirectDeployStrategy : public DeploymentStrategy {
public:
    struct Config {
    ...
    explicit DirectDeployStrategy(Config config);

    // --- DeploymentStrategy interface (per-file, inherited) ---
    ...
    // to do.
    [[nodiscard]] bool deploy_all(const DeployProgressFn& progress = {});
    ...
    [[nodiscard]] bool undeploy(const DeployProgressFn& progress = {});
    ...
    [[nodiscard]] SyncResult sync(const DeployProgressFn& progress = {});
    ...
    [[nodiscard]] const DeployedFileInfo* find(
    ...
    // SymlinkStrategy specifically).
    [[nodiscard]] const SymlinkStrategy& file_strategy() const {
        return file_strategy_;
    }
...
private:
    Config config_;
    SymlinkStrategy file_strategy_;  // per-file symlink/copy operations
};
