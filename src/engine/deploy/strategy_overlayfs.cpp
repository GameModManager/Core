...
OverlayFsKernel::OverlayFsKernel() = default;

OverlayFsKernel::~OverlayFsKernel() {
  if (mounted_) {
    unmount();
  }
}

bool OverlayFsKernel::deploy(const std::filesystem::path &source,
                             const std::filesystem::path &target) {
  // For OverlayFS, deploy is handled by the mount itself -
  // individual file mappings aren't needed when lowerdir order IS the priority
  // order
  (void)source;
  (void)target;
  return true;
...
bool OverlayFsKernel::remove(const std::filesystem::path &target) {
...
bool OverlayFsKernel::mount(
  ...
  if (mounted_) {
    engine::Logger::instance().warn("OverlayFS already mounted at " +
                                    mount_point.string());
    return false;
  }
  ...
  if (!is_available()) {
    engine::Logger::instance().error("OverlayFS not available on this system");
    return false;
  }
  ...
  if (ec) {
    engine::Logger::instance().error("Failed to create mount point: " +
                                     mount_point.string());
    return false;
  }
  ...
  if (result != 0) {
    engine::Logger::instance().error("OverlayFS mount failed with exit code " +
...
bool OverlayFsKernel::remount(
  ...
  if (!mounted_) {
    engine::Logger::instance().error("OverlayFS remount: not mounted");
    return false;
  }

  if (lower_dirs.empty()) {
    engine::Logger::instance().error(
        "OverlayFS remount: no lower directories provided");
    return false;
  }
...
bool OverlayFsKernel::is_available() {
...
bool OverlayFsKernel::can_mount_unprivileged() {
