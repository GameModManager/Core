...
static void count_nodes(const std::vector<TreeNode> &nodes, std::size_t &mods,
                        std::size_t &seps) {
  for (const auto &node : nodes) {
    if (auto *sep = std::get_if<SeparatorNode>(&node.data)) {
      ++seps;
...
static void validate_nodes(const std::vector<TreeNode> &nodes,
  ...
  for (size_t i = 0; i < nodes.size(); ++i) {
    ...
    if (auto *sep = std::get_if<SeparatorNode>(&node.data)) {
      ...
      if (mod->id.empty()) {
        diag.push_back(
            {Diagnostic::Severity::Error, loc + ".id", "mod id must not be empty"});
      }
      if (!seen_ids.insert(mod->id).second) {
        diag.push_back(
            {Diagnostic::Severity::Error, loc + ".id", "duplicate mod id: " + mod->id});
      }
    } else {
      diag.push_back({Diagnostic::Severity::Error, loc, "unknown node type"});
    }
  }
}
