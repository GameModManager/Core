...
ExtractResult extract_archive(const std::filesystem::path &archive_path) {
  ...
  if (archive_read_open_filename(a, archive_path.string().c_str(), 10240) !=
      ARCHIVE_OK) {
    result.diagnostics.push_back({Diagnostic::Severity::Error, "",
  ...
  while (archive_read_next_header(a, &entry) == ARCHIVE_OK) {
    ...
    while (total_read < size) {
      ...
      if (r == ARCHIVE_EOF || r == ARCHIVE_OK) {
        if (buf_size == 0)
          break;
        size_t to_copy = std::min(static_cast<size_t>(size - total_read), buf_size);
  ...
  if (it == result.archive.path_index.end()) {
    result.diagnostics.push_back(
        {Diagnostic::Severity::Error, "", "archive does not contain manifest.json"});
    return result;
  }

  try {
    result.manifest_json =
        nlohmann::json::parse(result.archive.files[it->second].content);
  } catch (const nlohmann::json::parse_error &e) {
    result.diagnostics.push_back({Diagnostic::Severity::Error, "manifest.json",
...
Diagnostics verify_archive_integrity(const ArchiveContents &archive,
  ...
  if (!manifest_json.contains("archive") || !manifest_json["archive"].is_object() ||
  ...
  for (auto it = hashes.begin(); it != hashes.end(); ++it) {
    ...
    if (!it.value().is_string()) {
      diag.push_back({Diagnostic::Severity::Error, "archive.fileHashes." + path,
    ...
    std::string actual_hex = sha256_hex(archive.files[fidx->second].content);
    if (actual_hex != expected_hex) {
      diag.push_back(
          {Diagnostic::Severity::Error, "archive.fileHashes." + path,
  ...
  for (const auto &f : archive.files) {
    ...
    if (!hashes.contains(f.path)) {
      diag.push_back({Diagnostic::Severity::Error, f.path,
...
Diagnostics validate_schemas(const ArchiveContents &archive,
  ...
  if (schemas.count("manifest.schema.json")) {
    ...
    auto manifest_diag = validator.validate(manifest_json, schema, schema);
  ...
  for (const auto &f : archive.files) {
    ...
    if (schemas.count(schema_name) == 0)
      continue;
...
Diagnostics check_referential_integrity(const Gmmpack &pack) {
  ...
  auto is_valid_id = [&](const std::string &id) {
    return mod_ids.count(id) || exec_ids.count(id) || synthetic_ids.count(id);
  };
  ...
  for (size_t i = 0; i < pack.manifest.rules.size(); ++i) {
    ...
    if (!is_valid_id(rule.from)) {
      diag.push_back(
          {Diagnostic::Severity::Error, prefix + ".from", "unknown id: " + rule.from});
    }
    if (!is_valid_id(rule.to)) {
      diag.push_back(
          {Diagnostic::Severity::Error, prefix + ".to", "unknown id: " + rule.to});
    }
  }
  ...
  for (size_t i = 0; i < pack.manifest.choice_groups.size(); ++i) {
    ...
    std::string prefix = "choiceGroups[" + std::to_string(i) + "]";
        ...
        for (size_t i = 0; i < nodes.size(); ++i) {
          if (auto *sep = std::get_if<SeparatorNode>(&nodes[i].data)) {
            check_tree(sep->children,
                       prefix + "nodes[" + std::to_string(i) + "].children");
          } else if (auto *mod = std::get_if<ModNode>(&nodes[i].data)) {
            if (!is_valid_mod_id(mod->id) && !synthetic_ids.count(mod->id)) {
              diag.push_back({Diagnostic::Severity::Error,
  ...
  for (const auto &p : pack.patches) {
    if (!is_valid_mod_id(p.mod_id)) {
      diag.push_back({Diagnostic::Severity::Error,
                      p.archive_path.empty() ? "patches/<unknown>.json.modId"
    ...
    if (!p.archive_path.empty()) {
      ...
      if (parsed_name) {
        ...
        if (filename_mod_id != p.mod_id) {
          diag.push_back({Diagnostic::Severity::Error, p.archive_path,
        ...
        if (filename_seq == 0 && p.sequence.has_value()) {
          ...
          diag.push_back({Diagnostic::Severity::Error, p.archive_path,
                          "filename sequence " + std::to_string(filename_seq) +
  ...
  for (const auto &[mod_id, seqs] : patch_sequences) {
    ...
    for (size_t i = 0; i < sorted.size(); ++i) {
      if (sorted[i] != static_cast<int>(i + 1)) {
        diag.push_back({Diagnostic::Severity::Error, "patches/" + mod_id,
                        "non-contiguous sequence: expected " + std::to_string(i + 1) +
  ...
  for (size_t fi = 0; fi < pack.ini_edits.size(); ++fi) {
    ...
    for (size_t ti = 0; ti < ini.tweaks.size(); ++ti) {
      ...
      if (tweak.has_source_mod_id && !is_valid_mod_id(tweak.source_mod_id)) {
        diag.push_back({Diagnostic::Severity::Error, where + ".sourceModId",
  ...
  auto check_platform_prefix_files = [&](const std::optional<PlatformOverride> &plat,
                                         const std::string &os_name) {
    if (!plat)
      return;
    for (size_t i = 0; i < plat->prefix_files.size(); ++i) {
      const auto &pf = plat->prefix_files[i];
      if (!pf.source_mod_id.empty() && !is_valid_mod_id(pf.source_mod_id)) {
        diag.push_back({Diagnostic::Severity::Error,
                        "platform." + os_name + ".prefixFiles[" + std::to_string(i) +
                            "].sourceModId",
...
static ModSource parse_mod_source(const nlohmann::json &j) {
  ...
  if (provider == "nexus") {
    ...
    if (j.contains("fileId"))
      s.file_id = j["fileId"].get<int64_t>();
...
ExecutableEntry parse_executable_entry(const nlohmann::json &j) {
  ...
  e.requires_virtual_fs_visible = j.value("requiresVirtualFsVisible", false);
...
UnpackResult unpack_gmmpack(const std::filesystem::path &archive_path,
  ...
  if (extract.manifest_json.contains("gmmpackSchema") &&
    ...
    std::string ver = extract.manifest_json["gmmpackSchema"].get<std::string>();
    auto dot1       = ver.find('.');
    if (dot1 != std::string::npos) {
      ...
      if (major != 1) {
        result.diagnostics.push_back({Diagnostic::Severity::Error, "gmmpackSchema",
                                      "unsupported major version: " + ver +
                                          " (only major version 1 is supported)"});
        result.diagnostics.insert(result.diagnostics.end(), extract.diagnostics.begin(),
  ...
  result.diagnostics.insert(result.diagnostics.end(), integrity_diag.begin(),
                            integrity_diag.end());
  if (!integrity_ok) {
    return result;
  }
  ...
  // Stage 2: Schema validation
  auto schema_diag = validate_schemas(extract.archive, extract.manifest_json, schemas);
  bool schema_ok   = true;
  ...
  result.diagnostics.insert(result.diagnostics.end(), ref_diag.begin(), ref_diag.end());
