...
bool split_filename(const std::string& filename, std::string& mod_id, int& seq) {
  ...
  if (stem.size() <= 5 || stem.substr(stem.size() - 5) != ".json") return false;
...
bool parse_patch_json(const std::string& json_text, const std::string& filename,
  ...
  BinaryPatch p;
  if (!need_str("modId", p.mod_id)) return false;
  if (!need_str("targetPath", p.target_path)) return false;
  if (!need_str("baseFileSha256", p.base_file_sha256)) return false;
  std::string algorithm;
  if (!need_str("algorithm", algorithm)) return false;
  if (!need_str("payloadBase64", p.payload_base64)) return false;
  ...
  if (file_seq != 0 &&
      (!p.sequence.has_value() || *p.sequence != file_seq)) {
...
bool group_patches(std::vector<BinaryPatch> entries,
  ...
  for (auto& [key, steps] : groups) {
    ...
    std::sort(steps.begin(), steps.end(), [](const BinaryPatch& a,
                                             const BinaryPatch& b) {
  ...
  std::sort(out_chains.begin(), out_chains.end(), [](const PatchChain& a,
                                                     const PatchChain& b) {
...
PatchPlan build_plan(std::vector<PatchChain> chains) {
  ...
  for (auto& c : plan.chains) {
    if (std::find(plan.affected_mods.begin(), plan.affected_mods.end(),
                  c.mod_id) == plan.affected_mods.end()) {
...
bool apply_chain(const PatchChain& chain, const std::filesystem::path& mod_dir,
  ...
  for (size_t i = 0; i < chain.steps.size(); ++i) {
    ...
    if (!bsdiff_apply(current.data(), current.size(), payload.data(),
                      payload.size(), next, patch_err)) {
      error = chain.mod_id + "/" + chain.target_path + ": " + patch_err;
      return false;
    }
    current = std::move(next);
  }
  std::string bytes(reinterpret_cast<const char*>(current.data()),
                    current.size());
...
bool apply_plan(
  ...
  for (auto& chain : plan.chains) {
    ...
    if (!apply_chain(chain, it->second, error)) return false;
  }
  return true;
}
