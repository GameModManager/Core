...
inline bool base64_decode(const std::string& in, std::vector<uint8_t>& out) {
  ...
  for (size_t i = 0; i < in.size(); i += 4) {
    ...
    if (c0 >= 128 || c1 >= 128 || kRev[c0] < 0 || kRev[c1] < 0) return false;
    // Padding may only appear as the last 1-2 chars of the whole input.
    bool end = (i + 4 == in.size());
    if (!end && (c2 == '=' || c3 == '=')) return false;
    if (c2 != '=' && (c2 >= 128 || kRev[c2] < 0)) return false;
    if (c3 != '=' && (c3 >= 128 || kRev[c3] < 0)) return false;
    if (c2 == '=') {
      if (c3 != '=') return false;  // padding must be the last 1-2 chars
      tmp.push_back(static_cast<uint8_t>((kRev[c0] << 2) | (kRev[c1] >> 4)));
    } else if (c3 == '=') {
      uint32_t n =
          (kRev[c0] << 18) | (kRev[c1] << 12) | (kRev[c2] << 6);
