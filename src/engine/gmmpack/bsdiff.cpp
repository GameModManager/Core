...
namespace {
  ...
  bool zlib_compress(const std::vector<uint8_t> &in, std::vector<uint8_t> &out,
    ...
    int rc = compress2(out.data(), &dest_len, in.data(), static_cast<uLong>(in.size()),
  ...
  std::vector<int64_t> build_suffix_array(const uint8_t *s, size_t n) {
    ...
    for (size_t k = 1; k < n; k *= 2) {
      ...
      rank = tmp;
      if (rank[sa[n - 1]] == static_cast<int64_t>(n - 1))
        break;
    }
    return sa;
  }
  ...
  int64_t matchlen(const uint8_t *old_data, int64_t old_len, const uint8_t *new_data,
                   int64_t new_len) {
    int64_t i = 0;
    while (i < old_len && i < new_len && old_data[i] == new_data[i])
      ++i;
    return i;
  }
  ...
  int64_t search_suffix(const std::vector<int64_t> &suffixes, const uint8_t *old_data,
    ...
    if (en - st < 2) {
      int64_t x =
          matchlen(old_data + suffixes[st], old_len - suffixes[st], new_data, new_len);
      int64_t y =
          matchlen(old_data + suffixes[en], old_len - suffixes[en], new_data, new_len);
      if (x > y) {
        pos = suffixes[st];
        return x;
      }
    ...
    if (std::memcmp(old_data + suffixes[x], new_data, static_cast<size_t>(cmp_len)) <
        0) {
      return search_suffix(suffixes, old_data, old_len, new_data, new_len, x, en, pos);
    }
    return search_suffix(suffixes, old_data, old_len, new_data, new_len, st, x, pos);
  }
...
bool bsdiff_create(const uint8_t *old_data, size_t old_len, const uint8_t *new_data,
    ...
    if (olds == 0) {
      ...
      while (scan < news) {
        ...
        for (scb = scan += len; scan < news; ++scan) {
          len = search_suffix(suffixes, old_data, olds, new_data + scan, news - scan, 0,
                              olds - 1, pos);
          for (; scb < scan + len; ++scb) {
            int64_t o = scb + lastoffset;
            if (o >= 0 && o < olds && old_data[o] == new_data[scb])
              ++oldscore;
          }
          if ((len == oldscore && len != 0) || len > oldscore + 8)
            break;
          int64_t o = scan + lastoffset;
          if (o >= 0 && o < olds && old_data[o] == new_data[scan])
            --oldscore;
        }
        if (len != oldscore || scan == news) {
          ...
          for (int64_t i = 0; i < lenf; ++i) {
            diff_raw.push_back(
                static_cast<uint8_t>(new_data[lastscan + i] - old_data[lastpos + i]));
          }
  ...
  if (!zlib_compress(ctrl_raw, ctrl, error))
    return false;
  if (!zlib_compress(diff_raw, diff, error))
    return false;
  if (!zlib_compress(extra_raw, extra, error))
    return false;
...
bool bsdiff_apply(const uint8_t *old_data, size_t old_len, const uint8_t *patch,
  ...
  std::vector<uint8_t> ctrl, diff, extra;
  if (!zlib_decompress(ctrl_cmp, ctrl_len, ctrl, error))
    return false;
  if (!zlib_decompress(diff_cmp, diff_len, diff, error))
    return false;
  if (!zlib_decompress(extra_cmp, extra_len, extra, error))
    return false;
  if (ctrl.size() % 24 != 0) {
