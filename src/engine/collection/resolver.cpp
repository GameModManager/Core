...
Resolver::validate_rules(const std::vector<ResolvedMod> &mods,
  ...
  for (const auto &rule : rules) {
    ...
    if (rule.type == RuleType::Requires) {
      ...
      if (to_present && !from_present) {
        diags.push_back({RuleDiagnostic::Severity::Error,
                         "Mod '" + rule.to + "' requires '" + rule.from +
    ...
    if (mod_ids.count(rule.from) == 0) {
      diags.push_back({RuleDiagnostic::Severity::Warning,
                       "Rule references unknown mod id '" + rule.from + "'", rule.from,
                       rule.to});
    }
    if (mod_ids.count(rule.to) == 0) {
      diags.push_back({RuleDiagnostic::Severity::Warning,
                       "Rule references unknown mod id '" + rule.to + "'", rule.from,
                       rule.to});
    }
  }
