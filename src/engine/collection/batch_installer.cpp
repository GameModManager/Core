...
namespace {
  ...
  struct HashExtractor {
    std::optional<std::string> operator()(const SourceNexus &s) const {
      return s.sha256.empty() ? std::nullopt
...
BatchInstaller::BatchInstaller(BatchInstallerDeps deps) : deps_(std::move(deps)) {}
  ...
  for (size_t i = 0; i < ordered.size(); ++i) {
    ...
    if (!rm.resolvable) {
      result.mods.push_back({id, ModVerdict::SkippedUnresolvable,
                             rm.error.empty() ? "Unknown source provider" : rm.error});
      ...
      if (auto want = expected_hash(rm.entry->source)) {
        if (auto have = deps_.installed_hash_for(id)) {
          if (normalize_hash(*have) == *want) {
            result.mods.push_back(
                {id, ModVerdict::SkippedUpToDate, "Installed hash matches manifest"});
    ...
    if (!deps_.install_one) {
      result.mods.push_back({id, ModVerdict::Failed, "No installer wired"});
      ++result.failed;
      report(id);
      continue;
    }
    ...
    if (outcome == PipelineResult::Canceled) {
      result.mods.push_back({id, ModVerdict::Canceled, "Install canceled by user"});
      result.canceled = true;
      report(id);
      // Everything after a user cancel was never attempted.
      for (size_t j = i + 1; j < ordered.size(); ++j) {
        const std::string rest =
            ordered[j]->entry ? ordered[j]->entry->id : ordered[j]->mod.id;
        result.mods.push_back(
            {rest, ModVerdict::NotAttempted, "Batch stopped after cancel"});
        report(rest);
      }
      break;
    }
