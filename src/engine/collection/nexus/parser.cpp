...
std::string require_string(const json& j, const char* key,
...
UpdatePolicy parse_update_policy(const std::string& s) {
    if (s == "latest") return UpdatePolicy::Latest;
...
RuleType parse_rule_type(const std::string& s) {
    if (s == "after") return RuleType::After;
    if (s == "requires") return RuleType::Requires;
    if (s == "conflicts") return RuleType::Conflicts;
        ...
        for (auto& [key, val] : mod_json["choices"].items()) {
            if (val.is_array()) {
                ...
                for (const auto& v : val) {
                    if (v.is_string()) opts.push_back(v.get<std::string>());
                }
...
Manifest parse(std::string_view json_str) {
    ...
    if (m.schema_version.empty()) m.schema_version = "nexus/1";
    ...
    if (root.contains("mods") && root["mods"].is_array()) {
        ...
        for (std::size_t i = 0; i < mods_arr.size(); ++i) {
            if (!mods_arr[i].is_object()) continue;
            m.mods.push_back(parse_mod(mods_arr[i], m.info.game_id,
                                       static_cast<int>(i)));
        }
    }
    ...
    if (root.contains("pluginLoadOrder") &&
        root["pluginLoadOrder"].is_array()) {
