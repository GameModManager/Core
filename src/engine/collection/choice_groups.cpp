...
bool same_members(const std::vector<std::string>& a, const std::vector<std::string>& b)
{
  if (a.size() != b.size()) {
    return false;
  }
...
ChoiceValidation validate_choice_groups(
  ...
  for (const auto& [key, _] : picks) {
    ...
    if (groupIt == groups.end()) {
      ...
      Logger::instance().debug(
          "validate_choice_groups: no group with id '" + key + "'");
    }
  }
...
ChoicePicks reconcile_prior_choices(const std::vector<ChoiceGroup>& groups,
    const PriorChoiceState& prior, const ChoicePicks& current_picks)
{
  ...
  for (const auto& group : groups) {
    ...
    if (priorMembersIt == prior.membership.end()
      ...
      effective.erase(group.id);
      Logger::instance().debug("reconcile_prior_choices: membership changed for group '"
          + group.id + "', dropping prior pick");
      continue;
    }
  ...
  for (auto it = effective.begin(); it != effective.end();) {
    const auto groupIt = std::find_if(groups.begin(), groups.end(),
        [&it](const auto& group) { return group.id == it->first; });
    if (groupIt == groups.end()) {
