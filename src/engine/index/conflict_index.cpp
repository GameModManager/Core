...
void ConflictIndex::set_mods_path(const std::string &mods_path) {
  mods_path_ = mods_path;
}

void ConflictIndex::set_conflict_extensions(
    const std::unordered_set<std::string> &extensions) {
  conflict_extensions_ = extensions;
}

void ConflictIndex::set_ignored_files(const std::unordered_set<std::string> &ignored) {
  ignored_files_ = ignored;
}

void ConflictIndex::scan(const std::string &db_path) {
  clear();
  ensure_schema(db_path);

  if (!fs::exists(mods_path_) || !fs::is_directory(mods_path_))
    return;
...
void ConflictIndex::rescan_mod(const std::string &mod_folder,
  ...
  std::sort(sorted_files.begin(), sorted_files.end());
  save_cache(db_path, mod_folder, fingerprint_folder(mod_path.string()), sorted_files,
             token);
...
void ConflictIndex::remove_mod(const std::string &mod_folder) {
...
ConflictIndex::owners_of(const std::string &relative_path) const {
...
bool ConflictIndex::has_conflict(const std::string &relative_path) const {
  auto it = conflicts_.find(relative_path);
  return it != conflicts_.end() && it->second.size() > 1;
}

std::string ConflictIndex::winner_of(const std::string &relative_path) const {
  auto it = conflicts_.find(relative_path);
  if (it == conflicts_.end() || it->second.empty())
    return {};
  return it->second.front().first;
}

std::unordered_set<std::string>
ConflictIndex::files_for_mod(const std::string &mod_folder) const {
  auto it = mod_files_.find(mod_folder);
  if (it == mod_files_.end())
    return {};
  return it->second;
}
...
std::unordered_set<std::string>
ConflictIndex::walk_mod(const std::string &mod_path) const {
  ...
  for (auto it = fs::recursive_directory_iterator(
    ...
    if (!conflict_extensions_.count(ext))
      continue;
...
std::string ConflictIndex::fingerprint_folder(const std::string &mod_path) const {
...
std::string ConflictIndex::quick_token(const std::string &mod_path) const {
  std::hash<std::string> hasher;
  size_t h = 0;

  std::error_code ec;
  auto mtime = fs::last_write_time(mod_path, ec);
  auto secs = std::chrono::duration_cast<std::chrono::seconds>(mtime.time_since_epoch())
                  .count();
  ...
  for (const auto &name : entries) {
    ...
    auto entry_secs =
        std::chrono::duration_cast<std::chrono::seconds>(entry_mtime.time_since_epoch())
            .count();
...
void ConflictIndex::ensure_schema(const std::string &db_path) const {
  sqlite3 *db = nullptr;
...
std::optional<ConflictIndex::CachedFingerprint>
ConflictIndex::load_cache(const std::string &db_path,
...
void ConflictIndex::save_cache(const std::string &db_path,
...
void ConflictIndex::delete_cache(const std::string &db_path,
                                 const std::string &mod_folder) const {
  sqlite3 *db = nullptr;
  if (sqlite3_open(db_path.c_str(), &db) != SQLITE_OK)
    return;
