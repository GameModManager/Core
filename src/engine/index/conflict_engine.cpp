...
ConflictEngine::walk_mod(const std::filesystem::path &mod_path,
...
void ConflictEngine::save_cache(const std::filesystem::path &cache_path,
...
std::unordered_map<std::string, ConflictStats> ConflictEngine::compute(
    const std::filesystem::path &mods_dir, const std::vector<ModInfo> &mods,
    const std::string &extensions_csv, const std::string &ignored_csv,
    bool conflict_reversed, const std::filesystem::path &cache_path,
    const std::filesystem::path &extra_mods_dir, const std::string &scan_dirs_csv) {
  if (mods_dir.empty() || mods.empty())
    return {};
  ...
  std::string filter_key =
      extensions_csv + "\x00" + ignored_csv + "\x00" + scan_dirs_csv;
