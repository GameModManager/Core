...
public:
  ...
  [[nodiscard]] std::string winner(const std::string &relative_path) const {
    return winner_of(relative_path);
  }
  ...
  void add_file(const std::string &relative_path, const std::string &mod_id,
                int priority);
...
private:
  ...
  std::optional<CachedFingerprint> load_cache(const std::string &db_path,
                                              const std::string &mod_folder) const;
  void save_cache(const std::string &db_path, const std::string &mod_folder,
                  const std::string &fingerprint, const std::vector<std::string> &files,
