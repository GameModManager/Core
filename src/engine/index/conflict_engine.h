...
public:
  ...
  //   extra_mods_dir  - additional dir to search for mod folders (e.g. game's native
  //   mods dir).
  //                     For each mod, mods_dir is tried first, then extra_mods_dir.
  //   mods            - list of (folder_name, priority) for every mod (excl. Overwrite
  //   / separators) extensions_csv  - comma-separated list of extensions to track (e.g.
  //   ".png,.lua").
  //                     If empty, all files are tracked.
  //   ignored_csv     - comma-separated list of filenames to skip (e.g.
  //   "metadata.xml,disable.it") conflict_reversed - if true, lower priority number =
  //   wins (Isaac convention) cache_path      - path to the JSON cache file (empty = no
  //   caching) scan_dirs_csv   - comma-separated list of subdirectory names to restrict
  //   scanning to
  //                     (e.g. "resources,resources-dlc3" for Isaac). Empty = scan
  //                     everything.
  [[nodiscard]] std::unordered_map<std::string, ConflictStats>
  compute(const std::filesystem::path &mods_dir, const std::vector<ModInfo> &mods,
          const std::string &extensions_csv, const std::string &ignored_csv,
          bool conflict_reversed, const std::filesystem::path &cache_path = {},
...
private:
  ...
  walk_mod(const std::filesystem::path &mod_path,
  ...
  void save_cache(const std::filesystem::path &cache_path, const CacheData &data) const;
