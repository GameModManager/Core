...
void StageRegistry::register_claim(const std::string &game_id,
                                   const std::string &stage_name, StageFn handler,
                                   int priority, const std::string &plugin_id) {
  StageClaim claim;
  ...
  for (const auto &claim : claims_) {
    ...
    if (claim.game_id.empty()) {
      ...
      if (claim.priority > best_wildcard_priority) {
        ...
        Logger::instance().warn("Conflicting wildcard stage claims for " + stage_name +
                                " - same priority, keeping first");
      }
      ...
      if (claim.priority > best_exact_priority) {
        ...
        Logger::instance().warn("Conflicting stage claims for " + game_id + "." +
