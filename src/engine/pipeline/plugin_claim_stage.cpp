...
PluginClaimStage::PluginClaimStage(std::string stage_name, std::string origin,
                                   StageFn handler)
    : stage_name_(std::move(stage_name)), origin_(std::move(origin)),
...
std::string PluginClaimStage::description() const {
  ...
  if (base.empty() || base == origin_)
    return "Provided by plugin " + origin_;
