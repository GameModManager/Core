...
namespace {
  ...
  public:
    StagingFileStateResolver(std::filesystem::path contentRoot,
                             std::filesystem::path gameDataDir)
        : contentRoot_(std::move(contentRoot)), gameDataDir_(std::move(gameDataDir)) {}
  ...
  std::string read_previous_choices(const std::filesystem::path &modsDir,
  ...
  bool flatten_into(const std::filesystem::path &stagingRoot,
...
bool FomodStage::execute(Mod &mod, PipelineContext &ctx) {
  ...
  if (stagingRoot.empty()) {
    Logger::instance().debug(
        "FomodStage: no extracted directory found, nothing to check");
    return true;
  }
  ...
  const auto moduleConfigPath = resolve_regular_file_ci(
      *fomodDir, toLower(std::string(fomod_files::MODULE_CONFIG)));
  ...
  const auto infoPath =
      resolve_regular_file_ci(*fomodDir, std::string(fomod_files::INFO_XML));
  if (!infoPath.empty()) {
    try {
      infoFile->deserialize(infoPath);
    } catch (const XmlParseException &e) {
      // info.xml is optional; a broken one only costs the metadata.
      Logger::instance().error("FomodStage: error parsing info.xml: " +
                               std::string(e.what()));
    }
  }
  ...
  std::string suggestedName =
      infoFile->getName().empty() ? mod.name : infoFile->getName();
  if (suggestedName.empty()) {
    suggestedName = mod.id;
  }

  // Previous choices from a reinstall, matched by mod folder name.
  const std::string previousChoices =
      read_previous_choices(ctx.mods_dir, suggestedName);
  ...
  if (!ctx.game_dir.empty() &&
      std::filesystem::is_directory(ctx.game_dir / ctx.deploy_prefix)) {
    gameDataDir = ctx.game_dir / ctx.deploy_prefix;
  }
  StagingFileStateResolver resolver(contentRoot, gameDataDir);
  auto viewModel = FomodViewModel::create(&resolver, nullptr, std::move(moduleConfig),
                                          std::move(infoFile));
  ...
  if (ctx.fomod_query_cb) {
    const auto decision =
        ctx.fomod_query_cb(viewModel, contentRoot, suggestedName, previousChoices);
    if (decision.manual) {
  ...
  if (!choicesJson.empty()) {
    try {
      ...
      Logger::instance().warn(
          "FomodStage: failed to parse previous choices; using defaults");
    }
  }
  ...
  if (!missing.empty()) {
    for (const auto &m : missing) {
      Logger::instance().warn(
          "FomodStage: FOMOD references a file missing from the archive: " + m);
    }
    if (!ignoreMissing) {
      Logger::instance().error(
