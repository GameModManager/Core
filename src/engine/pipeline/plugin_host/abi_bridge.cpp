...
static GmmModState to_gmm_state(ModState state) {
  switch (state) {
  case ModState::Downloaded:
    return GMM_MOD_DOWNLOADED;
  case ModState::Extracted:
    return GMM_MOD_EXTRACTED;
  case ModState::Installed:
    return GMM_MOD_INSTALLED;
  case ModState::Staged:
    return GMM_MOD_STAGED;
  case ModState::Deployed:
    return GMM_MOD_DEPLOYED;
  }
  return GMM_MOD_DOWNLOADED;
}
...
static InstanceKind to_instance_kind(GmmInstanceKind kind) {
  switch (kind) {
  case GMM_INSTANCE_MODS:
    return InstanceKind::Mods;
  case GMM_INSTANCE_PROFILES:
    return InstanceKind::Profiles;
  case GMM_INSTANCE_DOWNLOADS:
    return InstanceKind::Downloads;
  case GMM_INSTANCE_CACHE:
    return InstanceKind::Cache;
  case GMM_INSTANCE_LOGS:
    return InstanceKind::Logs;
  case GMM_INSTANCE_CONFIG:
    return InstanceKind::Config;
  case GMM_INSTANCE_MASTERLISTS:
    return InstanceKind::Masterlists;
  }
  return InstanceKind::Mods;
}
...
static ConflictIndex *as_conflict(GmmConflictIndexHandle h) {
  return reinterpret_cast<ConflictIndex *>(h);
}
static Profile *as_profile(GmmProfileHandle h) {
...
void gmm_conflict_add_file(GmmConflictIndexHandle h, const char *relative_path,
                           const char *mod_id, uint32_t priority) {
  auto *index = as_conflict(h);
  if (!index || !relative_path || !mod_id)
    return;
...
void gmm_profile_move_mod(GmmProfileHandle h, const char *mod_id,
