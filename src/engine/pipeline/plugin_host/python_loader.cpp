...
GmmPluginRequirement* py_requirements_bridge(size_t* out_count, void* user_data) {
    ...
    if (!p) { *out_count = 0; return nullptr; }
    py::gil_scoped_acquire acquire;
    try {
...
void py_diag_v1_bridge(const char* plugin_name,
                       char* out_buffer,
                       size_t out_capacity,
                       void* user_data) {
    ...
    try {
        ...
        if (py::isinstance<py::list>(result)) {
            for (auto item : py::cast<py::list>(result)) {
                ...
                if (!msg.empty()) messages.push_back(std::move(msg));
            }
        }
        size_t off = 0;
        for (const auto& msg : messages) {
        ...
        if (py::isinstance<py::list>(result)) {
            for (auto item : py::cast<py::list>(result)) {
                ...
                if (py::isinstance<py::tuple>(item)) {
                    py::tuple t = py::cast<py::tuple>(item);
                    if (py::len(t) >= 1) short_desc = py::cast<std::string>(t[0]);
                    if (py::len(t) >= 2) full_desc = py::cast<std::string>(t[1]);
                    if (py::len(t) >= 3) has_fix = py::cast<int>(t[2]);
                    if (py::len(t) >= 4) fix_fn = t[3];
                } else if (py::isinstance<py::dict>(item)) {
                    auto d = py::cast<py::dict>(item);
                    if (d.contains("short_description"))
...
int py_order_encoding_bridge(const char* const* ordered_mod_ids,
                             size_t count,
...
int py_deploy_bridge(const char* source, const char* target, void* user_data) {
    ...
    if (!p || p->deploy_fn.is_none()) return 0;
    py::gil_scoped_acquire acquire;
    try {
...
int py_remove_bridge(const char* target, void* user_data) {
    auto* p = static_cast<PyDeployProvider*>(user_data);
    if (!p || p->remove_fn.is_none()) return 0;
    py::gil_scoped_acquire acquire;
    try {
...
// shiboken6.getCppPointer(widget)[0] when building the widget with PySide6). The
// engine stores the void* exactly like a C plugin; the UI casts it back to
...
int py_modpage_bridge(const char* url, const char* output_path, void* user_data) {
    ...
    py::gil_scoped_acquire acquire;
    try {
        py::object result = p->fn(py::str(url ? url : ""),
                                  py::str(output_path ? output_path : ""));
        return py::cast<int>(result) != 0 ? 1 : 0;
    } catch (const py::error_already_set&) {
        PyErr_Clear();
        return 0;
...
int py_save_parser_bridge(const char* path, const char* game_id,
    ...
    py::gil_scoped_acquire acquire;
    try {
        py::object result = p->fn(py::str(path ? path : ""),
                                  py::str(game_id ? game_id : ""));
        if (result.is_none()) return 0;
        auto d = py::cast<py::dict>(result);
        std::memset(out, 0, sizeof(*out));
        if (d.contains("file_path")) out->file_path = dup_str(py::cast<std::string>(d["file_path"]));
        if (d.contains("game_id")) out->game_id = dup_str(py::cast<std::string>(d["game_id"]));
        if (d.contains("creation_time")) out->creation_time = py::cast<int64_t>(d["creation_time"]);
        if (d.contains("pc_name")) out->pc_name = dup_str(py::cast<std::string>(d["pc_name"]));
        if (d.contains("pc_level")) out->pc_level = py::cast<int32_t>(d["pc_level"]);
        if (d.contains("pc_location")) out->pc_location = dup_str(py::cast<std::string>(d["pc_location"]));
        if (d.contains("save_number")) out->save_number = py::cast<uint32_t>(d["save_number"]);
        ...
        if (d.contains("light_plugins")) {
            ...
            out->light_plugin_count = std::min<uint32_t>(static_cast<uint32_t>(pl.size()), 256u);
...
const char* const* py_sort_bridge(const char* const* mod_folders,
                                  size_t count,
                                  void* user_data) {
...
// Plugin paths loaded this session, used to scope registry teardown on shutdown.
...
public:
    ...
    void register_settings(const std::vector<std::pair<std::string, std::string>>& settings) {
        ...
        engine::PluginSettingsRegistry::instance().register_alias(plugin_->game_id, basename);
        engine::PluginSettingsRegistry::instance().register_alias(plugin_->plugin_name, basename);
    }
    ...
    void register_settings_tab(const std::string& title,
        ...
        engine::PluginSettingsRegistry::instance().register_settings_tab(
            basename, title.c_str(), keys.data(), types.data(),
            defaults.data(), options.data(), tab.settings.size());
        engine::PluginSettingsRegistry::instance().register_alias(plugin_->game_id, basename);
        engine::PluginSettingsRegistry::instance().register_alias(plugin_->plugin_name, basename);
    }
    ...
    void register_diagnostics(const std::string& game_id, py::object fn) {
        ...
        engine::Logger::instance().debug("Python plugin registered diagnostics for game=" + gid);
    }
    ...
    void register_file_mapper(const std::string& game_id, py::object fn) {
        ...
        engine::Logger::instance().debug("Python plugin registered file mapper for game=" + gid);
    }

    // -- v2 IPluginSaveParser --
    void register_save_parser(const std::string& game_id, py::object fn, int priority = 0) {
        if (!py::isinstance<py::function>(fn)) {
            engine::Logger::instance().warn(
        ...
        engine::SaveParserRegistry::instance().register_parser(
            ...
            [user_data](const std::filesystem::path& path,
                ...
                GmmSaveDataV2 c_out = {};
                if (!py_save_parser_bridge(path.string().c_str(), g.c_str(), &c_out, user_data)) {
        ...
        engine::Logger::instance().debug("Python plugin registered save parser for game=" + gid);
    }
    ...
    void register_hook(const std::string& tag, const std::string& data,
        ...
        ::HookRegistry::instance().register_hook(
            hook_tag.c_str(), py_hook_bridge, priority, user_data,
            plugin_->path.c_str());
    ...
    void register_order_encoding(py::object fn) {
        ...
        engine::Logger::instance().debug("Python plugin registered order encoding (game=" + game_id + ")");
    }
    ...
    void register_deploy_strategy(py::object deploy_fn, py::object remove_fn) {
        ...
        plugin_->remove_fn = py::isinstance<py::function>(remove_fn) ? py_remove_bridge : nullptr;
        ...
        engine::DeployStrategyRegistry::instance().register_provider(
            ...
            user_data, plugin_->path);
        engine::Logger::instance().debug("Python plugin registered deploy strategy (game=" + game_id + ")");
    }
    ...
    void register_tool(const std::string& tool_id, const std::string& kind,
        ...
        tool.kind = (kind_str == "workshop") ? engine::ToolKind::Workshop : engine::ToolKind::Advisory;
    ...
    void register_sort_provider(py::object fn) {
        ...
        engine::Logger::instance().debug("Python plugin registered sort provider for game=" + gid);
    }
    ...
    void register_game(const std::string& game_id,
                       ...
                       const std::string& exe_linux = "",
                       const std::string& exe_macos = "") {
        if (!game_id.empty()) plugin_->game_id = game_id;
        if (!display_name.empty()) plugin_->game_display_name = display_name;
        plugin_->steam_appid = steam_appid;
        plugin_->nexus_domain = nexus_domain;
        ...
        plugin_->game_support = true;
        engine::Logger::instance().debug("Python plugin registered game: id=" +
            plugin_->game_id + " name=" + plugin_->game_display_name +
            " nexus=" + nexus_domain);
    }
