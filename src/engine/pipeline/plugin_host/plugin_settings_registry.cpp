...
void PluginSettingsRegistry::register_settings(const std::string &plugin_id,
                                               ...
                                               size_t count) {
  if (plugin_id.empty() || !keys || !values || count == 0)
    return;
...
void PluginSettingsRegistry::register_settings_tab(
    const std::string &plugin_id, const char *title, const char *const *keys,
    const char *const *types, const char *const *defaults, const char *const *options,
    size_t count) {
  if (plugin_id.empty() || !title || !keys || !types || count == 0)
    return;
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    if (options && options[i]) {
      if (def.type == "choice") {
        ...
        if (!opts.empty())
          def.choices.emplace_back(std::move(opts));
...
void PluginSettingsRegistry::register_alias(const std::string &alias,
  ...
  if (alias.empty() || basename.empty() || alias == basename)
    return;
  std::lock_guard<std::mutex> lock(mutex_);
...
std::string PluginSettingsRegistry::get_setting(const std::string &plugin_id,
  ...
  if (sit != stores_.end()) {
    ...
    if (kit != sit->second.by_key.end())
      return kit->second.default_value;
  }
  return "";
}
...
const char *gmm_host_get_setting(const char *plugin_name, const char *key) {
  if (!plugin_name || !key)
    return "";
  g_host_get_buffer = PluginSettingsRegistry::instance().get_setting(plugin_name, key);
...
void gmm_host_set_setting(const char *plugin_name, const char *key, const char *value) {
  if (!plugin_name || !key)
    return;
  PluginSettingsRegistry::instance().set_setting(plugin_name, key, value ? value : "");
}
