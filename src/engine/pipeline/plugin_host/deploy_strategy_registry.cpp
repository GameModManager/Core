...
const DeployStrategyProvider *
DeployStrategyRegistry::get_provider(const std::string &game_id) const {
...
bool DeployStrategyRegistry::deploy(const std::string &game_id,
  ...
  if (!p || !p->deploy_fn)
    return false;
  const int ok =
      p->deploy_fn(source.string().c_str(), target.string().c_str(), p->user_data);
  return ok != 0;
}
