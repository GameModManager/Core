...
void RequirementsRegistry::register_requirements(const std::string &plugin_path,
  ...
  Logger::instance().debug("Registered requirements provider (plugin=" + plugin_path +
                           ")");
}

std::vector<UnmetRequirement>
RequirementsRegistry::check_requirements(const std::vector<PluginInfo> &plugins) const {
...
bool RequirementsRegistry::is_requirement_met(const std::string &type,
  ...
  if (type == "plugin") {
    for (const auto &p : plugins) {
      if (p.plugin_name == name || p.game_id == name || p.game_display_name == name) {
        return true;
      }
    }
    return false;
