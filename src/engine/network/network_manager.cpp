...
namespace redaction {
  ...
  namespace {
    ...
    bool name_matches(const std::string &header_name,
  ...
  bool is_sensitive_header(const std::string &header_name) {
    return name_matches(
        header_name,
        {"apikey", "authorization", "cookie", "set-cookie", "x-api-key", "x-auth-token",
         "x-csrf-token", "csrf-token", "proxy-authorization",
  ...
  std::string redact_url(const std::string &url) {
    ...
    if (host_end != std::string::npos) {
      ...
      while (i < url.size() && url[i] != '?' && url[i] != '#') {
        ...
        if (seg_end == std::string::npos)
          seg_end = url.size();
        const std::string segment = url.substr(seg_start, seg_end - seg_start);
        ...
        if (prev_name_start != std::string::npos) {
          ...
          const std::string prev_name  = url.substr(name_start, seg_start - name_start);
          if (is_sensitive_header(prev_name))
            redact = true;
        }
        ...
        if (!redact) {
          const std::string seg_name =
              segment.size() && segment.front() == '/' ? segment.substr(1) : segment;
          if (looks_like_token(seg_name))
            redact = true;
        }
      ...
      if (i < url.size())
        out.append(url, i, std::string::npos);
    ...
    std::string frag_suffix = (frag == std::string::npos) ? "" : rewritten.substr(frag);
  ...
  std::string redact_body(const std::string &body, std::size_t preview_bytes) {
    ...
    const bool json_form =
        body.find('"') != std::string::npos && body.find(':') != std::string::npos;
    if (body.empty() || (!url_form && !json_form)) {
      if (body.size() > preview_bytes)
        return body.substr(0, preview_bytes) + "... <truncated>";
      return body;
    }
    ...
    auto scrub_pairs = [&](const std::string &body, char sep, char eq) {
      ...
      while (pos < body.size()) {
        ...
        if (e != std::string::npos) {
          ...
          while (key_start < key.size() && key[key_start] != '"')
            ++key_start;
          ...
          if (is_sensitive_header(key_only_str)) {
            ...
            if (val_open < segment.size() &&
              ...
              if (val_close == std::string::npos)
                val_close = segment.size();
            }
            // Keep the opening quote (val_open includes it) and the
            // closing one (val_close).
            out += segment.substr(0, val_open + (quoted ? 1 : 0)) + "<redacted>" +
                   (quoted && val_close < segment.size() ? segment.substr(val_close)
                                                         : std::string());
          } else {
            out += segment;
          }
...
namespace {
  ...
  size_t write_to_string(char *ptr, std::size_t size, std::size_t nmemb,
                         void *userdata) {
  ...
  size_t capture_headers(char *ptr, std::size_t size, std::size_t nmemb,
                         void *userdata) {
    const std::size_t bytes = size * nmemb;
  ...
  int xfer_forwarder(void *userdata, curl_off_t dltotal, curl_off_t dlnow,
    ...
    if (pf->cb->on) {
      ...
      const double seconds = std::chrono::duration<double>(now - pf->started).count();
      ...
      pf->cb->on(pf->cb->resume_base + dlnow, pf->cb->resume_base + dltotal, bps);
    }
    return 0;
  }
...
int parse_retry_after_ms(const std::string &response_headers) {
  ...
  value_start = pos;
  while (pos < response_headers.size() && response_headers[pos] != '\r' &&
...
void Manager::apply_options(CURL *curl) const {
  ...
  curl_easy_setopt(curl, CURLOPT_USERAGENT, "GameModManager/" VERSION);
...
CURL *Manager::prepare_request(const Request &req, Response &out_resp,
  ...
  if (!opts_.use_proxy && opts_.use_http3 && req.url.rfind("https://", 0) == 0) {
...
CURL *Manager::prepare_download(DownloadRequest &req, DownloadResult &out_res,
...
Response Manager::request(const Request &req) {
  ...
  if (!curl) {
    ...
    if (priv) {
      if (priv->slist)
        curl_slist_free_all(priv->slist);
      delete priv->cap;
      delete priv;
    }
    curl_easy_cleanup(curl);
  ...
  for (int i = 0; i < attempts; ++i) {
    ...
    if (ra_ms > 0)
      sleep_ms = std::min(ra_ms, kMaxBackoffMs);
  ...
  entry.request_headers_redacted = join_headers([&]() {
    ...
    for (const auto &h : req.headers)
      v.push_back(redaction::redact_header_line(h));
    return v;
  }());
  ...
  entry.response_headers_redacted = [&]() {
    ...
    for (std::size_t i = 0; i < resp.response_headers.size() && s.size() < 2048; ++i) {
      ...
      const char next =
          (i + 1 < resp.response_headers.size()) ? resp.response_headers[i + 1] : '\0';
      if (c == '\r' && next == '\n')
        continue;
      if (c == '\n')
        continue;
  ...
  if (!resp.error.empty()) {
    Logger::instance().debug("Network:: " + method_for_log + " " + url_for_log +
                             " failed: " + resp.error + " (HTTP " +
...
DownloadResult Manager::download(DownloadRequest &req) {
  Logger::instance().debug("[Network::Manager] download: caller=" + req.caller +
                           " url=" + req.url + " dest=" + req.dest.string() +
                           " resume_from=" + std::to_string(req.resume_from) +
                           " long_lived=" + (req.long_lived ? "true" : "false"));
  DownloadResult res;
  if (cancelled_) {
    Logger::instance().debug(
        "[Network::Manager] download: cancelled_ flag set, returning cancelled");
    res.error = "cancelled";
    return res;
  }
  {
    ...
    if (opts_.offline_mode) {
      Logger::instance().debug(
          "[Network::Manager] download: offline_mode enabled, returning offline");
      res.error = "offline";
      return res;
    }
  }
  ...
  Logger::instance().debug(
      "[Network::Manager] curl_easy_perform returned: rc=" + std::to_string(rc) + " (" +
      curl_easy_strerror(rc) + ")" + " http_code=" + std::to_string(res.http_code));
  ...
  entry.response_headers_redacted = [&]() {
    ...
    for (std::size_t i = 0; i < res.response_headers.size() && s.size() < 2048; ++i) {
      ...
      const char next =
          (i + 1 < res.response_headers.size()) ? res.response_headers[i + 1] : '\0';
      if (c == '\r' && next == '\n')
        continue;
      if (c == '\n')
        continue;
