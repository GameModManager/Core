...
ModPins mod_pins(const gmmpack::ModEntry& mod) {
  ...
  const auto policy = std::visit(
      [](const auto& s) -> std::string { return s.update_policy; },
      mod.source);
  ...
  std::visit(
      [&](const auto& s) {
        ...
        if constexpr (std::is_same_v<T, gmmpack::ModSourceNexus>) {
          ...
          if (s.version.has_value()) pins.version = *s.version;
          if (s.sha256.has_value()) pins.hash = *s.sha256;
...
std::optional<std::string> pin_change_reason(const gmmpack::ModEntry& mod,
  ...
  if (pins.latest) return std::string("latest re-check");
...
std::optional<IniKeyParts> split_ini_key(const std::string& key) {
  ...
  if (parts.target.empty() || parts.tweak.empty()) return std::nullopt;
  return parts;
}
...
bool qualifies_for_tree(const InstalledPackState& state,
                        const std::string& mod_id) {
  if (state.skip_in_update(mod_id)) return false;
...
UpdatePlan diff_update(
  ...
  const auto err = [&](const std::string& path, const std::string& msg) {
    plan.diagnostics.push_back(
        {gmmpack::Diagnostic::Severity::Error, path, msg});
  };
  if (!state.has_pack()) {
  ...
  if (new_pack.manifest.id != state.pack_id()) {
    err("pack", "pack id mismatch: " + new_pack.manifest.id + " != " +
                     state.pack_id());
    return plan;
  }
  if (new_pack.manifest.revision <= state.installed_revision()) {
    err("revision", "revision " +
                         std::to_string(new_pack.manifest.revision) +
  ...
  std::unordered_set<std::string> applied_patches(
      state.applied_patches().begin(), state.applied_patches().end());
  ...
  std::unordered_set<std::string> patch_changed_mods;
  {
    ...
    for (const auto& mod : new_pack.mods) new_mods[mod.id] = &mod;
    for (const auto& patch : new_pack.patches) {
      if (new_mods.find(patch.mod_id) == new_mods.end()) continue;
      if (state.skip_in_update(patch.mod_id)) continue;
      const std::string key = patch_key(patch);
      const std::string base = patch_base(patch);
      if (applied_patches.count(key) != 0) continue;
      PatchChange change;
      change.mod_id = patch.mod_id;
      change.patch_key = key;
      change.action = applied_bases.count(base) != 0
                          ? PatchChange::Action::Changed
    ...
    for (const auto& key : state.applied_patches()) {
      ...
      if (new_mods.find(mod_id) == new_mods.end()) continue;  // removals cover
      if (new_bases.count(base) != 0) continue;
      if (state.skip_in_update(mod_id)) continue;
      plan.patch_changes.push_back(
          {PatchChange::Action::Removed, mod_id, key});
    ...
    plan.patch_consent_mods.erase(
        std::unique(plan.patch_consent_mods.begin(),
  ...
  // -- per-mod diff --
  {
    ...
    for (const auto& mod : new_pack.mods) {
      ...
      if (state.skip_in_update(mod.id)) continue;  // manual or user-removed
    ...
    for (const auto& [mod_id, entry] : state.all_resolved_mods()) {
      if (new_mods.find(mod_id) != new_mods.end()) continue;
      if (entry.origin == PackModOrigin::Manual) continue;  // never pack-owned
  ...
  // -- tree diff: conforming mods follow the new layout, nothing else moves --
  {
    ...
    if (!state.tree_snapshot().empty()) {
      try {
        old_pos.clear();
        flatten_tree(gmmpack::parse_tree(
                         nlohmann::json::parse(state.tree_snapshot()))
                         .nodes,
                     "", old_pos);
        have_old_tree = true;
    ...
    if (have_old_tree) {
      ...
      for (const auto& mod : new_pack.mods) {
        if (!qualifies_for_tree(state, mod.id)) continue;
        if (removed.count(mod.id) != 0) continue;
        const auto new_it = new_pos.find(mod.id);
        if (new_it == new_pos.end()) continue;  // not in tree: widget default
        ...
        if (old_it == old_pos.end()) {
          ...
          plan.tree_changes.push_back({TreeChange::Action::Move, mod.id,
  ...
  // -- INI diff: per (targetFile, tweakId) --
  {
    ...
    const auto owner_removed = [&](const std::string& source_mod_id,
                                   bool has_source) {
      if (!has_source || source_mod_id.empty()) return false;
      const ResolvedModEntry* owner = state.resolved_mod(source_mod_id);
      return owner != nullptr &&
             owner->presence == PackModPresence::Removed;
    };
    ...
    for (const auto& old : applied) {
      ...
      if (!effective_enabled(tweak)) continue;  // user/author disabled: alone
    ...
    if (!current_ini_text.empty()) {
      ...
      const auto disk_edits = [&](const std::string& target)
        ...
        if (cached != disk_cache.end()) return cached->second;
        ...
        if (text != disk.end()) {
          if (auto parsed = parse_ini_content(text->second)) {
            for (const auto& edit : *parsed) {
              edits[ascii_lower(edit.section) + "\x1f" +
                    ascii_lower(edit.key)] = edit.value;
            }
          }
        }
      ...
      for (const auto& old : applied) {
        const std::string id = old.target + "\x1f" + old.tweak;
        const auto it = fresh.find(id);
        if (it == fresh.end()) continue;  // retract path owns it
        const gmmpack::IniTweak& tweak = it->second.tweak;
        if (!effective_enabled(tweak)) continue;
        if (!old.hash.empty() && old.hash != hash8(tweak.content)) {
          continue;  // reapply path owns it
        }
        auto parsed = parse_ini_content(tweak.content);
        if (!parsed.has_value()) continue;  // validated packs never hit this
        const auto& values = disk_edits(old.target);
        for (const auto& edit : *parsed) {
          const auto found = values.find(ascii_lower(edit.section) + "\x1f" +
                                         ascii_lower(edit.key));
  ...
  // -- informational diagnostics: user intent in force for this run --
  {
    ...
    for (const auto& [mod_id, entry] : state.all_resolved_mods()) {
      ...
      if (entry.presence == PackModPresence::Removed) removed.push_back(mod_id);
    }
