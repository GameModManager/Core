...
namespace {
  ...
  int snap_dpi(int raw) {
    ...
    for (int step : kDpiSteps) {
      if (std::abs(raw - step) < std::abs(raw - best))
        best = step;
    }
    return best;
  }
  ...
  SyncResult run_sync(const fs::path &program, const std::vector<std::string> &args,
    ...
    if (!proc.waitForStarted(kRegTimeoutMs))
      return r;
    r.started = true;
  ...
  std::optional<std::string> query_reg_value(const fs::path &wine,
    ...
    auto res = run_sync(wine, {"reg", "query", key, "/v", name}, prefix, kRegTimeoutMs);
    if (!res.started || res.exit_code != 0)
      return std::nullopt;
    const QString qname = QString::fromStdString(name);
    for (const auto &line : res.output.split('\n')) {
      const auto tokens = line.trimmed().split(QChar(' '), Qt::SkipEmptyParts);
  ...
  std::string set_reg_value(const fs::path &wine, const fs::path &prefix,
    ...
    auto res =
        run_sync(wine, {"reg", "add", key, "/v", name, "/t", type, "/d", data, "/f"},
                 prefix, kRegTimeoutMs);
    if (!res.started)
      return "could not start wine reg";
    if (res.exit_code != 0)
  ...
  std::string check_prefix(const fs::path &prefix) {
    if (prefix.empty())
      return "no Proton prefix configured";
    std::error_code ec;
    if (!fs::exists(prefix, ec))
      return "prefix not found: " + prefix.string();
    return {};
  }
  ...
  std::string ensure_corefonts_impl(const fs::path &prefix, uint32_t appid) {
    ...
    if (appid == 0)
      return "corefonts missing but no Steam appid for routing";
    ...
    if (tricks.empty()) {
      ...
      if (tricks.empty())
        return "neither winetricks nor protontricks found";
    }
    ...
    if (!res.started)
      return "could not start winetricks backend";
    if (res.exit_code != 0) {
      std::string msg =
          "corefonts install failed (exit " + std::to_string(res.exit_code) + ")";
  ...
  std::string ensure_font_smoothing_impl(const fs::path &prefix) {
    ...
    if (wine.empty())
      return "wine binary not found on PATH";
    struct Want {
      const char *name;
      const char *type;
      bool is_string;
  ...
  std::string ensure_dpi_impl(const fs::path &prefix, int target_dpi) {
    ...
    if (auto cur = query_reg_value(wine, prefix, kFontsKey, "LogPixels")) {
      if (auto n = parse_reg_number(*cur))
        correct = (*n == target_dpi);
    }
    if (!correct) {
...
#ifdef __linux__
  if (auto *gui = qobject_cast<QGuiApplication *>(QGuiApplication::instance())) {
