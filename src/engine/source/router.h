...
struct ModlLink {
  ...
  [[nodiscard]] bool valid() const { return !game_id.empty() && !direct_url.empty(); }
};
...
public:
  ...
  [[nodiscard]] static std::string normalize_modl_game_id(const std::string &host);

  // Match a nexus_domain to a game_id via the registered plugin's nexus_domain.
  // Returns empty string if no match.
  [[nodiscard]] static std::string
  match_game(const std::string &nexus_domain,
  ...
  [[nodiscard]] static DerivedSource
  derive_source_from_direct_url(const std::string &direct_url);
};
