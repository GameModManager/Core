  ...
  if (!engine::download::curl_download(mod.download_url, dest_path, http_code, opts,
    ...
    if (aborted) {
      ...
      Logger::instance().error(
          "ModlProvider: download rejected (HTTP " + std::to_string(http_code) +
          ") - the host may require authentication (not yet supported "
          "for modl:// links)");
    } else {
      Logger::instance().error("ModlProvider: download failed (HTTP " +
                               std::to_string(http_code) + ")");
    }
    return false;
  }
  Logger::instance().debug("ModlProvider: download complete -> " + dest_path.string());
  return true;
}
