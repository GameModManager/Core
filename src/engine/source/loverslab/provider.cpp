    ...
    for (auto it = begin; it != end; ++it) {
      ...
      try {
        ...
        if (j.is_object() && j.contains("@graph") && j["@graph"].is_array()) {
          const auto found =
              std::find_if(j["@graph"].begin(), j["@graph"].end(), check);
      ...
      if (j.is_object()) {
        ...
        if (j.contains("author")) {
          ...
          if (a.is_object())
            ...
            result.author =
                trim(Http::decode_html_entities(trim(a.get<std::string>())));
        }
...
ModInfoResult Provider::fetch_mod_info(const std::string &file_id_or_url) {
  ModInfoResult result;
  ...
  if (file_id_or_url.find("://") != std::string::npos) {
    ...
    const bool digits_only =
        std::all_of(file_id_or_url.begin(), file_id_or_url.end(), [](unsigned char c) {
          return std::isdigit(c);
        });
    if (!digits_only) {
      Logger::instance().debug("LoversLabProvider: fetch_mod_info id is not numeric");
      return result;
    }
  ...
  req.headers.push_back("User-Agent: GameModManager/0.1 (LoversLab Provider)");
  ...
  if (resp.http_code != 200) {
    Logger::instance().debug("LoversLabProvider: fetch_mod_info HTTP " +
