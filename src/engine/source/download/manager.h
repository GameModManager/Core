...
// caller: optional identifier for the Debug panel Network tab (use NET_CALLER at call
// site).
bool curl_download(const std::string &url, const std::filesystem::path &dest_path,
                   long &http_code, const Options &opts = {},
                   Progress *progress = nullptr, int64_t resume_from = 0,
                   bool *aborted = nullptr, const std::string &caller = {});
