...
namespace {
  ...
  network::DownloadRequest to_request(const std::string &url,
                                      ...
                                      const Options &opts, Progress *progress,
                                      int64_t resume_from, const std::string &caller) {
...
std::string url_path_basename(const std::string &url) {
  ...
  std::string path = url.substr(slash + 1, (q == std::string::npos) ? std::string::npos
                                                                    : q - slash - 1);
...
std::string parse_content_disposition_filename(const std::string &header_value) {
  ...
  std::string value =
      header_value.substr(pos + (encoded ? rfc5987.size() : plain.size()));
