...
NxmLink Router::parse(const std::string &url) {
  ...
  for (size_t i = 0; i + 1 < segments.size(); ++i) {
    if (segments[i] == "mods") {
      try {
        link.mod_id = std::stoll(segments[i + 1]);
      } catch (...) {
      }
      break;
    }
  ...
  // Find query string - could start with '?' or be embedded with '&'
  {
    ...
    auto parse_kv = [&](const std::string &qs) {
      ...
      while (p < qs.size()) {
        ...
        auto val = (amp != std::string::npos) ? qs.substr(eq + 1, amp - eq - 1)
                                              : qs.substr(eq + 1);
        ...
        if (key == "key")
          ...
          try {
            link.expire = std::stoll(val);
          } catch (...) {
          }
        } else if (key == "user_id") {
          try {
            link.user_id = std::stoll(val);
          } catch (...) {
          }
        }
...
ModlLink Router::parse_modl(const std::string &url) {
  ...
  if (raw_value.size() > kMaxDecoded * 3)
    return link;

  std::string decoded = engine::download::percent_decode(raw_value);
  if (decoded.empty() || decoded.size() > kMaxDecoded)
    return link;
  ...
  if (!istarts_with_ascii(decoded, "https://"))
    return link;
  link.direct_url = std::move(decoded);
  return link;
}

Router::DerivedSource
Router::derive_source_from_direct_url(const std::string &direct_url) {
  DerivedSource out;
  if (direct_url.empty())
    return out;
  ...
  const auto host_end   = direct_url.find_first_of("/?#", host_start);
  std::string host      = direct_url.substr(host_start, (host_end == std::string::npos)
                                                            ? std::string::npos
                                                            : host_end - host_start);
  for (auto &c : host)
    c = static_cast<char>(std::tolower(static_cast<unsigned char>(c)));
