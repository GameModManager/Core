...
void ManagedGames::load() {
  ...
  try {
    ...
    for (auto &[game_id, val] : j.items()) {
      if (!val.is_object() || !val.contains("sources"))
        continue;
      ...
      for (const auto &src : val["sources"]) {
        if (!src.is_object())
          continue;
        GameSource source;
        if (src.contains("source_id"))
          source.source_id = src["source_id"].get<std::string>();
        if (src.contains("website_url"))
          source.website_url = src["website_url"].get<std::string>();
        if (src.contains("nexus_domain"))
          source.nexus_domain = src["nexus_domain"].get<std::string>();
...
bool ManagedGames::save() const {
  ...
  for (const auto &entry : entries_) {
    ...
    for (const auto &src : entry.sources) {
      src_array.push_back({{"source_id", src.source_id},
...
void ManagedGames::remove_source(const std::string &game_id,
                                 const std::string &source_id) {
  auto it =
      std::find_if(entries_.begin(), entries_.end(), [&](const ManagedGameEntry &e) {
...
std::string ManagedGames::game_id_for_domain(const std::string &nexus_domain) const {
  for (const auto &e : entries_) {
    for (const auto &s : e.sources) {
      if (s.nexus_domain == nexus_domain)
        return e.game_id;
    }
  }
  return {};
