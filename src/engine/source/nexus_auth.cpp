...
Auth::Auth() : fallback_(config_dir()) {}
...
RateLimitInfo Auth::get_rate_limit() const {
  ...
  if (info.daily_limit == 0)
    info.daily_limit = j.value("limit", 0);
  if (info.daily_remaining == 0)
    info.daily_remaining = j.value("remaining", 0);
  if (info.daily_reset == 0)
    info.daily_reset = j.value("reset", int64_t{0});

  return info;
}

void Auth::update_rate_limit(int hourly_limit, int hourly_remaining,
                             int64_t hourly_reset, int daily_limit, int daily_remaining,
                             int64_t daily_reset) {
  std::error_code ec;
  std::filesystem::create_directories(config_dir(), ec);

  nlohmann::json j = {
      {"hourly_limit", hourly_limit},       {"hourly_remaining", hourly_remaining},
      {"hourly_reset", hourly_reset},       {"daily_limit", daily_limit},
      {"daily_remaining", daily_remaining}, {"daily_reset", daily_reset},
...
void Auth::set_user_info(const NexusUserInfo &info) {
  ...
  if (info.account_type == NexusUserInfo::AccountType::Premium)
    type = "premium";
  else if (info.account_type == NexusUserInfo::AccountType::Supporter)
    type = "supporter";

  nlohmann::json j = {
      {"user_id", info.user_id}, {"name", info.name}, {"account_type", type}};
