...
WorkshopClient::WorkshopClient(const std::string &db_path, int rate_limit,
                               int rate_window)
...
void WorkshopClient::ensure_schema() {
  ...
  sqlite3_exec(db, "ALTER TABLE workshop_items ADD COLUMN tags TEXT DEFAULT ''",
               nullptr, nullptr, nullptr);
...
std::optional<WorkshopItem> WorkshopClient::get_details(int64_t workshop_id) {
  ...
  if (dead_ids_.count(workshop_id))
    return std::nullopt;
...
void WorkshopClient::mark_dead(int64_t workshop_id) {
  ...
  sqlite3_stmt *stmt = nullptr;
  const char *sql =
      "INSERT OR REPLACE INTO workshop_items (id, status) VALUES (?, 'dead')";
  if (sqlite3_prepare_v2(db, sql, -1, &stmt, nullptr) == SQLITE_OK) {
    sqlite3_bind_int64(stmt, 1, workshop_id);
    sqlite3_step(stmt);
    sqlite3_finalize(stmt);
...
std::pair<int, std::optional<double>> WorkshopClient::rate_limit_state() const {
  ...
  for (double ts : request_timestamps_) {
    if (ts > now - rate_window_)
      count++;
  }

  if (count >= rate_limit_ && !request_timestamps_.empty()) {
    double oldest =
        *std::min_element(request_timestamps_.begin(), request_timestamps_.end());
...
void WorkshopClient::set_rate_limit(int limit, int window) {
  ...
  double now =
      std::chrono::duration<double>(std::chrono::system_clock::now().time_since_epoch())
          .count();
  request_timestamps_.erase(std::remove_if(request_timestamps_.begin(),
                                           request_timestamps_.end(),
                                           [&](double ts) {
                                             return ts <= now - rate_window_;
                                           }),
                            request_timestamps_.end());
}
