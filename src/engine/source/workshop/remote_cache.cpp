...
RemoteCache::RemoteCache(const std::string &url, const std::string &cache_path,
                         const std::string &bundled_path, std::chrono::seconds ttl,
                         const std::string &user_agent)
    : url_(url), cache_path_(cache_path), bundled_path_(bundled_path), ttl_(ttl),
...
void *RemoteCache::try_fetch() {
  ...
  if (!resp.error.empty() || resp.http_code != 200)
    return nullptr;
  if (resp.body.empty())
    return nullptr;
