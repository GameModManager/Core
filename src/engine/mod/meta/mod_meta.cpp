...
std::string ModMeta::get(const std::string& section, const std::string& key) const {
  ...
  if (it == sections_[idx].second.end())
    return {};
  return it->second;
}
  ...
  while (std::getline(in, line)) {
    ...
    if (trimmed_line[0] == '[') {
      ...
      if (current_section.empty())
        return false;  // empty section name
    ...
    if (current_section.empty())
      return false;  // value outside section
...
ModMeta ModMeta::from_default(const std::string& folder_name,
  ...
  if (!version.empty())
    meta.set("General", "version", version);
  if (!installation_file.empty())
    meta.set("General", "installationfile", installation_file);
...
bool ModMeta::is_mirrored() const {
  return has_section("Mirror");
}

std::string ModMeta::mirror_source_path() const {
  return get("Mirror", "sourcePath");
}

int64_t ModMeta::mirror_source_timestamp() const {
  auto v = get("Mirror", "sourceTimestamp");
  if (v.empty())
    return 0;
  try {
    return std::stoll(v);
...
bool ModMeta::write_game_metadata(const std::filesystem::path& mod_dir,
  ...
  if (!mf)
    return false;
  std::string name = display_name.empty() ? mod_dir.filename().string() : display_name;
  mf << "<?xml version=\"1.0\"?>\n"
     << "<mod>\n"
     << "  <name>" << name << "</name>\n"
