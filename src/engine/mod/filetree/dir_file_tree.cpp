...
DirectoryFileTree::DirectoryFileTree(std::shared_ptr<const FileTree> parent,
                                     ...
                                     std::filesystem::path path, bool ignore_meta_ini)
    : FileTree(std::move(parent), std::move(name), cmp), m_path(std::move(path)),
      m_ignore_meta_ini(ignore_meta_ini) {}

std::shared_ptr<FileTree> DirectoryFileTree::make_tree(const std::filesystem::path &dir,
                                                       NameCompare cmp,
                                                       bool ignore_meta_ini) {
  return std::make_shared<DirectoryFileTree>(nullptr, dir.filename().string(), cmp, dir,
                                             ignore_meta_ini);
}
...
bool DirectoryFileTree::do_populate(std::shared_ptr<const FileTree> parent,
  ...
  for (auto it = std::filesystem::directory_iterator(m_path, ec); it != end;
    ...
    if (is_dir) {
      ...
      if (m_ignore_meta_ini && name_equals(name, "meta.ini", m_cmp))
        continue;
