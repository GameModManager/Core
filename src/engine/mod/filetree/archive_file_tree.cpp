...
namespace {
  ...
  bool contains_child(const std::vector<FileTree::value_type> &children,
    ...
    for (const auto &child : children) {
      if (name_equals(child->name(), name, cmp))
        return true;
    }
    return false;
  }
...
ArchiveFileTree::ArchiveFileTree(
    std::shared_ptr<const FileTree> parent, std::string name, NameCompare cmp,
    std::filesystem::path archive, std::string prefix,
    std::shared_ptr<const std::vector<ArchiveEntryInfo>> entries)
    : FileTree(std::move(parent), std::move(name), cmp), m_archive(std::move(archive)),
      m_prefix(std::move(prefix)), m_entries(std::move(entries)) {}

std::shared_ptr<FileTree>
ArchiveFileTree::make_tree(const std::filesystem::path &archive, std::string *error,
                           NameCompare cmp) {
  auto arch = open_libarchive(archive);
  if (!arch) {
    if (error)
      *error = "no archive backend for " + archive.string();
    return nullptr;
  }
  ...
  for (const auto &info : *m_entries) {
    ...
    if (slash != std::string::npos) {
      ...
      if (!contains_child(out, first, m_cmp)) {
        out.push_back(std::make_shared<ArchiveFileTree>(parent, first, m_cmp, m_archive,
                                                        base + first, m_entries));
      }
    } else if (info.is_dir) {
      if (!contains_child(out, first, m_cmp)) {
        out.push_back(std::make_shared<ArchiveFileTree>(parent, first, m_cmp, m_archive,
