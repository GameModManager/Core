...
public:
  ...
  public:
    ...
    friend bool operator==(const_iterator a, const_iterator b) { return a.v == b.v; }
    friend bool operator!=(const_iterator a, const_iterator b) { return a.v != b.v; }
  ...
  static std::shared_ptr<FileTree>
  make_tree_from_directory(const std::filesystem::path &dir,
                           NameCompare cmp      = NameCompare::CaseInsensitive,
                           bool ignore_meta_ini = true);
  ...
  static std::shared_ptr<FileTree>
  make_tree_from_archive(const std::filesystem::path &archive,
  ...
  std::string path_to(const const_reference &entry, std::string_view sep = "/") const;
  ...
  void walk(const std::function<WalkReturn(const std::string &prefix,
...
protected:
  ...
  FileTree(std::shared_ptr<const FileTree> parent, std::string name, NameCompare cmp);
