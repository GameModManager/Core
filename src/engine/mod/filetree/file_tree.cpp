...
FileTreeEntry::FileTreeEntry(std::shared_ptr<const FileTree> parent, std::string name,
                             NameCompare cmp)
...
std::shared_ptr<FileTree>
FileTree::make_tree_from_directory(const std::filesystem::path &dir, NameCompare cmp,
                                   bool ignore_meta_ini) {
  return DirectoryFileTree::make_tree(dir, cmp, ignore_meta_ini);
}

std::shared_ptr<FileTree>
FileTree::make_tree_from_archive(const std::filesystem::path &archive,
...
const std::vector<FileTree::value_type> &FileTree::entries() const {
  if (!m_populated.load(std::memory_order_acquire))
    populate();
  return m_entries;
}
...
std::shared_ptr<FileTreeEntry> FileTree::at(std::size_t i) {
  auto &v = entries();
  if (i >= v.size())
    throw std::out_of_range("FileTree::at");
  return v[i];
}
...
std::shared_ptr<const FileTreeEntry> FileTree::find(const std::string &path) const {
  ...
  for (const auto &e : node->entries()) {
    if (name_equals(e->name(), last, node->m_cmp))
      return e;
  }
  return {};
}
...
std::shared_ptr<const FileTree>
FileTree::find_directory(const std::string &path) const {
...
namespace {
  ...
  bool walk_node(std::shared_ptr<const FileTree> tree, const std::string &prefix,
    ...
    for (auto entry : *tree) {
      ...
      if (r == FileTree::WalkReturn::Stop)
        return false;
      if (r == FileTree::WalkReturn::Skip)
        continue;
      if (entry->is_dir()) {
        if (!walk_node(entry->as_tree(), prefix + entry->name() + std::string(sep), cb,
                       sep)) {
          return false;
        }
      }
