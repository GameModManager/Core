...
ReplayResult replay_fomod_choices(FomodViewModel &view_model,
                                  const Collection::InstallerChoices &choices) {
  ReplayResult result;

  if (choices.type != "fomod") {
    Logger::instance().warn("replay_fomod_choices: unsupported installer type '" +
                            choices.type + "'");
  ...
  for (const auto &[key, pluginNames] : choices.selections) {
    ...
    const auto stepIt =
        std::find_if(steps.begin(), steps.end(), [&stepName](const auto &step) {
    ...
    const auto groupIt =
        std::find_if(groups.begin(), groups.end(), [&groupName](const auto &group) {
          return group->getName() == groupName;
        });

    if (groupIt == groups.end()) {
      result.unrecognized_keys.push_back(key);
      Logger::instance().debug("replay_fomod_choices: no group named '" + groupName +
                               "' in step '" + stepName + "'");
      continue;
    }
    ...
    for (const auto &pluginName : pluginNames) {
      ...
      const auto pluginIt =
          std::find_if(plugins.begin(), plugins.end(), [&pluginName](const auto &p) {
            return p->getName() == pluginName;
          });

      if (pluginIt == plugins.end()) {
        result.unrecognized_options.push_back(pluginName);
        Logger::instance().debug("replay_fomod_choices: no plugin named '" +
                                 pluginName + "' in group '" + groupName +
                                 "' of step '" + stepName + "'");
        continue;
      }
  ...
  Logger::instance().info(
      ...
      " selection(s), " + std::to_string(result.unrecognized_keys.size()) +
      " unrecognized key(s), " + std::to_string(result.unrecognized_options.size()) +
