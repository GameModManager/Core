...
bool FomodConditionTester::isStepVisible(
    const std::shared_ptr<FlagMap> &flags,
        ...
        for (const auto &plugin : group->getPlugins()) {
          if (std::any_of(plugin->getPlugin()->conditionFlags.flags.begin(),
                          ...
                          [&flagDependency](const ConditionFlag &flag) {
                            return flag.name == flagDependency.flag &&
                                   flag.value == flagDependency.value;
                          })) {
  ...
  const auto anyVisible =
      std::any_of(stepsThatSetThisFlag.begin(), stepsThatSetThisFlag.end(),
                  [this, &steps, &flags](const int index) {
                    return isStepVisible(flags, steps[index]->getVisibilityConditions(),
                                         index, steps);
                  });
  if (!anyVisible) {
    Logger::instance().debug("Step " + steps[stepIndex]->getName() +
                             " has no dependent steps that are visible.");
    Logger::instance().debug("Steps that set this flag: " +
                             setToString(stepsThatSetThisFlag));
  }
  return anyVisible;
}
...
bool FomodConditionTester::testCompositeDependency(
  ...
  if (globalOperatorType == OperatorTypeEnum::AND) {
    return std::all_of(results.begin(), results.end(), [](const bool result) {
      return result;
    });
  }
  return std::any_of(results.begin(), results.end(), [](const bool result) {
    return result;
  });
}

bool FomodConditionTester::testFlagDependency(const std::shared_ptr<FlagMap> &flags,
...
bool FomodConditionTester::testFileDependency(
    const FileDependency &fileDependency) const {
...
bool FomodConditionTester::testGameDependency(
    const GameDependency &gameDependency) const {
  if (mGameVersionProvider == nullptr) {
    Logger::instance().warn(
        "FomodConditionTester: no game-version provider; assuming version condition '" +
        gameDependency.version + "' is satisfied");
    return true;
  }
  const auto gameVersion = mGameVersionProvider->game_version();
  Logger::instance().debug("Comparing condition version " + gameDependency.version +
                           " against " + gameVersion);
...
FileDependencyTypeEnum
FomodConditionTester::getFileDependencyState(const std::string &fileName) const {
  if (const auto it = fileDependencyCache.find(fileName);
      it != fileDependencyCache.end()) {
    return it->second;
  }
  ...
  if (mFileStateResolver != nullptr) {
    ...
    Logger::instance().warn(
        "FomodConditionTester: no file-state resolver; assuming file '" + fileName +
        "' is missing");
  }
