...
FomodViewModel::FomodViewModel(FomodFileStateResolver *fileStateResolver,
    ...
    : mFomodFile(std::move(fomodFile)), mInfoFile(std::move(infoFile)),
...
std::shared_ptr<FomodViewModel>
FomodViewModel::create(FomodFileStateResolver *fileStateResolver,
                       FomodGameVersionProvider *gameVersionProvider,
                       std::unique_ptr<ModuleConfiguration> fomodFile,
                       std::unique_ptr<FomodInfoFile> infoFile) {
  auto viewModel =
      std::make_shared<FomodViewModel>(fileStateResolver, gameVersionProvider,
                                       std::move(fomodFile), std::move(infoFile));
  ...
  if (viewModel->mSteps.empty()) {
    ...
    Logger::instance().info(
        "FomodViewModel: FOMOD with no steps - initialization complete");
    return viewModel;
  }
...
void FomodViewModel::forEachFuturePlugin(const int fromStepIndex,
                                         const PluginCallback &callback) const {
...
void FomodViewModel::createStepViewModels() {
  ...
  if (mFomodFile->installSteps.installSteps.empty()) {
    Logger::instance().info("FomodViewModel: No install steps found - creating default "
                            "step for legacy FOMOD");
    return;
  }
  ...
  for (int stepIndex = 0;
       stepIndex < static_cast<int>(mFomodFile->installSteps.installSteps.size());
       ++stepIndex) {
    const auto &installStep = mFomodFile->installSteps.installSteps[stepIndex];
    shared_ptr_list<GroupViewModel> groupViewModels;

    for (int groupIndex = 0;
         groupIndex < static_cast<int>(installStep.optionalFileGroups.groups.size());
         ++groupIndex) {
      ...
      for (int pluginIndex = 0;
           pluginIndex < static_cast<int>(group.plugins.plugins.size());
           ++pluginIndex) {
        const auto &plugin   = group.plugins.plugins[pluginIndex];
        auto pluginViewModel = std::make_shared<PluginViewModel>(
            std::make_shared<Plugin>(plugin), false, true, pluginIndex);
      ...
      if (groupViewModel->getType() == SelectAtMostOne &&
          groupViewModel->getPlugins().size() > 1) {
        createNonePluginForGroup(groupViewModel);
      }
      groupViewModels.emplace_back(groupViewModel);
    }
    auto stepViewModel =
        std::make_shared<StepViewModel>(std::make_shared<InstallStep>(installStep),
...
void FomodViewModel::createNonePluginForGroup(GroupRef group) {
  ...
  const auto nonePluginViewModel =
      std::make_shared<PluginViewModel>(nonePlugin, true, true, newIndex);
...
void FomodViewModel::enforceRadioGroupConstraints(GroupRef group) const {
  ...
  if (moreThanOneSelected(group)) {
    for (const auto &plugin : group->getPlugins()) {
      plugin->setSelected(
          false);  // don't call toggle here, that'll do the radio stuff.
    }
  }
...
void FomodViewModel::processPlugin(GroupRef group, PluginRef plugin) const {
  ...
  const auto typeDescriptor =
      mConditionTester.getPluginTypeDescriptorState(plugin->getPlugin(), mFlags);
  ...
  const bool isOnlyPlugin =
      group->getPlugins().size() == 1 &&
  ...
  case PluginTypeEnum::Optional:
    ...
    // In the case where we're changing flags to make something optional from
    // Recommended, set it back to unchecked.
...
void FomodViewModel::processPluginConditions(const int fromStepIndex) const {
  // We only want to update plugins that haven't been seen yet. Otherwise, we could undo
  // manual selections by the user.
  if (fromStepIndex >= 0) {
    forEachFuturePlugin(
        fromStepIndex, [this](const auto &groupViewModel, const auto &pluginViewModel) {
...
bool FomodViewModel::togglePlugin(GroupRef group, PluginRef plugin,
...
void FomodViewModel::updateVisibleSteps() const {
  ...
  for (int i = 0; i < static_cast<int>(mSteps.size()); ++i) {
    ...
    if (mConditionTester.isStepVisible(mFlags, mSteps[i]->getVisibilityConditions(), i,
                                       mSteps)) {
...
void FomodViewModel::stepBack() {
  ...
  const auto it = std::find(mVisibleStepIndices.begin(), mVisibleStepIndices.end(),
                            mCurrentStepIndex);
...
bool FomodViewModel::isLastVisibleStep() const {
  ...
  return !mVisibleStepIndices.empty() &&
         mCurrentStepIndex == mVisibleStepIndices.back();
}
...
bool FomodViewModel::isFirstVisibleStep() const {
  ...
  return !mVisibleStepIndices.empty() &&
         mCurrentStepIndex == mVisibleStepIndices.front();
}
    ...
    for (int groupIndex = 0; groupIndex < static_cast<int>(groupCount); ++groupIndex) {
      ...
      if (group.contains("plugins") && group["plugins"].is_array()) {
        for (const auto &jsonPlugin : group["plugins"]) {
          ...
          const auto currentPlugin = std::find_if(allPlugins.begin(), allPlugins.end(),
      ...
      for (const auto &jsonPlugin : group["deselected"]) {
        ...
        (*currentPlugin)->manuallySet =
            true;  // To preserve this state when serializing JSON.
      }
    }
  }
}
