...
PluginTypeEnum pluginTypeFromString(const std::string &typeStr) {
  if (typeStr == fomod_files::TYPE_REQUIRED)
    return PluginTypeEnum::Required;
  if (typeStr == fomod_files::TYPE_OPTIONAL)
    return PluginTypeEnum::Optional;
  if (typeStr == fomod_files::TYPE_RECOMMENDED)
    return PluginTypeEnum::Recommended;
  if (typeStr == fomod_files::TYPE_NOT_USABLE)
    return PluginTypeEnum::NotUsable;
  if (typeStr == fomod_files::TYPE_COULD_BE_USABLE)
    return PluginTypeEnum::CouldBeUsable;
  return PluginTypeEnum::Optional;
}

template <typename T>
bool deserializeList(pugi::xml_node &node, const char *childName,
                     std::vector<T> &list) {
...
bool PluginType::deserialize(pugi::xml_node &node) {
  name = pluginTypeFromString(node.attribute("name").as_string());
  return true;
}
...
bool CompositeDependency::deserialize(pugi::xml_node &node) {
  ...
  // If the dependencies are all right inside, just use the root node as the dependency
  // base. This looks hacky but accommodates both _nested_ dependencies for plugins, and
  // extremely simple ones for step visibility.
  if (node.child("dependencies") && !node.child("fileDependency") &&
      !node.child("flagDependency") && !node.child("gameDependency")) {
  ...
  if (const std::string operatorStr = possibleNode.attribute("operator").as_string();
      operatorStr == "Or") {
...
bool File::deserialize(pugi::xml_node &node) {
  ...
  isFolder = std::strcmp(node.name(), "folder") == 0;
...
bool PluginList::deserialize(pugi::xml_node &node) {
  ...
  order = getOrderType(node.attribute("order").as_string(), OrderTypeEnum::Ascending);

  // Sort the plugins based on the specified order
  std::sort(plugins.begin(), plugins.end(), [this](const Plugin &a, const Plugin &b) {
...
bool GroupList::deserialize(pugi::xml_node &node) {
  ...
  order = getOrderType(node.attribute("order").as_string());

  // Sort the groups based on the specified order
  std::sort(groups.begin(), groups.end(), [this](const Group &a, const Group &b) {
    if (order == OrderTypeEnum::Ascending) {
      return a.name < b.name;
    }
    if (order == OrderTypeEnum::Descending) {
...
bool ModuleConfiguration::deserialize(const std::filesystem::path &filePath) {
  pugi::xml_document doc;
  if (const pugi::xml_parse_result result = doc.load_file(filePath.c_str()); !result) {
    throw XmlParseException(
        std::format("XML parsed with errors: {}", result.description()));
  }
  ...
  pugi::xml_node conditionalFileInstallsNode =
      configNode.child("conditionalFileInstalls");
  conditionalFileInstalls.deserialize(conditionalFileInstallsNode);

  pugi::xml_node csharpScriptNode = configNode.child("csharpScript");
  csharpScript                    = csharpScriptNode.text().as_string();

  return true;
}

bool FomodInfoFile::deserialize(const std::filesystem::path &filePath) {
  pugi::xml_document doc;
