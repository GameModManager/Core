...
find_fomod_dir(const std::filesystem::path &dir) {
  std::error_code ec;
  ...
  for (const auto &entry : std::filesystem::directory_iterator(dir, ec)) {
    ...
    if (entry.is_directory() &&
        name_matches_ci(entry.path(), std::string(fomod_files::FOMOD_DIR))) {
      return entry.path();
    }
    singleChild = entry.path();
    ++entryCount;
  }
  if (entryCount == 1 && singleChild &&
      std::filesystem::is_directory(*singleChild, ec)) {
...
[[nodiscard]] inline std::shared_ptr<const FileTree>
find_fomod_dir(const std::shared_ptr<const FileTree> &tree) {
  if (!tree)
    return nullptr;
  ...
  if (tree->size() == 1) {
    auto only = tree->at(0);
    if (only->is_dir())
      return find_fomod_dir(only->as_tree());
  }
  return nullptr;
}
