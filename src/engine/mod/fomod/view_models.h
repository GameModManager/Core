...
public:
  PluginViewModel(const std::shared_ptr<Plugin> &plugin_, const bool selected,
                  bool /*enabled*/, const int index)
      : ownIndex(index), selected(selected), enabled(true), manuallySet(false),
        plugin(plugin_) {}
  ...
  [[nodiscard]] std::string getDescription() const {
    return plugin ? plugin->description : std::string();
  }
  [[nodiscard]] std::string getImagePath() const {
    return plugin ? plugin->image.path : std::string();
  }
  [[nodiscard]] bool isSelected() const { return selected; }
  [[nodiscard]] bool isEnabled() const { return enabled; }
  [[nodiscard]] int getOwnIndex() const { return ownIndex; }
  [[nodiscard]] std::vector<ConditionFlag> getConditionFlags() const {
    return plugin->conditionFlags.flags;
  }
  [[nodiscard]] PluginTypeEnum getCurrentPluginType() const {
    return currentPluginType;
  }
...
public:
  GroupViewModel(const std::shared_ptr<Group> &group_,
                 const shared_ptr_list<PluginViewModel> &plugins, const int index,
                 const int stepIndex)
      : plugins(plugins), group(group_), ownIndex(index), stepIndex(stepIndex) {}

  void addPlugin(const std::shared_ptr<PluginViewModel> &plugin) {
    plugins.emplace_back(plugin);
  }

  [[nodiscard]] std::string getName() const { return group->name; }
  [[nodiscard]] GroupTypeEnum getType() const { return group->type; }
  [[nodiscard]] const shared_ptr_list<PluginViewModel> &getPlugins() const {
    return plugins;
  }
  [[nodiscard]] int getOwnIndex() const { return ownIndex; }
  [[nodiscard]] int getStepIndex() const { return stepIndex; }
...
public:
  ...
  [[nodiscard]] CompositeDependency &getVisibilityConditions() const {
    return installStep->visible;
  }
  [[nodiscard]] std::string getName() const { return installStep->name; }
  [[nodiscard]] const shared_ptr_list<GroupViewModel> &getGroups() const {
    return groups;
  }
  [[nodiscard]] int getOwnIndex() const { return ownIndex; }
...
class FlagMap {
public:
  [[nodiscard]] std::vector<std::shared_ptr<PluginViewModel>>
  getPluginsSettingFlag(const std::string &key, const std::string &value) const {
  ...
  [[nodiscard]] FlagList getFlagsByKey(const std::string &key) const {
    ...
    // Sort plugins by stepIndex and ownIndex, stepIndex descending and ownIndex
    // ascending
    std::sort(orderedPlugins.begin(), orderedPlugins.end(),
              [](const auto &a, const auto &b) {
                return a.first > b.first ||
                       (a.first == b.first &&
                        a.second->getOwnIndex() < b.second->getOwnIndex());
              });
