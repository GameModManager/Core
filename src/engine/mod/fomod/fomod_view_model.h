...
public:
  FomodViewModel(FomodFileStateResolver *fileStateResolver,
  ...
  static std::shared_ptr<FomodViewModel>
  create(FomodFileStateResolver *fileStateResolver,
         FomodGameVersionProvider *gameVersionProvider,
         std::unique_ptr<ModuleConfiguration> fomodFile,
         std::unique_ptr<FomodInfoFile> infoFile);
  ...
  void
  forEachFuturePlugin(int fromStepIndex,
                      const std::function<void(GroupRef, PluginRef)> &callback) const;
  ...
  [[nodiscard]] const FomodConditionTester &condition_tester() const {
    return mConditionTester;
  }
  ...
  // Info
  [[nodiscard]] std::shared_ptr<InfoViewModel> getInfoViewModel() const {
...
private:
  ...
  void enforceSelectAllConstraint(
      const std::shared_ptr<GroupViewModel> &groupViewModel) const;

  void
  enforceSelectAtLeastOneConstraint(const std::shared_ptr<GroupViewModel> &group) const;
