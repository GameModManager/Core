...
FomodFileInstaller::FomodFileInstaller(const std::filesystem::path &modRoot,
    ...
    : mModRoot(modRoot), mViewModel(viewModel) {}

static bool copy_file_to(const std::filesystem::path &src,
...
static std::optional<std::filesystem::path> safe_join(const std::filesystem::path &root,
  ...
  if (!joined.string().starts_with(rootStr)) {
    Logger::instance().error("FomodFileInstaller: path escapes mod root, skipping: " +
                             relative);
    return std::nullopt;
  }
  return joined;
...
bool FomodFileInstaller::apply(std::vector<std::string> *missing) {
  ...
  const auto installTmp =
      mModRoot.parent_path() / (mModRoot.filename().string() + "_gmm_fomod_install");
  std::error_code ec;
  std::filesystem::remove_all(installTmp, ec);
  std::filesystem::create_directories(installTmp, ec);
  if (ec) {
    Logger::instance().error("FomodFileInstaller: failed to create install tree: " +
                             ec.message());
    return false;
  }
    ...
    if (!gf) {
      ...
      if (escaped) {
        Logger::instance().error(
            "FomodFileInstaller: path escapes mod root, skipping: " + file.source);
        continue;
      }
      Logger::instance().error("FomodFileInstaller: could not find source: " +
                               file.source);
      if (missing != nullptr)
  ...
  if (ec) {
    Logger::instance().error("FomodFileInstaller: failed to remove staging dir: " +
                             ec.message());
    std::filesystem::remove_all(installTmp, ec);
    return false;
  }
  ...
  if (ec) {
    Logger::instance().error("FomodFileInstaller: failed to swap install tree: " +
                             ec.message());
    std::filesystem::remove_all(installTmp, ec);
    return false;
  }
...
std::vector<engine::File> collect_files_to_install(const FomodViewModel &view_model) {
  ...
  allFiles.insert(allFiles.end(), requiredInstallFiles.files.begin(),
                  requiredInstallFiles.files.end());

  // Selected files from visible steps.
  for (const auto &stepViewModel : view_model.getSteps()) {
    if (!conditionTester.testCompositeDependency(
            flags, stepViewModel->getVisibilityConditions())) {
      continue;
    }
  ...
  std::stable_sort(allFiles.begin(), allFiles.end(),
                   [](const engine::File &a, const engine::File &b) {
