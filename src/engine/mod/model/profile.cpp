...
void ProfileModel::add_mod(const std::string &mod_id, bool enabled) {
  if (is_overwrite(mod_id))
    return;  // cannot add Overwrite manually
  remove_mod(mod_id);
  // Insert before the Overwrite entry (which is always last)
  auto it = std::find_if(mods_.rbegin(), mods_.rend(), [](const ProfileMod &m) {
    return false;
  });
  // Always insert before Overwrite
  auto ow = std::find_if(mods_.begin(), mods_.end(), [](const ProfileMod &m) {
    return m.mod_id == kProfileOverwriteId;
  });
  if (ow != mods_.end()) {
...
void ProfileModel::remove_mod(const std::string &mod_id) {
  if (is_overwrite(mod_id))
    return;  // cannot remove Overwrite
...
void ProfileModel::set_enabled(const std::string &mod_id, bool enabled) {
  if (is_overwrite(mod_id))
    return;  // Overwrite is always enabled
  for (auto &m : mods_) {
    if (m.mod_id == mod_id) {
      m.enabled = enabled;
      return;
...
void ProfileModel::move_mod(const std::string &mod_id, uint32_t new_position) {
  if (is_overwrite(mod_id))
    return;  // cannot move Overwrite

  auto it = std::find_if(mods_.begin(), mods_.end(), [&](const ProfileMod &m) {
    return m.mod_id == mod_id;
  });
  if (it == mods_.end())
    return;
...
void ProfileModel::ensure_overwrite_pinned() {
  for (const auto &m : mods_) {
    if (m.mod_id == kProfileOverwriteId)
...
int ProfileModel::overwrite_index() const {
  for (int i = 0; i < static_cast<int>(mods_.size()); ++i) {
    if (mods_[i].mod_id == kProfileOverwriteId)
      return i;
  }
  return -1;
}
