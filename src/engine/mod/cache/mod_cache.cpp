...
ModCache::ModCache(const std::string &db_path) : db_path_(db_path) {}
...
void ModCache::create_tables() {
  ...
  if (rc != SQLITE_OK) {
    Logger::instance().error(std::string("Failed to create tables: ") +
                             (err ? err : "unknown"));
    sqlite3_free(err);
  }
}
...
bool ModCache::add_mod(const CachedMod &mod) {
  const char *sql =
      "INSERT INTO mods (id, name, version, state, path, priority, enabled) "
      "VALUES (?, ?, ?, ?, ?, ?, ?);";
...
bool ModCache::update_mod(const CachedMod &mod) {
  const char *sql =
      "UPDATE mods SET name=?, version=?, state=?, path=?, priority=?, enabled=? "
      "WHERE id=?;";
...
CachedMod ModCache::get_mod(const std::string &id) const {
  CachedMod mod;
  const char *sql =
      "SELECT id, name, version, state, path, priority, enabled FROM mods WHERE id=?;";
...
std::vector<CachedMod> ModCache::get_all_mods() const {
  ...
  const char *sql =
      "SELECT id, name, version, state, path, priority, enabled FROM mods "
