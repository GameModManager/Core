...
namespace {
  ...
  void prune_up(const std::filesystem::path &start, const std::filesystem::path &root) {
    std::error_code ec;
    auto dir = start;
    while (dir != root && std::filesystem::is_directory(dir, ec) && !ec) {
  ...
  bool is_mapping_root(const std::filesystem::path &entry,
    ...
    std::transform(first_segment.begin(), first_segment.end(), first_segment.begin(),
                   ::tolower);
  ...
  bool move_entry_recursive(const std::filesystem::path &src,
    ...
    for (const auto &child : children) {
      ...
      auto mod_rel = overwrite_to_mod_rel(normalize_rel(rel.string()), mods_subpath,
      ...
      if (mod_rel.empty() && std::filesystem::is_directory(child, ec)) {
        ...
        if (!move_entry_recursive(child, overwrite_root, mod_root, mods_subpath,
                                  include_mod_id, mod_id))
          ok = false;
        continue;
      }
      ...
      if (std::filesystem::is_directory(child, ec)) {
        ...
        if (!move_file_robust(child, dest, ec)) {
          Logger::instance().error("overwrite_utils: failed to move " + child.string() +
  ...
  bool sync_file(const std::filesystem::path &overwrite_dir,
    ...
    auto mod_rel =
        overwrite_to_mod_rel(overwrite_rel, mods_subpath, include_mod_id, mod_id);
    ...
    if (std::filesystem::exists(dest, ec) && !ec) {
      ...
      if (ec) {
        Logger::instance().error("overwrite_utils: failed to remove " + dest.string() +
                                 ": " + ec.message());
        ec.clear();
        return false;
      }
    ...
    if (!move_file_robust(src, dest, ec)) {
      Logger::instance().error("overwrite_utils: failed to move " + src.string() +
                               " -> " + dest.string() + ": " + ec.message());
      ec.clear();
      return false;
    }
  ...
  std::filesystem::path resolve_child_ci(const std::filesystem::path &parent,
    ...
    for (const auto &entry : std::filesystem::directory_iterator(parent, ec)) {
      if (ec)
        break;
      if (name_matches_ci(entry.path(), name))
        return entry.path();
    }
    return exact;
  }
    ...
    for (const auto &child : children) {
      ...
      if (std::filesystem::is_directory(st)) {
        ...
        if (std::filesystem::is_directory(dest, de) && !de) {
          if (!merge_dir_into(dest, child))
            ok = false;
        } else {
          de.clear();
          if (!move_file_robust(child, dest, ec)) {
            Logger::instance().error("overwrite_utils: failed to move " +
                                     child.string() + " -> " + dest.string() + ": " +
                                     ec.message());
            ec.clear();
            ok = false;
          }
  ...
  std::size_t normalize_level(const std::filesystem::path &dir) {
    ...
    while (it != end && !ec) {
      ...
      if (!sec && st.type() != std::filesystem::file_type::symlink) {
        ...
        entries.push_back({it->path(), std::move(name), std::move(lower),
    ...
    for (const auto &[lower, group] : buckets) {
      ...
      for (size_t i = 1; i < dirs.size(); ++i) {
        ...
        if (c > keep_count || (c == keep_count && dirs[i]->name < keep->name)) {
          keep       = dirs[i];
          keep_count = c;
        }
      }
      ...
      for (const Entry *other : dirs) {
        ...
        if (merge_dir_into(keep->path, other->path)) {
          ...
          Logger::instance().error("overwrite CI-normalize: failed to merge " +
                                   other->path.string() + " into " +
                                   keep->path.string());
        }
      }
...
bool move_overwrite_to_mod(const std::filesystem::path &overwrite_dir,
  ...
  if (ec) {
    Logger::instance().error("overwrite_utils: cannot create " + mod_dir.string() +
                             ": " + ec.message());
    return false;
  }
  return move_entry_recursive(overwrite_dir, overwrite_dir, mod_dir, mods_subpath,
                              include_mod_id, mod_id);
}
...
bool move_overwrite_entry_to_mod(const std::filesystem::path &overwrite_dir,
  ...
  if (std::filesystem::is_directory(entry_path, ec)) {
    ...
    return move_entry_recursive(entry_path, overwrite_dir, mod_dir, mods_subpath,
                                include_mod_id, mod_id);
  }
  if (ec)
    return false;
  ...
  if (!move_file_robust(entry_path, dest, ec)) {
    Logger::instance().error("overwrite_utils: failed to move " + entry_path.string() +
                             " -> " + dest.string() + ": " + ec.message());
    ec.clear();
    return false;
  }
...
bool game_has_file(const std::filesystem::path &game_dir,
                   const std::string &overwrite_rel) {
  if (game_dir.empty() || overwrite_rel.empty())
    return false;
...
std::vector<OverwriteSyncFile> collect_overwrite_sync_files(
    ...
    const std::string &mods_subpath, bool conflict_reversed, bool include_mod_id,
  ...
  std::vector<std::string> files;
  {
    std::filesystem::recursive_directory_iterator rit(
        overwrite_dir, std::filesystem::directory_options::skip_permission_denied, ec);
    ...
    while (rit != rend) {
      ...
      if (rit->is_regular_file()) {
        ...
        if (!ec)
          files.push_back(normalize_rel(rel.string()));
      }
      rit.increment(ec);
    }
  }
  ...
  for (const auto &rel : files) {
    ...
    for (const auto &[registry_key, owners] : registry) {
      (void)registry_key;
      if (resolver.normalize(registry_key) != full_key)
        continue;
...
size_t apply_sync_plan(const std::vector<OverwriteSyncTarget> &targets,
                       ...
                       const std::string &mods_subpath,
                       const std::string &metadata_file, bool include_mod_id) {
  size_t moved = 0;

  std::set<std::string> ensured;
  for (const auto &target : targets) {
    if (target.mod_folder.empty())
      continue;
    ...
    if (!std::filesystem::is_directory(mod_dir, ec) || ec) {
      ...
      if (ensured.insert(target.mod_folder).second && !metadata_file.empty()) {
        ...
        ModMeta::write_game_metadata(mod_dir, metadata_file, target.mod_folder, "1.0",
                                     "");
      }
    }

    if (sync_overwrite_file(overwrite_dir, target.overwrite_rel, mod_dir, mods_subpath,
                            include_mod_id, target.mod_folder)) {
      ++moved;
    }
  }
  return moved;
