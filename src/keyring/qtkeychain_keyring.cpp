...
bool QtKeychainKeyring::available() const {
  return run_on_main(this, [this] {
    return do_available();
  });
}

bool QtKeychainKeyring::has(const std::string &name) const {
  return run_on_main(this, [this, &name] {
    return do_has(name);
  });
}
...
bool QtKeychainKeyring::do_available() const {
  ...
  QObject::connect(&job, &QKeychain::Job::finished, &loop, [&](QKeychain::Job *j) {
    err = j->error();
    loop.quit();
  });
  job.start();
...
std::string QtKeychainKeyring::do_get(const std::string &name) const {
  ...
  QObject::connect(&job, &QKeychain::Job::finished, &loop, [&](QKeychain::Job *j) {
    ...
    if (err == QKeychain::NoError)
      data = static_cast<QKeychain::ReadPasswordJob *>(j)->textData();
    loop.quit();
  });
  job.start();
  loop.exec();
...
bool QtKeychainKeyring::do_set(const std::string &name, const std::string &value) {
  ...
  job.setKey(QString::fromStdString(name));
  job.setTextData(QString::fromUtf8(value.data(), static_cast<int>(value.size())));
  QObject::connect(&job, &QKeychain::Job::finished, &loop, [&](QKeychain::Job *j) {
    err = j->error();
    loop.quit();
  });
