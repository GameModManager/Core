...
SettingsContentWidget::SettingsContentWidget(
    engine::StyleManager *style_manager, const QString &native_style_name,
    const std::filesystem::path &instance_root,
    engine::PluginLoader *plugin_loader, QWidget *parent)
    : QWidget(parent), style_manager_(style_manager),
      native_style_name_(native_style_name), instance_root_(instance_root),
      plugin_loader_(plugin_loader) {
  auto *layout = new QVBoxLayout(this);
  tabs_ = new QTabWidget(this);
  layout->addWidget(tabs_, 1); // tab pages fill the whole widget height

  tabs_->addTab(build_general_tab(), tr("General"));
  tabs_->addTab(build_theme_tab(), tr("Theme"));
  tabs_->addTab(build_modlist_tab(), tr("Mod List"));
  tabs_->addTab(build_paths_tab(), tr("Paths"));
  tabs_->addTab(build_sources_tab(), tr("Sources"));
  tabs_->addTab(build_plugins_tab(), tr("Plugins"));
  tabs_->addTab(build_workarounds_tab(), tr("Workarounds"));
  tabs_->addTab(build_diagnostics_tab(), tr("Diagnostics"));
}
...
QWidget *SettingsContentWidget::build_general_tab() {
  ...
  QDir i18n_dir(":/i18n");
  for (const auto &info :
       i18n_dir.entryInfoList({"*.qm"}, QDir::Files | QDir::NoDotAndDotDot)) {
  ...
  auto *update_box =
      new QCheckBox(tr("Check for updates on startup"), gen_group);
  ...
  smooth_box->setToolTip(
      tr("Animates wheel scrolling in mod/executable lists."));
  auto *dl_notify_box =
      new QCheckBox(tr("Show download notifications"), gen_group);
  ...
  connect(update_box, &QCheckBox::toggled, this,
          [&s](bool on) { s.set_check_for_updates(on); });
  connect(prerelease_box, &QCheckBox::toggled, this,
          [&s](bool on) { s.set_use_prereleases(on); });
  connect(smooth_box, &QCheckBox::toggled, this,
          [&s](bool on) { s.set_smooth_scrolling(on); });
  connect(dl_notify_box, &QCheckBox::toggled, this,
          [&s](bool on) { s.set_show_download_notifications(on); });
  ...
  auto *saves_box =
      new QCheckBox(tr("Store savegames per profile"), profile_group);
  saves_box->setChecked(s.local_saves());
  auto *inis_box =
      new QCheckBox(tr("Store INI files per profile"), profile_group);
  inis_box->setChecked(s.local_inis());
  auto *archinv_box =
      new QCheckBox(tr("Automatic archive invalidation"), profile_group);
  ...
  connect(saves_box, &QCheckBox::toggled, this,
          [&s](bool on) { s.set_local_saves(on); });
  connect(inis_box, &QCheckBox::toggled, this,
          [&s](bool on) { s.set_local_inis(on); });
  connect(archinv_box, &QCheckBox::toggled, this,
          [&s](bool on) { s.set_archive_invalidation(on); });
  ...
  auto *reset_btn =
      new QPushButton(tr("Reset dialog sizes and positions"), geom_group);
  geom_layout->addWidget(center_box);
  geom_layout->addWidget(reset_btn);
  layout->addWidget(geom_group);

  connect(center_box, &QCheckBox::toggled, this,
          [&s](bool on) { s.set_center_dialogs(on); });
...
QWidget *SettingsContentWidget::build_theme_tab() {
  auto *page = new QWidget(this);
  auto *layout = new QVBoxLayout(page);
  ...
  if (style_manager_) {
    ...
    if (!theme_names.empty()) {
      ...
      for (const auto &name : theme_names)
        theme_combo->addItem(QString::fromStdString(name),
  ...
  theme_combo->setToolTip(
      tr("Editing a theme's .qss or tokens.json on disk live-reloads it. "
         "Qt styles (Fusion, Windows, ...) are the built-in Qt look - no "
         "custom theme files."));
  ...
  connect(pack_combo, &QComboBox::currentIndexChanged, this,
          [&s, pack_combo](int index) {
            ...
            engine::Logger::instance().info("Icon pack: " + data.toStdString());
          });

  connect(theme_combo, &QComboBox::currentIndexChanged, this,
          [this, &s, theme_combo](int index) {
            ...
            // GMM theme (or Default): restore the native platform style first
            // so the QSS renders on Breeze/etc., then apply the theme.
            s.set_theme(data);
            s.clear_style();
  ...
  auto make_swatch = [this](QColor initial,
    ...
    auto apply_swatch = [btn](const QColor &c) {
      btn->setStyleSheet(QString("background-color: rgba(%1,%2,%3,%4);")
                             .arg(c.red())
                             .arg(c.green())
                             .arg(c.blue())
                             .arg(c.alpha()));
  ...
  auto *compact_box =
      new QCheckBox(tr("Compact rows in the Downloads tab"), design_group);
  ...
  auto *center_sep_box =
      new QCheckBox(tr("Center text on separators"), design_group);
...
QWidget *SettingsContentWidget::build_modlist_tab() {
  auto &s = Settings::instance();
  auto *page = new QWidget(this);
  auto *layout = new QVBoxLayout(page);
  ...
  auto *hover_box =
      new QCheckBox(tr("Auto-collapse separators on hover"), page);
  ...
  auto *check_update_box =
      new QCheckBox(tr("Check for updates after install"), page);
  ...
  connect(foreign_box, &QCheckBox::toggled, this,
          [&s](bool on) { s.set_display_foreign(on); });
  connect(save_filters_box, &QCheckBox::toggled, this,
          [&s](bool on) { s.set_save_filters(on); });
  connect(hover_box, &QCheckBox::toggled, this,
          [&s](bool on) { s.set_auto_collapse_on_hover(on); });
  connect(sep_scrollbar_box, &QCheckBox::toggled, this,
          [&s](bool on) { s.set_color_separator_scrollbar(on); });
  connect(check_update_box, &QCheckBox::toggled, this,
          [&s](bool on) { s.set_check_update_after_install(on); });

  // Collapsible Separators box (mirrors MO2 settingsdialog.ui
  // collapsibleSeparatorsWidget)
  ...
  auto *conflicts_label =
      new QLabel(tr("Show conflicts and plugins"), sep_group);
  ...
  auto *per_profile_box =
      new QCheckBox(tr("Collapsible separators per profile"), page);
  ...
  const QString inst_key =
      instance_root_.empty()
          ? QString()
          : QString::fromStdString(instance_root_.filename().string());
  auto *nested_box =
      new QCheckBox(tr("Nested mod list ⚠️ (Experimental)"), page);
  ...
  connect(asc_box, &QCheckBox::toggled, this,
          [&s](bool on) { s.set_collapsible_separators_asc(on); });
  connect(dsc_box, &QCheckBox::toggled, this,
          [&s](bool on) { s.set_collapsible_separators_dsc(on); });
  connect(highlight_to_box, &QCheckBox::toggled, this,
          [&s](bool on) { s.set_collapsible_separators_highlight_to(on); });
  connect(highlight_from_box, &QCheckBox::toggled, this,
          [&s](bool on) { s.set_collapsible_separators_highlight_from(on); });
  connect(icons_conflicts, &QCheckBox::toggled, this,
          [&s](bool on) { s.set_collapsible_separators_icons_conflicts(on); });
  connect(icons_flags, &QCheckBox::toggled, this,
          [&s](bool on) { s.set_collapsible_separators_icons_flags(on); });
  connect(icons_content, &QCheckBox::toggled, this,
          [&s](bool on) { s.set_collapsible_separators_icons_content(on); });
  connect(icons_version, &QCheckBox::toggled, this,
          [&s](bool on) { s.set_collapsible_separators_icons_version(on); });
  connect(per_profile_box, &QCheckBox::toggled, this,
          [&s](bool on) { s.set_collapsible_separators_per_profile(on); });
  connect(nested_box, &QCheckBox::toggled, this, [&s, inst_key](bool on) {
    if (!inst_key.isEmpty())
      s.set_modlist_nested(inst_key, on);
  });
...
QWidget *SettingsContentWidget::build_paths_tab() {
  ...
  auto *inst_hint = new QLabel(tr("Where new instances are created. Leave "
  ...
  if (!instance_root_.empty()) {
    ...
    const FolderRow folders[] = {
        ...
        {engine::InstanceKind::Downloads, tr("Downloads"),
         "$BASE_DIRECTORY/downloads"},
        {engine::InstanceKind::Cache, tr("Cache"), "$BASE_DIRECTORY/cache"},
        {engine::InstanceKind::Profiles, tr("Profiles"),
         "$BASE_DIRECTORY/profiles"},
        {engine::InstanceKind::Overwrite, tr("Overwrite"),
         "$BASE_DIRECTORY/overwrite"},
    };
    ...
    connect(base_browse, &QPushButton::clicked, this,
            [base_edit, commit_base]() {
              const QString dir = QFileDialog::getExistingDirectory(
                  base_edit, QObject::tr("Choose base directory"),
                  base_edit->text());
              if (!dir.isEmpty()) {
    ...
    base_form->addRow(tr("Managed Game"), game_row);
    {
      ...
      if (inst.read_toml() && !inst.info().game_dir.empty())
        game_edit->setText(
            QString::fromStdString(inst.info().game_dir.string()));
    }
    ...
    connect(game_browse, &QPushButton::clicked, this,
            [game_edit, commit_game]() {
              const QString dir = QFileDialog::getExistingDirectory(
                  game_edit, QObject::tr("Choose game directory"),
                  game_edit->text());
              if (!dir.isEmpty()) {
...
QWidget *SettingsContentWidget::build_sources_tab() {
  auto *page = new QWidget(this);
  auto *layout = new QVBoxLayout(page);
  ...
  if (providers.empty()) {
    layout->addWidget(
        new QLabel(tr("No download sources are available."), page));
  } else {
    ...
    for (auto *provider : providers) {
      ...
      if (content == nullptr) {
        auto *lbl =
            new QLabel(tr("This source has no configurable settings."), tabs);
...
QWidget *SettingsContentWidget::build_plugins_tab() {
  struct Entry {
    QString name;
    ...
    // are merged into `options` so every plugin setting renders as a
    // key:value row in the info pane's table.
    engine::PluginInfo::SettingTab settings_tab;
  };
  ...
  if (plugin_loader_) {
    for (const auto &p : plugin_loader_->plugins()) {
      ...
      e.abi_version =
          p.abi_version > 0 ? QString::number(p.abi_version) : QString();
      e.enabled_basename = QString::fromStdString(
      ...
      for (const auto &[key, def] : p.settings) {
        if (tab_key(key))
          continue; // avoid duplicates with the tab
        const QString k = QString::fromStdString(key);
        e.options.emplace_back(
            k, Settings::instance().plugin_setting(
                   e.enabled_basename, k, QString::fromStdString(def)));
      }
      for (const auto &st : p.settings_tab.settings) {
        const QString k = QString::fromStdString(st.key);
        e.options.emplace_back(k,
                               Settings::instance().plugin_setting(
                                   e.enabled_basename, k,
                                   QString::fromStdString(st.default_value)));
      }
  ...
  layout->addWidget(splitter, 1); // columns stretch to the widget bottom

  // -- Left: category-grouped plugin list + bottom filter bar --------------
  auto *left = new QWidget(splitter);
  left->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);
  left->setMinimumWidth(kSettingsMinWidth / 2); // half the min window width
  ...
  for (size_t g = 0; g < group_ids.size(); ++g) {
    ...
    group->setToolTip(0,
                      tr("Click the arrow to collapse/expand this category"));
    group_items[g] = group;
  }
  for (size_t i = 0; i < state->entries.size(); ++i) {
    ...
    item->setToolTip(0, e.is_plugin ? tr("Plugin: %1").arg(e.name)
                                    : tr("Source provider: %1").arg(e.name));
  }

  std::function<void(int)> rebuild_info = [this, info_pane, info_layout,
                                           state](int index) {
    if (index < 0 || static_cast<size_t>(index) >= state->entries.size())
      return;
    ...
    content_layout->addWidget(new QLabel(
        e.is_plugin ? tr("Plugin")
                    : tr("Source provider (%1)").arg(e.provider_type),
        state->content));
    ...
    if (e.is_plugin) {
      ...
      auto add_row = [&](const QString &label, const QString &value) {
        auto *lbl = new QLabel(value.isEmpty() ? tr("(not set)") : value,
                               state->content);
        lbl->setWordWrap(true);
    ...
    if (e.is_plugin) {
      ...
      connect(enabled_box, &QCheckBox::toggled, this, [basename](bool on) {
        Settings::instance().set_plugin_enabled(basename, on);
      });
      enabled_box->setToolTip(
          tr("Disabled plugins are not loaded on the next start."));
    } else {
    ...
    if (e.is_plugin && e.options.empty()) {
              ...
              [basename, table](QTableWidgetItem *item) {
                ...
                const QString value =
                    (item->flags() & Qt::ItemIsUserCheckable)
                        ...
                        : item->text();
                Settings::instance().set_plugin_setting(
                    basename, key_item->text(), value);
              });
      ...
      auto *hint =
          new QLabel(tr("Source provider settings live on the Sources tab."),
                     state->content);
...
QWidget *SettingsContentWidget::build_workarounds_tab() {
  auto &s = Settings::instance();
  auto *page = new QWidget(this);
  auto *layout = new QVBoxLayout(page);
  ...
  auto *offline_box =
      new QCheckBox(tr("Offline mode - do not access the internet"), net_group);
  ...
  auto *browser_box =
      new QCheckBox(tr("Use a custom browser for web links"), net_group);
  ...
  connect(offline_box, &QCheckBox::toggled, this,
          [&s](bool on) { s.set_offline_mode(on); });
  connect(proxy_box, &QCheckBox::toggled, this,
          [&s, proxy_host, proxy_port](bool on) {
            s.set_use_proxy(on);
  ...
  connect(proxy_port, &QSpinBox::valueChanged, this,
          [&s](int v) { s.set_proxy_port(v); });
  ...
  auto *skip_suffixes =
      new QLineEdit(s.skip_file_suffixes().join(", "), misc_group);
  ...
  auto *core_box =
      new QCheckBox(tr("Force-enable game core files"), misc_group);
  core_box->setChecked(s.force_enable_core_files());
  auto *archive_box =
      new QCheckBox(tr("Experimental archive parsing"), misc_group);
  ...
  connect(skip_suffixes, &QLineEdit::editingFinished, this,
          [&s, skip_suffixes]() {
            s.set_skip_file_suffixes(
                skip_suffixes->text().split(',', Qt::SkipEmptyParts));
          });
  ...
  connect(exec_blacklist, &QLineEdit::editingFinished, this,
          [&s, exec_blacklist]() {
            s.set_executables_blacklist(exec_blacklist->text().trimmed());
          });
  connect(delay_spin, &QSpinBox::valueChanged, this,
          [&s](int v) { s.set_overlay_capture_delay_ms(v); });
  connect(core_box, &QCheckBox::toggled, this,
          [&s](bool on) { s.set_force_enable_core_files(on); });
  connect(archive_box, &QCheckBox::toggled, this,
          [&s](bool on) { s.set_experimental_archive_parsing(on); });
...
QWidget *SettingsContentWidget::build_diagnostics_tab() {
  auto &s = Settings::instance();
  auto *page = new QWidget(this);
  auto *layout = new QVBoxLayout(page);
  ...
  connect(dumps_spin, &QSpinBox::valueChanged, this,
          [&s](int v) { s.set_max_core_dumps(v); });
