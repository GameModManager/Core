...
SettingsContentWidget::SettingsContentWidget(engine::StyleManager *style_manager,
...
QWidget *SettingsContentWidget::build_general_tab() {
  ...
  auto *multicore_box = new QCheckBox(tr("Enable multi-core processing"), gen_group);
  ...
  connect(smooth_box, &QCheckBox::toggled, this, [&s](bool on) {
    s.set_smooth_scrolling(on);
  });
  ...
  connect(extract_prio_box, &QCheckBox::toggled, this, [&s](bool on) {
    s.set_extraction_low_priority(on);
  });
  ...
  connect(saves_box, &QCheckBox::toggled, this, [&s](bool on) {
    s.set_local_saves(on);
  });
  connect(inis_box, &QCheckBox::toggled, this, [&s](bool on) {
    s.set_local_inis(on);
  });
  connect(archinv_box, &QCheckBox::toggled, this, [&s](bool on) {
    s.set_archive_invalidation(on);
  });
...
QWidget *SettingsContentWidget::build_theme_tab() {
  ...
  auto &s = Settings::instance();
  // Per-instance appearance (Workspace-1065): show the effective value and
  // persist changes to instance.toml while an instance is loaded; globals
  // stay the fallback for instances that never customized anything.
  const bool per_instance = !instance_root_.empty();
  const QString current_style =
      per_instance ? effective_style(instance_root_) : s.style();
  const QString current_theme =
      per_instance ? effective_theme(instance_root_) : s.theme();
  ...
  if (!pack_names.empty()) {
    ...
    for (const auto &name : pack_names)
      pack_combo->addItem(QString::fromStdString(name), QString::fromStdString(name));
  }
  const QString current_pack =
      per_instance ? effective_icon_pack(instance_root_) : s.icon_pack();
  ...
  connect(theme_combo, &QComboBox::currentIndexChanged, this,
          [this, &s, theme_combo](int index) {
            ...
            const bool per_inst = !instance_root_.empty();
            if (data.startsWith("qt:")) {
              ...
              s.clear_theme();
              if (per_inst) {
                set_instance_style(instance_root_, style);
                set_instance_theme(instance_root_, {});
              }
            ...
            s.clear_style();
            if (per_inst) {
              set_instance_theme(instance_root_, data);
              set_instance_style(instance_root_, {});
            }
  ...
  connect(compact_box, &QCheckBox::toggled, this, [&s](bool on) {
    s.set_compact_downloads(on);
  });
  ...
  connect(center_sep_box, &QCheckBox::toggled, this, [&s](bool on) {
    s.set_center_separator_text(on);
  });
...
QWidget *SettingsContentWidget::build_modlist_tab() {
  ...
  connect(foreign_box, &QCheckBox::toggled, this, [&s](bool on) {
    s.set_display_foreign(on);
  });
  connect(save_filters_box, &QCheckBox::toggled, this, [&s](bool on) {
    s.set_save_filters(on);
  });
  connect(hover_box, &QCheckBox::toggled, this, [&s](bool on) {
    s.set_auto_collapse_on_hover(on);
  });
  connect(sep_scrollbar_box, &QCheckBox::toggled, this, [&s](bool on) {
  ...
  connect(asc_box, &QCheckBox::toggled, this, [&s](bool on) {
    s.set_collapsible_separators_asc(on);
  });
  connect(dsc_box, &QCheckBox::toggled, this, [&s](bool on) {
    s.set_collapsible_separators_dsc(on);
  });
  connect(highlight_to_box, &QCheckBox::toggled, this, [&s](bool on) {
    s.set_collapsible_separators_highlight_to(on);
  });
  connect(highlight_from_box, &QCheckBox::toggled, this, [&s](bool on) {
    s.set_collapsible_separators_highlight_from(on);
  });
  connect(icons_conflicts, &QCheckBox::toggled, this, [&s](bool on) {
    s.set_collapsible_separators_icons_conflicts(on);
  });
  connect(icons_flags, &QCheckBox::toggled, this, [&s](bool on) {
    s.set_collapsible_separators_icons_flags(on);
  });
  connect(icons_content, &QCheckBox::toggled, this, [&s](bool on) {
    s.set_collapsible_separators_icons_content(on);
  });
  connect(icons_version, &QCheckBox::toggled, this, [&s](bool on) {
    s.set_collapsible_separators_icons_version(on);
  });
  connect(per_profile_box, &QCheckBox::toggled, this, [&s](bool on) {
    s.set_collapsible_separators_per_profile(on);
  });
...
QWidget *SettingsContentWidget::build_sources_tab() {
  ...
  if (providers.empty()) {
    ...
    for (auto *provider : providers) {
      ...
      if (content == nullptr) {
        auto *lbl = new QLabel(tr("This source has no configurable settings."), tabs);
        lbl->setWordWrap(true);
        content = lbl;
      }
      tabs->addTab(content, QString::fromStdString(provider->display_name()));
      const std::string vendor_key = engine::vendor_icon_key(provider->source_type());
      if (!vendor_key.empty())
...
QWidget *SettingsContentWidget::build_plugins_tab() {
  ...
  if (plugin_loader_) {
    for (const auto &p : plugin_loader_->plugins()) {
      ...
      e.abi_version = p.abi_version > 0 ? QString::number(p.abi_version) : QString();
      e.enabled_basename =
          QString::fromStdString(std::filesystem::path(p.path).filename().string());
      e.enabled = effective_plugin_enabled(instance_root_, e.enabled_basename);
      ...
      for (const auto &[key, def] : p.settings) {
        ...
        e.options.emplace_back(
            k, effective_plugin_setting(instance_root_, e.enabled_basename, k,
  ...
  std::function<void(int)> rebuild_info = [this, info_pane, info_layout,
    ...
    if (e.is_plugin) {
      ...
      auto add_row = [&](const QString &label, const QString &value) {
        auto *lbl =
            new QLabel(value.isEmpty() ? tr("(not set)") : value, state->content);
        lbl->setWordWrap(true);
    ...
    if (e.is_plugin) {
      ...
      const std::filesystem::path root = instance_root_;
      connect(enabled_box, &QCheckBox::toggled, this, [basename, root](bool on) {
        Settings::instance().set_plugin_enabled(basename, on);
        if (!root.empty())
          set_instance_plugin_enabled(root, basename, on);
      });
    ...
    if (e.is_plugin && e.options.empty()) {
      ...
      auto is_bool = [](const QString &v) {
        ...
        return t == "1" || t == "0" || t == "true" || t == "false" || t == "yes" ||
               t == "no" || t == "on" || t == "off";
      };
      ...
      const std::filesystem::path opts_root = instance_root_;
      connect(
          table, &QTableWidget::itemChanged, this,
          [basename, table, opts_root](QTableWidgetItem *item) {
            if (!item || item->column() != 1)
              return;
            auto *key_item = table->item(item->row(), 0);
            if (!key_item)
              return;
            const QString value = (item->flags() & Qt::ItemIsUserCheckable)
                                      ? (item->checkState() == Qt::Checked ? "1" : "0")
                                      : item->text();
            Settings::instance().set_plugin_setting(basename, key_item->text(), value);
            if (!opts_root.empty())
              set_instance_plugin_setting(opts_root, basename, key_item->text(), value);
          });
...
QWidget *SettingsContentWidget::build_workarounds_tab() {
  ...
  connect(offline_box, &QCheckBox::toggled, this, [&s](bool on) {
    s.set_offline_mode(on);
  });
  ...
  connect(proxy_port, &QSpinBox::valueChanged, this, [&s](int v) {
    s.set_proxy_port(v);
  });
  ...
  connect(delay_spin, &QSpinBox::valueChanged, this, [&s](int v) {
    s.set_overlay_capture_delay_ms(v);
  });
  connect(core_box, &QCheckBox::toggled, this, [&s](bool on) {
    s.set_force_enable_core_files(on);
  });
  connect(archive_box, &QCheckBox::toggled, this, [&s](bool on) {
    s.set_experimental_archive_parsing(on);
  });
