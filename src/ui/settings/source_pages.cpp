...
NexusManualKeyDialog::NexusManualKeyDialog(QWidget *parent) : QDialog(parent) {
  ...
  connect(open_browser_btn, &QPushButton::clicked, this, [this] {
    open_browser();
  });
  connect(paste_btn, &QPushButton::clicked, this, [this] {
    paste();
  });
...
NexusPanel::NexusPanel(QWidget *parent) : QWidget(parent) {
  auto &auth = engine::Source::Nexus::Auth::instance();
  auto &s    = Settings::instance();
  ...
  connect_btn_->setToolTip(tr("Disabled: account validation currently crashes the app "
                              "(see implementation.md Log)."));
  ...
  associate_btn_ = new QPushButton(tr("Associate with \"Download with manager\" links"),
                                   options_group);
  ...
  known_col->addWidget(
      new QLabel(tr("Known Servers (updated on download)"), servers_group));
  ...
  preferred_col->addWidget(
      new QLabel(tr("Preferred Servers (Drag & Drop)"), servers_group));
  ...
  connect(manual_btn_, &QPushButton::clicked, this, [this] {
    enter_key_manually();
  });
  connect(disconnect_btn_, &QPushButton::clicked, this, [this] {
    disconnect_from_nexus();
  });
  connect(associate_btn_, &QPushButton::clicked, this, [this] {
    associate_with_nxm();
  });

  connect(endorse_box, &QCheckBox::toggled, [&s](bool on) {
    s.set_endorsement_integration(on);
  });
  connect(track_box, &QCheckBox::toggled, [&s](bool on) {
    s.set_tracked_integration(on);
  });
  connect(counter_box, &QCheckBox::toggled, [&s](bool on) {
    s.set_hide_api_counter(on);
  });
  connect(queue_box, &QCheckBox::toggled, [&s](bool on) {
    s.set_nexus_queue_downloads(on);
  });
...
void NexusPanel::refresh_account_and_stats() {
  ...
  if (rl.hourly_limit > 0)
    nexus_hourly_->setText(
        QString("%1/%2").arg(rl.hourly_remaining).arg(rl.hourly_limit));
  else
...
QListWidgetItem *NexusPanel::make_server_item(const engine::NexusServer &srv) const {
  ...
  if (avg > 0)
    text += QString(" (%1)").arg(format_speed(avg));
...
void NexusPanel::persist_server_preferences() {
  ...
  for (int i = 0; i < preferred_list_->count(); ++i) {
    ...
    if (!name.isEmpty())
      ordered.push_back(name.toStdString());
  }
...
QWidget *build_workshop_page(engine::SteamWorkshopProvider *provider, QWidget *parent) {
  ...
  auto *group = new QGroupBox(QWidget::tr("Steam Web API Rate Limit"), page);
  ...
  QObject::connect(limit_spin, &QSpinBox::valueChanged, [&s, provider](int v) {
...
QWidget *build_loverslab_page(QWidget *parent) {
  ...
  auto *hint =
      new QLabel(QWidget::tr("LoversLab has no public API, so GMM downloads with the "
  ...
  cookie_edit->setPlaceholderText(QWidget::tr("memberID=...; pass_hash=...; ..."));
  if (auth.has_cookie()) {
  ...
  QObject::connect(paste_btn, &QPushButton::clicked, [cookie_edit]() {
    const QString text = QApplication::clipboard()->text();
    if (!text.isEmpty())
      cookie_edit->setText(text);
  });
  QObject::connect(
      save_btn, &QPushButton::clicked, [&auth, cookie_edit, update_status]() {
