...
QIcon GameSelectionWidget::resolve_icon(const GameEntry &entry) {
  ...
  // letter avatar.
  return GameIconCache::instance().icon_for(QString::fromStdString(entry.game_id),
...
std::string prompt_instance_name(QWidget *parent, const QString &game_name) {
  ...
  auto *input = new QLineEdit(
      QString::fromStdString(engine::sanitize_directory_name(game_name.toStdString())),
      &dialog);
  ...
  auto validate = [&](const QString &text) {
    ...
    if (trimmed.isEmpty()) {
    ...
    } else if (trimmed == QLatin1String(".") || trimmed == QLatin1String("..")) {
      ...
      if (!found.isEmpty() || has_control) {
        ...
        problem = QObject::tr("You cannot use these characters: %1").arg(found);
      }
    }
  ...
  QObject::connect(buttons, &QDialogButtonBox::accepted, &dialog, [&]() {
    ...
    if (std::find(existing.begin(), existing.end(), sanitized) != existing.end()) {
      error->setText(QObject::tr("An instance named \"%1\" already exists.")
...
GameSelectionWidget::GameSelectionWidget(QWidget *parent) : QWidget(parent) {
  ...
  connect(&GameIconCache::instance(), &GameIconCache::icon_ready, this,
          &GameSelectionWidget::on_icon_ready);
}
...
void GameSelectionWidget::apply_filter(const QString &text) {
  ...
  for (auto &card : cards_) {
    if (!card)
      continue;
    const bool match =
        needle.isEmpty() || QString::fromStdString(card->entry().display_name)
...
void GameSelectionWidget::set_games(const std::vector<GameEntry> &installed,
  ...
  std::sort(all.begin(), all.end(), [](const GameEntry &a, const GameEntry &b) {
    return QString::fromStdString(a.display_name)
               .compare(QString::fromStdString(b.display_name), Qt::CaseInsensitive) <
           0;
  });
  GameEntry generic;
