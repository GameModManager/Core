...
private:
  void rescale() {
    ...
    setPixmap(QPixmap::fromImage(image_).scaled(size(), Qt::KeepAspectRatio,
...
FomodWizardDialog::FomodWizardDialog(
    const std::shared_ptr<engine::FomodViewModel> &view_model,
    const std::filesystem::path &content_root, QString suggested_name,
    QString previous_choices, bool always_restore_choices, bool show_images,
    QWidget *parent)
    : QDialog(parent), view_model_(view_model), content_root_(content_root),
      suggested_name_(std::move(suggested_name)),
      previous_choices_(std::move(previous_choices)),
      always_restore_choices_(always_restore_choices), show_images_(show_images) {
  setup_ui();
  ...
  if (view_model_->getSteps().empty()) {
    ...
    layout->addWidget(new QLabel(tr("This mod will install all files automatically."),
                                 legacy_widget));
...
void FomodWizardDialog::on_plugin_toggled(bool selected, engine::GroupRef group,
                                          engine::PluginRef plugin) {
...
void FomodWizardDialog::update_buttons() {
  ...
  next_install_button_->setText(view_model_->isLastVisibleStep() ? tr("Install")
                                                                 : tr("Next"));
}
...
void FomodWizardDialog::update_display_for_active_plugin() {
  ...
  if (!plugin) {
    ...
    if (!active_step || active_step->getGroups().empty() ||
        active_step->getGroups().front()->getPlugins().empty()) {
...
void FomodWizardDialog::show_image_viewer() {
  ...
  auto *viewer = new FomodImageViewer(
      QImage(QString::fromStdString(gf->absolute().string())), this);
...
QWidget *FomodWizardDialog::create_top_row() {
  ...
  values_column->addWidget(
      new QLabel(QString::fromStdString(info->getAuthor()), top_row));
  values_column->addWidget(
      new QLabel(QString::fromStdString(info->getVersion()), top_row));
...
QWidget *FomodWizardDialog::create_bottom_row() {
  ...
  connect(manual_button_, &QPushButton::clicked, this,
          &FomodWizardDialog::on_manual_clicked);
  connect(next_install_button_, &QPushButton::clicked, this,
          &FomodWizardDialog::on_next_clicked);
  connect(back_button_, &QPushButton::clicked, this,
          &FomodWizardDialog::on_back_clicked);
  connect(cancel_button_, &QPushButton::clicked, this,
          &FomodWizardDialog::on_cancel_clicked);
  connect(select_previous_button_, &QPushButton::clicked, this,
          &FomodWizardDialog::select_previously_selected_options);
  connect(reset_choices_button_, &QPushButton::clicked, this,
          &FomodWizardDialog::on_reset_choices_clicked);
...
QWidget *FomodWizardDialog::create_step_widget(
...
QWidget *
FomodWizardDialog::render_group(engine::GroupRef group,
                                const std::shared_ptr<engine::StepViewModel> &step) {
...
QString FomodWizardDialog::object_name_for(engine::PluginRef plugin,
                                           engine::GroupRef group) {
...
QRadioButton *FomodWizardDialog::create_plugin_radio(
    engine::PluginRef plugin, engine::GroupRef group,
...
QCheckBox *FomodWizardDialog::create_plugin_checkbox(
    engine::PluginRef plugin, engine::GroupRef group,
    const std::shared_ptr<engine::StepViewModel> &step, QWidget *parent) {
  ...
  connect(checkbox, &QCheckBox::toggled, this, [this, group, plugin](bool checked) {
    on_plugin_toggled(checked, group, plugin);
  });
...
void FomodWizardDialog::render_select_exactly_one(
    QWidget *parent, QLayout *parent_layout, engine::GroupRef group,
    const std::shared_ptr<engine::StepViewModel> &step) {
...
void FomodWizardDialog::render_checkbox_group(
    QWidget *parent, QLayout *parent_layout, engine::GroupRef group,
    const std::shared_ptr<engine::StepViewModel> &step) {
  for (const auto &plugin : group->getPlugins()) {
...
QButtonGroup *FomodWizardDialog::render_radio_group(
    QWidget *parent, QLayout *parent_layout, engine::GroupRef group,
    const std::shared_ptr<engine::StepViewModel> &step) {
...
void FomodWizardDialog::update_could_be_usable_indicator(QAbstractButton *button,
                                                         engine::PluginRef plugin) {
...
void FomodWizardDialog::apply_fn_from_json(
    const std::string &plugin_selector,
  ...
  for (int step_index = 0; step_index < static_cast<int>(json_steps.size());
       ++step_index) {
    const auto &step = json_steps[step_index];
    if (!step.contains("groups") || !step["groups"].is_array()) {
      continue;
    }
    for (int group_index = 0; group_index < static_cast<int>(step["groups"].size());
         ++group_index) {
      ...
      for (const auto &plugin : group[plugin_selector]) {
        plugins.emplace_back(
            "[" + std::to_string(step_index) + ":" + std::to_string(group_index) +
...
void FomodWizardDialog::style_previously_selected_options() {
  ...
  const auto stylesheet =
      QString("QCheckBox { background-color: rgba(152, 151, 26, 0.4); } "
              "QRadioButton { background-color: rgba(152, 151, 26, 0.4); }");
  const auto tooltip =
      tr("You previously selected this plugin when installing this mod.");
...
void FomodWizardDialog::style_previously_deselected_options() {
  const auto stylesheet =
      QString("QCheckBox { border: 1px dashed rgb(152, 151, 26); } "
              "QRadioButton { border: 1px dashed rgb(152, 151, 26); }");
  const auto tooltip =
      tr("You previously unchecked this plugin when installing this mod.");
...
void FomodWizardDialog::select_previously_selected_options() {
  ...
  try {
    ...
    engine::Logger::instance().warn(
        std::string("FomodWizardDialog: failed to restore previous choices: ") +
        e.what());
    return;
  }
  update_checkbox_states();
...
void FomodWizardDialog::on_reset_choices_clicked() {
  try {
    ...
    engine::Logger::instance().warn(
        std::string("FomodWizardDialog: failed to reset choices: ") + e.what());
    return;
  }
...
namespace {
  ...
  engine::FomodDecision
  ask_fomod_impl(const std::shared_ptr<engine::FomodViewModel> &view_model,
    ...
    FomodWizardDialog dialog(view_model, content_root,
                             QString::fromStdString(suggested_name),
                             QString::fromStdString(previous_choices),
...
engine::FomodDecision
ask_fomod(const std::shared_ptr<engine::FomodViewModel> &view_model,
          const std::filesystem::path &content_root, const std::string &suggested_name,
          const std::string &previous_choices, bool always_restore_choices,
          bool show_images, QWidget *parent) {
  ...
  QMetaObject::invokeMethod(
      ...
      [&] {
        result =
            ask_fomod_impl(view_model, content_root, suggested_name, previous_choices,
                           always_restore_choices, show_images, parent);
      },
