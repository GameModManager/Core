...
void LaunchController::add_toolbar_shortcut_from_path(
    const QString &full_path, const QString &icon_path) {
  if (w_->toolbar_shortcut_paths_.contains(full_path))
    return;
  ...
  if (icon.isNull())
    icon = ui::extractExeIcon(full_path, w_->cache_thumbnails_dir_path());

  // Derive tooltip from game name (replace underscores with spaces) + exe
  // filename
  ...
  connect(btn, &QToolButton::clicked, this,
          [this, full_path]() { launch_with_executable(full_path); });
  w_->toolbar_shortcut_paths_.append(full_path);
  w_->toolbar_shortcut_icons_.append(icon_path);
...
void LaunchController::add_shortcut_to_desktop() {
  ...
  out << "Name=" << game_name << "\n";
  out << "Exec=" << exec_qstr << "\n";
  out << "Path=" << QString::fromStdString(w_->current_game_dir_.string())
      << "\n";
  {
