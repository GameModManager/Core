...
void LaunchController::launch_game() {
  ...
  if (w_->running_process_pid_ > 0) {
    engine::Logger::instance().debug("Launch skipped - game already running (pid " +
  ...
  std::error_code ce;
  auto canon_game = std::filesystem::weakly_canonical(w_->current_game_dir_, ce);
  auto exec_path  = (ce || canon_game.empty() ? w_->current_game_dir_ : canon_game) /
  ...
  launch_with_executable(QString::fromStdString(exec_path.string()), output_mod_dir,
                         entry.arguments, entry.start_in, entry.environment);
}

std::filesystem::path LaunchController::ensure_output_mod_dir(const QString &mod_name) {
  if (mod_name.isEmpty())
    return {};
...
void LaunchController::launch_with_executable(
    ...
    const QString &arguments, const QString &start_in, const QStringList &environment) {
  ...
  if (w_->launch_prep_pending_) {
    engine::Logger::instance().debug("Launch skipped - deploy already in progress");
    return;
  }
  ...
  if (!effective_output.empty()) {
    const QString folder = QString::fromStdString(effective_output.filename().string());
    bool disabled        = false;
  ...
  if (!w_->launch_deploy_thread_) {
    ...
    connect(w_->launch_deploy_thread_->worker(), &ui::DeployWorker::progress, this,
            &LaunchController::on_deploy_progress);
    connect(w_->launch_deploy_thread_->worker(), &ui::DeployWorker::prepared, this,
            &LaunchController::on_launch_params_prepared);
  }
...
void LaunchController::flush_deferred_disable_queue() {
  ...
  engine::Logger::instance().debug("Flushing " +
                                   std::to_string(w_->deferred_disable_queue_.size()) +
...
void LaunchController::on_launch_params_prepared(engine::LaunchParams lparams) {
  ...
  engine::PluginDb::Database::write_plugins_txt_for_launch(
      ...
      lparams.steam_appid, w_->knowledge_ ? *w_->knowledge_ : engine::GameKnowledge(),
      w_->platform_);
  ...
  if (!w_->process_watch_timer_) {
    ...
    connect(w_->process_watch_timer_, &QTimer::timeout, this, [this]() {
...
void LaunchController::check_running_process() {
  ...
  if (!w_->cgroup_path_.empty()) {
    ...
    if (engine::cgroup_is_empty({w_->cgroup_path_})) {
      ...
      if (w_->running_process_pid_ > 0) {
        int st;
        pid_t r = waitpid(static_cast<pid_t>(w_->running_process_pid_), &st, WNOHANG);
      ...
      engine::Logger::instance().debug("Watchdog: cgroup empty, game fully exited");
      int supervisor_exit =
  ...
  int reap_status;
  pid_t early_reaped =
      waitpid(static_cast<pid_t>(w_->running_process_pid_), &reap_status, WNOHANG);
  if (early_reaped > 0) {
  ...
  engine::Logger::instance().debug("Watchdog: process group " + std::to_string(pgid) +
  ...
  engine::EventBus::instance().dispatch(
      engine::events::kGameFinished,
      engine::json_obj({{"exit_code", WIFEXITED(reap_status)
...
void LaunchController::do_capture_overwrite(
  ...
  if (session_active) {
    ...
    auto relayed       = engine::relay_output_to_mod(
        w_->output_session_scratch_, w_->output_mod_dir_, overwrite_dir, mods_subpath,
        inc_id == "true", w_->output_mod_dir_.filename().string());
    engine::Logger::instance().debug(
...
void LaunchController::add_toolbar_shortcut_from_path(const QString &rel_path,
  ...
  for (const auto &e : w_->right_panel_->exec_controls()->executable_entries()) {
    if (e.path.compare(rel_path, Qt::CaseInsensitive) == 0) {
      entry = e;
      break;
    }
  ...
  if (!entry.title.isEmpty()) {
    ...
    auto game_name = QString::fromStdString(
  ...
  connect(btn, &QToolButton::clicked, this, [this, rel_path]() {
    launch_toolbar_shortcut(rel_path);
  });
...
void LaunchController::add_shortcut_to_desktop() {
  ...
  auto exec_qstr = quote_for_desktop(QString::fromStdString(exec_path.string()));
  ...
  if (!entry.environment.isEmpty()) {
    wrapper_script = write_desktop_wrapper(entry, exec_path, work_dir, base_name);
  ...
  QMessageBox::information(w_, tr("Shortcut"),
                           tr("Desktop shortcut created:\n%1").arg(desktop_file));
}

QString LaunchController::write_desktop_wrapper(const Executables::Entry &entry,
...
void LaunchController::create_game_lock_overlay() {
  ...
  w_->game_lock_label_ = new QLabel(tr("The game is running"), w_->game_lock_overlay_);
  ...
  auto *tree_label = new QLabel(tr("Show process tree"), w_->game_lock_overlay_);
  ...
  QObject::connect(w_->process_tree_checkbox_, &QCheckBox::toggled, copy_tree_btn,
  ...
  QObject::connect(w_->unlock_button_, &QPushButton::clicked, this, [this]() {
    hide_game_lock_overlay();
  });
  ...
  QObject::connect(w_->kill_button_, &QPushButton::clicked, this, [this]() {
    ...
    if (ret != 0) {
      int err = errno;
      if (err == ESRCH) {
        engine::Logger::instance().debug("Kill: process group " + std::to_string(pgid) +
                                         " already empty");
      ...
      engine::Logger::instance().error("Kill: kill(-" + std::to_string(pgid) +
                                       ", SIGTERM) failed: " + std::strerror(err) +
                                       " (" + std::to_string(err) + ")");
...
void LaunchController::show_game_lock_overlay(const QString &binary_name, int64_t pid) {
  w_->locked_pid_       = pid;
  ...
  if (pid > 0 && w_->process_tree_checkbox_ && w_->process_tree_checkbox_->isChecked())
    refresh_process_tree();
}
...
InstanceOptionsParams LaunchController::instance_options_params() const {
  ...
  engine::Instance inst = engine::Instance::from_root(w_->current_instance_root_);
  inst.read_toml();
...
void LaunchController::show_instance_options() {
  ...
  ui::InstanceOptionsDialog dlg(w_->platform_, w_->plugin_loader_, p.game_id,
                                p.game_name, p.game_dir, p.steam_appid, p.instance_root,
                                p.current_runner, p.deploy_strategy, p.deploy_config,
                                w_);
  ...
  dlg.content()->set_flush_deferred_disable_queue([this]() {
    flush_deferred_disable_queue();
  });
  if (dlg.exec() == QDialog::Accepted) {
