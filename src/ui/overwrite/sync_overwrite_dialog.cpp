...
void SyncOverwriteDialog::build_tree() {
  ...
  top->setText(0, QString::fromStdString(
                      ctx_.mods_subpath.empty() ? "<root>" : ctx_.mods_subpath));
  add_items(top, root_);
  top->setExpanded(true);
}

void SyncOverwriteDialog::add_items(QTreeWidgetItem *parent_item, const DirNode &node) {
...
void SyncOverwriteDialog::add_file_row(QTreeWidgetItem *item, size_t file_index) {
  ...
  if (f.game_has_file && !ctx_.game_folder.empty()) {
    combo->addItem(QString::fromStdString(ctx_.game_label.empty() ? ctx_.game_folder
                                                                  : ctx_.game_label),
...
void SyncOverwriteDialog::accept() {
  ...
  for (const auto &row : rows_) {
    ...
    if (folder.isEmpty())
      continue;
    targets_.push_back({files_[row.index].overwrite_rel, folder.toStdString()});
  }
  QDialog::accept();
}
