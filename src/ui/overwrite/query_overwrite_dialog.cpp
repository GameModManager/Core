...
QueryOverwriteDialog::QueryOverwriteDialog(const QString &mod_name, bool default_backup,
                                           QWidget *parent)
  ...
  message->setText(tr(
      "This mod seems to be installed already, what would you like to do?<br><br>"
      ...
      "<b>Rename:</b> Install this as a separate mod with a new name "
      "<i>(recommended)</i>."));
  ...
  connect(merge_btn, &QPushButton::clicked, this, [this] {
    set_action(engine::OverwriteAction::Merge);
  });
  connect(replace_btn, &QPushButton::clicked, this, [this] {
    set_action(engine::OverwriteAction::Replace);
  });
  connect(rename_btn, &QPushButton::clicked, this, [this] {
    set_action(engine::OverwriteAction::Rename);
  });
...
engine::OverwriteAction QueryOverwriteDialog::action() const {
  return action_;
}

bool QueryOverwriteDialog::backup() const {
  return backup_box_->isChecked();
}
...
namespace {
  ...
  engine::OverwriteDecision ask_overwrite_impl(const QString &mod_name,
    ...
    if (decision.action == engine::OverwriteAction::Rename) {
      bool ok = false;
      QString new_name =
          QInputDialog::getText(parent, QObject::tr("Mod Name"), QObject::tr("Name"),
