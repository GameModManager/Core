...
OverwriteInfoDialog::OverwriteInfoDialog(const std::filesystem::path &overwrite_dir,
  ...
  connect(view_, &QTreeView::customContextMenuRequested, this,
          &OverwriteInfoDialog::show_context_menu);
...
bool OverwriteInfoDialog::is_mapping_root(const QModelIndex &index) const {
  ...
  const auto path = model_->filePath(index);
  std::error_code ec;
  if (!std::filesystem::is_directory(path.toStdString(), ec) || ec)
    return false;
  if (path.toStdString() == overwrite_dir_.string())
    return true;
...
void OverwriteInfoDialog::show_context_menu(const QPoint &pos) {
  QMenu menu(this);
  auto *open_act       = menu.addAction(tr("&Open"), this, [this]() {
    open_selected();
  });
  auto *rename_act     = menu.addAction(tr("&Rename"), this, [this]() {
    rename_selected();
  });
  auto *new_folder_act = menu.addAction(tr("&New Folder"), this, [this]() {
    new_folder();
  });
  menu.addSeparator();
  auto *delete_act = menu.addAction(tr("&Delete"), this, [this]() {
    delete_selected();
  });
  ...
  for (const auto &index : to_delete) {
    ...
    if (!engine::remove_path(path)) {
      QMessageBox::warning(this, tr("Overwrite"),
                           tr("Failed to delete \"%1\".").arg(model_->fileName(index)));
    }
  }
}
...
void OverwriteInfoDialog::rename_selected() {
  auto sel = selected_indexes();
  if (sel.size() != 1 || model_->isDir(sel[0]))
    return;

  bool ok = false;
  const auto new_name =
      QInputDialog::getText(this, tr("Rename"), tr("New name:"), QLineEdit::Normal,
