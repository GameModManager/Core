...
ProtonPanel::ProtonPanel(engine::PlatformInterface* platform,
    ...
    : QDialog(parent) {
    setWindowTitle(
        tr("Proton Options — %1").arg(QString::fromStdString(game_display_name)));

    auto* layout = new QVBoxLayout(this);
    content_ = new ProtonContentWidget(platform, plugin_loader, game_id,
                                       game_display_name, game_dir, steam_appid,
                                       instance_root, current_runner,
                                       current_deploy_strategy, deploy_config,
                                       this);
    ...
    connect(content_, &ProtonContentWidget::save_requested, this,
            &QDialog::accept);
    connect(content_, &ProtonContentWidget::cancel_requested, this,
            &QDialog::reject);
}

ProtonPanel::~ProtonPanel() = default;
