...
VideoViewer::VideoViewer(QWidget *parent) : FileViewerWidget(parent) {
  ...
  connect(position_slider_, &QSlider::sliderPressed, this, [this]() {
    user_seeking_ = true;
  });
  ...
  connect(player_, &QMediaPlayer::positionChanged, this, &VideoViewer::on_position);
  connect(player_, &QMediaPlayer::durationChanged, this, &VideoViewer::on_duration);
}
...
void VideoViewer::on_position(qint64 position) {
  ...
  time_label_->setText(
      tr("%1 / %2").arg(format_time(position), format_time(duration_)));
}
