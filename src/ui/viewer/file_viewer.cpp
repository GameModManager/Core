...
bool FileViewer::open(const QString &path) {
  ...
  switch (kind) {
  case Kind::Image:
    target = image_viewer_;
    break;
  case Kind::Video:
    target = video_viewer_;
    break;
  case Kind::Scene:
    target = scene_viewer_;
    break;
  default:
    break;
  }
...
void FileViewer::clear() {
  for (auto *widget :
       {static_cast<QWidget *>(image_viewer_), static_cast<QWidget *>(video_viewer_),
...
FileViewerWidget *FileViewer::current_viewer() const {
  if (current_kind_ == Kind::Image)
    return image_viewer_;
  if (current_kind_ == Kind::Video)
    return video_viewer_;
  if (current_kind_ == Kind::Scene)
    return scene_viewer_;
  return nullptr;
}

FileViewer::Kind FileViewer::kind_for(const QString &path) {
  const QString suffix = QFileInfo(path).suffix().toLower();
  if (in_list(suffix, image_suffixes()))
    return Kind::Image;
  if (in_list(suffix, video_suffixes()))
    return Kind::Video;
  if (in_list(suffix, scene_suffixes()))
    return Kind::Scene;
  return Kind::Unknown;
}
...
void FileViewer::show_unsupported(const QString &path) {
  unsupported_label_->setText(tr("No viewer for this file type.\n\n%1").arg(path));
