...
MultiProcess::MultiProcess(QObject* parent)
  ...
  // mtime check; dead-PID detection still applies). Crashed instances
  // are reclaimed via the PID check instead of waiting out the
  // 30 s default.
...
bool MultiProcess::tryAcquire(int staleLockTimeoutMs) {
  ...
  // Non-blocking probe when the timeout is 0: QLockFile still performs
  // stale-lock detection (dead PID) on the single attempt and returns
  // immediately when a live holder exists. A nonzero timeout waits up to
  // that long for the lock - never use that on the startup path.
