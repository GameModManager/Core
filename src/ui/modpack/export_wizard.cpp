...
QString ExportWizard::step_title(Step step)
{
  ...
  case Step::Mods:
    return tr("Mods");
  case Step::Executables:
    return tr("Executables");
  case Step::Tree:
    return tr("Tree");
  case Step::Review:
    return tr("Review");
...
QWidget* ExportWizard::build_mods_page()
{
  ...
  mods_table_->horizontalHeader()->setSectionResizeMode(1, QHeaderView::Stretch);
  ...
  for (int row = 0; row < static_cast<int>(mods_.size()); ++row) {
    ...
    if (mod.is_vanilla) {
      ...
      name_item->setToolTip(tr("Vanilla game master - cannot be exported."));
    } else if (mod.is_manual) {
      name_item->setToolTip(tr("Manual/unknown source. Checking this will bundle the "
                               "mod folder into the archive as-is."));
    } else if (mod.disabled) {
...
QWidget* ExportWizard::build_executables_page()
{
  ...
  exes_table_->horizontalHeader()->setSectionResizeMode(1, QHeaderView::Stretch);
...
QWidget* ExportWizard::build_tree_page()
{
  ...
  tree_->setHeaderLabel(tr("Archive contents"));
...
void ExportWizard::refresh_tree()
{
  ...
  if (output_edit_ != nullptr) {
    const QString out = output_edit_->text().trimmed();
    if (!out.isEmpty())
      archive_name = QString::fromStdString(
  ...
  const QString manifest_summary =
      tr("%1 by %2 - game %3")
          .arg(QString::fromStdString(manifest.info.name))
          .arg(QString::fromStdString(manifest.info.author).isEmpty()
                   ? tr("Unknown")
  ...
  for (const auto& exe : exe_entries) {
    ...
    if (it != exe_titles.end())
      title = QString::fromStdString(it->second);
    if (title.isEmpty())
      title = QString::fromStdString(exe.relative_path);
