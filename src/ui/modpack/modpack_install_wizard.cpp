...
namespace {
    ...
    return std::visit(
        [](const auto &src) -> QString {
          ...
          if constexpr (std::is_same_v<T, engine::gmmpack::ModSourceNexus>) {
            ...
            if (src.file_name)
              s += QStringLiteral(" (") + QString::fromStdString(*src.file_name) +
                   QStringLiteral(")");
            return s;
  ...
  QString format_size(std::optional<int64_t> bytes) {
    if (!bytes)
      return ModpackInstallWizard::tr("unknown size");
  ...
  QString category_name(ModCategory category) {
    ...
    case ModCategory::Required:
      return ModpackInstallWizard::tr("Required");
  ...
  QString status_icon(const QString &status) {
    if (status == QStringLiteral("downloaded"))
  ...
  Collection::SourceResolution declared_resolution(const std::string &raw) {
    if (raw == "browser")
      return Collection::SourceResolution::Browser;
  ...
  QString nexus_file_page(const engine::gmmpack::ModSourceNexus &src) {
    QString url = QStringLiteral("https://www.nexusmods.com/") +
                  QString::fromStdString(src.game_domain) + QStringLiteral("/mods/") +
                  QString::number(src.mod_id) + QStringLiteral("?tab=files");
    return url;
  }
        ...
        [&](const auto &src) {
          ...
          if constexpr (std::is_same_v<T, engine::gmmpack::ModSourceNexus>) {
            const Collection::RouteOutcome outcome = Collection::route_download(
                ...
                Collection::capabilities_for(Collection::SourceNexus::kProvider));
            ...
            const Collection::RouteOutcome outcome = Collection::route_download(
                declared_resolution(src.resolution), Collection::AccountStatus{},
                Collection::capabilities_for(Collection::SourceDirect::kProvider));
            route.path   = outcome.path;
            route.reason = QString::fromStdString(outcome.reason);
            if (route.path == Collection::DownloadPath::Browser)
            ...
            const Collection::RouteOutcome outcome = Collection::route_download(
                declared_resolution(src.resolution), Collection::AccountStatus{},
                Collection::capabilities_for(Collection::SourceModPub::kProvider));
            route.path   = outcome.path;
            route.reason = QString::fromStdString(outcome.reason);
          } else if constexpr (std::is_same_v<T, engine::gmmpack::ModSourceLoversLab>) {
            // LoversLab declares browser-only resolution (no public API).
            const Collection::RouteOutcome outcome = Collection::route_download(
                Collection::SourceResolution::Browser, Collection::AccountStatus{},
                Collection::capabilities_for(Collection::SourceLoversLab::kProvider));
            route.path   = outcome.path;
            route.reason = QString::fromStdString(outcome.reason);
            ...
            std::visit(
                [&](const auto &id) {
                  ...
                  if constexpr (std::is_same_v<I, int64_t>) {
                    ...
                    if (numeric)
                      num = QString::fromStdString(id);
                  }
                  if (!num.isEmpty()) {
                    route.open_url =
                        QStringLiteral("https://www.loverslab.com/files/file/") + num +
                        QStringLiteral("/");
                  }
                },
                src.mod_id);
  ...
  void ensure_download_providers() {
    ...
    if (registry.provider_for("nexus") == nullptr)
      registry.register_provider(std::make_unique<engine::Source::Nexus::Provider>());
    ...
    if (registry.provider_for("direct") == nullptr)
      registry.register_provider(std::make_unique<engine::Source::Modl::Provider>());
    if (registry.provider_for("modpub") == nullptr)
      registry.register_provider(std::make_unique<engine::Source::ModPub::Provider>());
  }
  ...
  public:
    ...
    using DoneFn = std::function<void(bool, const std::string &, const std::string &)>;

    FetchThread(QString id, engine::Mod mod, std::filesystem::path dest_dir,
                std::atomic_bool *cancel, ProgressFn on_progress, MetaFn on_meta,
                DoneFn on_done, QObject *parent = nullptr)
        : QThread(parent), id_(std::move(id)), mod_(std::move(mod)),
          dest_dir_(std::move(dest_dir)), cancel_(cancel),
          on_progress_(std::move(on_progress)), on_meta_(std::move(on_meta)),
          on_done_(std::move(on_done)) {}

    void run() override {
      auto *provider =
          engine::Source::Registry::instance().provider_for(mod_.download_source_type);
      if (provider == nullptr) {
        on_done_(false, {},
                 "no download provider for source type '" + mod_.download_source_type +
                     "'");
        return;
      }
      ...
      if (!info.display_name.empty())
        mod_.name = info.display_name;
      mod_.archive_filename = filename;

      std::error_code ec;
      std::filesystem::create_directories(dest_dir_, ec);
      if (ec) {
        on_done_(false, {}, "cannot create downloads dir: " + ec.message());
        return;
      }
      ...
      ctx.on_progress = [this](int64_t downloaded, int64_t total, double /*speed*/) {
...
ModpackInstallWizard::ModpackInstallWizard(engine::gmmpack::Gmmpack pack, Mode mode,
                                           QWidget *parent)
    : QDialog(parent), pack_(std::move(pack)), mode_(mode) {
  const QString pack_name = QString::fromStdString(pack_.manifest.info.name);
  setWindowTitle(pack_name.isEmpty() ? tr("Install Modpack")
  ...
  for (const auto &group : pack_.manifest.choice_groups) {
    ...
    if (group.mode == "exactly-one" && !group.member_mod_ids.empty()) {
      choice_selections_[gid] =
          QStringList(QString::fromStdString(group.member_mod_ids.front()));
    } else {
  ...
  for (const auto &mod : pack_.mods) {
    download_status_[QString::fromStdString(mod.id)] = QStringLiteral("pending");
  }
  rebuild_patch_plan();
  for (const auto &exe : pack_.executables) {
    if (exe.role == "setup" && exe.auto_run) {
      tool_status_[QString::fromStdString(exe.id)] = QStringLiteral("pending");
    }
  }
  ...
  connect(back_button_, &QPushButton::clicked, this, &ModpackInstallWizard::on_back);
  connect(next_button_, &QPushButton::clicked, this, &ModpackInstallWizard::on_next);
  connect(cancel_button, &QPushButton::clicked, this, &ModpackInstallWizard::on_cancel);
...
void ModpackInstallWizard::rebuild_patch_plan() {
  auto [chains, diagnostics] = engine::gmmpack::build_patch_chains(pack_.patches);
  ...
  for (const auto &chain : patch_chains_) {
    patch_allowed_[QString::fromStdString(chain.mod_id)] = false;  // Deny by default.
  }
}
...
void ModpackInstallWizard::start_fetch(const QString &mod_id) {
  ...
  if (dest_dir.isEmpty()) {
    ...
    if (base.isEmpty())
      base = instance_root();
    dest_dir =
        base.isEmpty() ? QStringLiteral(".") : base + QStringLiteral("/../downloads");
  }
  ...
  auto *thread  = new FetchThread(mod_id, *engine_mod,
                                  std::filesystem::path(dest_dir.toStdString()),
                                  &fetch_cancel_, std::move(on_progress),
                                  std::move(on_meta), std::move(on_done), this);
  fetch_thread_ = thread;
  connect(thread, &QThread::finished, this, [this, thread]() {
    if (fetch_thread_ != thread)
      return;  // torn down in the dtor
    fetch_thread_ = nullptr;
    thread->deleteLater();
    pump_download_queue();
  });
...
void ModpackInstallWizard::build_steps() {
  const std::vector<Step> order = {
      Step::Intro,    Step::Paths,   Step::Choices,   Step::IniTweaks, Step::Downloads,
      Step::RunTools, Step::Patches, Step::Finishing, Step::End,
  };
  for (Step id : order) {
    StepState state;
    state.id    = id;
...
void ModpackInstallWizard::on_next() {
  ...
  if (steps_[current_].id == Step::Choices && !choices_complete()) {
    QMessageBox::information(this, tr("Choices"),
...
void ModpackInstallWizard::on_cancel() {
  if (current_ > 0) {
    const auto answer =
        QMessageBox::question(this, tr("Install Modpack"),
...
void ModpackInstallWizard::on_step_clicked(QListWidgetItem *item) {
  ...
  for (int i = 0; i < static_cast<int>(steps_.size()); ++i) {
    if (steps_[i].visited && !steps_[i].skipped)
      furthest = i;
  }
  if (index <= furthest && !steps_[index].skipped)
    go_to(index);
  refresh_chrome();
}
...
bool ModpackInstallWizard::choices_complete() const {
  for (const auto &group : pack_.manifest.choice_groups) {
    if (group.mode != "exactly-one")
      continue;
    const QString gid = QString::fromStdString(group.id);
    if (choice_selections_.value(gid).isEmpty())
      return false;
  }
  return true;
}
...
bool ModpackInstallWizard::downloads_complete() const {
  for (const auto &mod : pack_.mods) {
    if (mod.category != ModCategory::Required)
      continue;
    const QString status = download_status_.value(QString::fromStdString(mod.id));
    if (status != QStringLiteral("downloaded"))
      return false;
  }
  return true;
...
QWidget *ModpackInstallWizard::build_intro_page() {
  ...
  auto *name = new QLabel(
      QString::fromStdString(info.name.empty() ? pack_.manifest.id : info.name), page);
  ...
  if (!info.author.empty()) {
    layout->addWidget(
        new QLabel(tr("by %1").arg(QString::fromStdString(info.author)), page));
  }
  ...
  if (!info.homepage.empty()) {
    auto *link =
        new QLabel(QStringLiteral("<a href=\"%1\">%1</a>")
  ...
  if (!info.created_at.empty())
    dates << tr("Created: %1").arg(QString::fromStdString(info.created_at));
  if (!info.updated_at.empty())
    dates << tr("Updated: %1").arg(QString::fromStdString(info.updated_at));
  if (!dates.isEmpty()) {
    auto *dates_label = new QLabel(dates.join(QStringLiteral("  |  ")), page);
    dates_label->setWordWrap(true);
    layout->addWidget(dates_label);
  }

  layout->addWidget(new QLabel(
      tr("%1 mods, revision %2").arg(pack_.mods.size()).arg(pack_.manifest.revision),
      page));

  if (!pack_.manifest.tools.empty()) {
    auto *tools_box    = new QGroupBox(tr("Required tools"), page);
    auto *tools_layout = new QVBoxLayout(tools_box);
    for (const auto &tool : pack_.manifest.tools) {
      QString entry = QString::fromStdString(tool.name.empty() ? tool.id : tool.name);
      if (!tool.homepage.empty()) {
        entry += QStringLiteral(" (<a href=\"%1\">%1</a>)")
                     .arg(QString::fromStdString(tool.homepage).toHtmlEscaped());
      }
...
QWidget *ModpackInstallWizard::build_paths_page() {
  ...
  auto *layout = new QVBoxLayout(page);
  layout->addWidget(
      new QLabel(tr("Choose where the new instance and its directories live."), page));
  ...
  for (const auto &group : pack_.manifest.choice_groups) {
    ...
    auto *box         = new QGroupBox(
        QString::fromStdString(group.name.empty() ? group.id : group.name) +
            (group.mode == "exactly-one" ? tr(" (pick one)")
                                         : tr(" (pick at most one)")),
        page);
    ...
    for (const auto &member_id : group.member_mod_ids) {
      ...
      const bool selected = choice_selections_.value(gid).contains(mid);
      if (exactly_one) {
...
QWidget *ModpackInstallWizard::build_downloads_page() {
  ...
  downloads_table_->horizontalHeader()->setSectionResizeMode(1, QHeaderView::Stretch);
...
QWidget *ModpackInstallWizard::build_run_tools_page() {
  ...
  tools_table_->horizontalHeader()->setSectionResizeMode(0, QHeaderView::Stretch);
  ...
  for (const auto *exe : setup) {
    ...
    tools_table_->setItem(row, 0,
                          new QTableWidgetItem(QString::fromStdString(exe->id)));
    tools_table_->setItem(row, 1,
                          new QTableWidgetItem(mod_display_name(exe->source_mod_id)));
    tools_table_->setItem(row, 2, new QTableWidgetItem(tool_status_.value(eid)));
    tools_table_->item(row, 0)->setData(Qt::UserRole, eid);
    auto *run = new QPushButton(tr("Run"), tools_table_);
    run->setProperty("exe_id", eid);
    connect(run, &QPushButton::clicked, this, &ModpackInstallWizard::on_run_tool_one);
  ...
  for (const auto *exe : setup) {
    ...
    if (exe->output && exe->output->capture == "syntheticMod") {
      note += tr(" (output captured as mod %1)")
                  .arg(QString::fromStdString(exe->output->synthetic_mod_id));
    }
    notes << note;
  }
...
QWidget *ModpackInstallWizard::build_patches_page() {
  ...
  auto *layout = new QVBoxLayout(page);
  layout->addWidget(new QLabel(
      tr("These patches modify mod files. Allow only patches you trust."), page));
  if (!patch_plan_error_.isEmpty()) {
    auto *warning = new QLabel(
        tr("Patch validation reported errors:\n%1").arg(patch_plan_error_), page);
  ...
  patches_table_->horizontalHeader()->setSectionResizeMode(1, QHeaderView::Stretch);
  ...
  for (const auto &chain : patch_chains_) {
    ...
    for (const auto &patch : chain.patches) {
      const QString target = QString::fromStdString(patch.target_path);
      if (!targets.contains(target))
        targets << target;
    }
    patches_table_->setItem(row, 0,
                            new QTableWidgetItem(mod_display_name(chain.mod_id)));
    patches_table_->setItem(row, 1,
                            new QTableWidgetItem(targets.join(QStringLiteral(", "))));
    patches_table_->setItem(
        row, 2,
        new QTableWidgetItem(
            tr("%n step(s)", nullptr, static_cast<int>(chain.patches.size()))));
    auto *status =
        new QTableWidgetItem(patch_allowed_.value(key) ? tr("Allow") : tr("Deny"));
  ...
  connect(patches_table_, &QTableWidget::cellClicked, this,
          [this](int row, int column) {
            if (column != 3 || patches_table_ == nullptr)
              return;
...
QWidget *ModpackInstallWizard::build_finishing_page() {
  ...
  finishing_table_->horizontalHeader()->setSectionResizeMode(1, QHeaderView::Stretch);
...
QWidget *ModpackInstallWizard::build_end_page() {
  ...
  if (closing.isEmpty()) {
    closing = tr("Thanks for installing %1.")
                  .arg(QString::fromStdString(pack_.manifest.info.name.empty()
                                                  ? pack_.manifest.id
...
void ModpackInstallWizard::refresh_downloads_ui() {
  ...
  for (const QString &mid : ids) {
    ...
    if (mod == nullptr)
      continue;
    const QString status = download_status_.value(mid, QStringLiteral("pending"));
    if (status == QStringLiteral("downloaded"))
      ++done;
    ...
    QString label = QStringLiteral("%1 %2").arg(
        status_icon(status),
        QString::fromStdString(mod->name.empty() ? mod->id : mod->name));
    if (status == QStringLiteral("downloading") && download_fraction_.contains(mid)) {
      const int pct = static_cast<int>(download_fraction_.value(mid) * 100.0);
      label += tr(" (%1%)").arg(pct);
    }
    downloads_table_->setItem(
        row, 0, new QTableWidgetItem(QStringLiteral("phase %1").arg(mod->phase)));
    ...
    if (!route.reason.isEmpty())
      tip += QStringLiteral(" - ") + route.reason;
    if (status == QStringLiteral("failed") && download_error_.contains(mid)) {
      tip += QStringLiteral("\n") + download_error_.value(mid);
    }
    name_item->setToolTip(tip);
    downloads_table_->setItem(row, 1, name_item);
    downloads_table_->setItem(row, 2,
                              new QTableWidgetItem(source_provider(*mod) +
                                                   QStringLiteral(" - ") +
                                                   source_detail(*mod)));
    downloads_table_->setItem(
    ...
    if (status == QStringLiteral("downloading")) {
      cell = new QLabel(status_icon(status) + tr(" active..."), downloads_table_);
      ...
      auto *dl = new QPushButton(
          status == QStringLiteral("failed") ? tr("Retry") : tr("Get"), box);
      dl->setProperty("mod_id", mid);
      connect(dl, &QPushButton::clicked, this, &ModpackInstallWizard::on_download_one);
      ...
      auto *dl =
          new QPushButton(status == QStringLiteral("failed") ? tr("Retry") : tr("Get"),
                          downloads_table_);
      dl->setProperty("mod_id", mid);
      connect(dl, &QPushButton::clicked, this, &ModpackInstallWizard::on_download_one);
      cell = dl;
  ...
  if (!active_download_id_.isEmpty() &&
    ...
    const int pct =
        static_cast<int>(download_fraction_.value(active_download_id_) * 100.0);
  ...
  if (downloads_start_all_ != nullptr) {
    downloads_start_all_->setEnabled(fetch_thread_ == nullptr && !downloads_complete());
  }
  if (downloads_hint_ != nullptr) {
    const Collection::AccountStatus nexus = Collection::Nexus::account_status();
    const QString mode = nexus.can_auto_download
                             ? tr("Nexus API auto-download (premium)")
                             : tr("Nexus via browser (free/anonymous)");
    downloads_hint_->setText(tr("%1. Downloads run in phase order, one at a time. "
                                "Next unlocks once all required mods finish.")
                                 .arg(mode));
  }
  refresh_next_enabled();
}
...
void ModpackInstallWizard::on_browse_path(QLineEdit *edit) {
  const QString dir =
      QFileDialog::getExistingDirectory(this, tr("Choose directory"), edit->text());
  if (!dir.isEmpty())
    edit->setText(dir);
}
...
void ModpackInstallWizard::on_choice_toggled() {
  ...
  if (radio != nullptr) {
    ...
    if (radio->isChecked())
      choice_selections_[gid] = QStringList(mid);
    return;
  }
  ...
  if (check->isChecked()) {
    ...
    for (auto *sibling : boxes) {
      if (sibling != check && sibling->property("group_id").toString() == gid) {
...
void ModpackInstallWizard::on_fetch_progress(const QString &mod_id, int64_t downloaded,
                                             int64_t total) {
  if (total > 0) {
...
void ModpackInstallWizard::on_fetch_meta(const QString &mod_id,
  ...
  if (!display_name.isEmpty()) {
    ...
    for (int row = 0; row < downloads_table_->rowCount(); ++row) {
      ...
      if (item != nullptr && item->data(Qt::UserRole).toString() == mod_id) {
...
void ModpackInstallWizard::on_fetch_done(const QString &mod_id, bool ok,
  ...
  if (ok) {
    ...
    download_error_[mod_id]  = error.isEmpty() ? tr("download failed") : error;
...
void ModpackInstallWizard::on_run_tool_one() {
  ...
  connect(sim_timer_, &QTimer::timeout, this, &ModpackInstallWizard::on_tool_sim_tick);
  sim_timer_->start(150);
}

void ModpackInstallWizard::on_tool_sim_tick() {
  if (++tool_sim_ticks_ < 3)
    return;
  sim_timer_->stop();
  ...
  if (tools_table_ != nullptr) {
    for (int row = 0; row < tools_table_->rowCount(); ++row) {
      ...
      if (id_item != nullptr &&
        ...
        tools_table_->setItem(row, 2, new QTableWidgetItem(QStringLiteral("done")));
...
void ModpackInstallWizard::on_run_tool_skip() {
  ...
  if (tools_table_ != nullptr) {
    for (int row = 0; row < tools_table_->rowCount(); ++row) {
      auto *id_item = tools_table_->item(row, 0);
      if (id_item != nullptr && id_item->data(Qt::UserRole).toString() == eid) {
        tools_table_->setItem(row, 2, new QTableWidgetItem(QStringLiteral("skipped")));
      }
    }
  }
}
...
void ModpackInstallWizard::on_patch_cell_changed(int row, int column) {
  if (patches_table_ == nullptr || column != 3)
    return;
  auto *item = patches_table_->item(row, column);
  if (item == nullptr)
    return;
...
void ModpackInstallWizard::start_finishing_animation() {
  if (finishing_table_ == nullptr || finishing_started_)
    return;
  finishing_started_ = true;
  finishing_tick_    = 0;
  disconnect(sim_timer_, nullptr, nullptr, nullptr);
  connect(sim_timer_, &QTimer::timeout, this, &ModpackInstallWizard::on_finishing_tick);
...
void ModpackInstallWizard::on_finishing_tick() {
  ...
  finishing_table_->setItem(finishing_tick_, 0,
