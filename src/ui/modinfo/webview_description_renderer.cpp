...
namespace {
  ...
  QString wrap_web_html(const QString &body, SourceCSS style) {
    ...
    const QString css = (style == SourceCSS::LoversLab) ? kLoversLabCss : kNexusCss;
    return QStringLiteral(
               "<!DOCTYPE "
               "html><html><head><style>%1</style></head><body>%2</body></html>")
        .arg(css, body);
  }

}  // namespace

// Lazy Chromium init (Workspace-2vmp): touching QWebEngineProfile spins up
// the Chromium engine, so this must not run at startup. The
// NoPersistentCookies policy (MO2 pattern: untrusted remote content must
// not persist cookies) is applied here, once, on first actual WebEngine
// use. Runs after the QApplication exists (QWebEngineProfile needs one).
void ensure_webengine_profile() {
