...
void FiletreeTab::on_rename() {
  ...
  const QString new_name = QInputDialog::getText(
      this, tr("Rename"), tr("New name:"), QLineEdit::Normal, info.fileName(), &ok);
  if (!ok || new_name.trimmed().isEmpty() || new_name.trimmed() == info.fileName()) {
    return;
  }
  ...
  if (!QFile::rename(path, new_path)) {
    QMessageBox::warning(this, tr("Rename"), tr("Failed to rename \"%1\".").arg(path));
  }
}
...
void FiletreeTab::on_delete() {
  ...
  if (QMessageBox::question(
          this, tr("Delete"),
          tr("Move \"%1\" to the trash bin?\n\nIt can be restored from the "
             "system trash.")
              .arg(path),
          QMessageBox::Yes | QMessageBox::No, QMessageBox::No) != QMessageBox::Yes) {
    return;
  }
  // Mod content is user data: never delete permanently. Route through the
  // canonical trash helper (Workspace-f4f7) like every other mod / download
  // / save removal in the app.
  if (!engine::remove_path(path.toStdString())) {
    QMessageBox::warning(this, tr("Delete"),
                         tr("Failed to move \"%1\" to the trash bin.").arg(path));
    return;
  }
  engine::Logger::instance().debug("FiletreeTab: moved to trash: " +
                                   path.toStdString());
}
