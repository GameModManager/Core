...
namespace {
  QString ideal_text_color(const QColor &bg) {
    const qreal luminance = 0.299 * bg.red() + 0.587 * bg.green() + 0.114 * bg.blue();
...
NotesTab::NotesTab(QWidget *parent) : ModInfoTab(parent) {
  ...
  connect(comments_, &QLineEdit::editingFinished, this, &NotesTab::on_comments_edited);
...
void NotesTab::set_mod(const ModInfoData &data) {
  ...
  comments_->setText(QString::fromStdString(meta.get("General", "comments")));
  ...
  set_has_data(!comments_->text().isEmpty() || !notes_->toPlainText().isEmpty() ||
