...
void EspsTab::set_mod(const ModInfoData &data) {
  ...
  if (root.exists()) {
    ...
    while (it.hasNext()) {
      ...
      if (rel.contains(QLatin1Char('/')) || rel.contains(QLatin1Char('\\'))) {
        e.inactive_path = rel;
      } else {
        e.active_path = rel;
      }
  ...
  std::sort(esps_.begin(), esps_.end(), [](const Esp &a, const Esp &b) {
...
void EspsTab::repopulate(const QString &focus_active, const QString &focus_inactive) {
  active_list_->clear();
  inactive_list_->clear();
  for (const auto &e : esps_) {
    auto *item =
        new QListWidgetItem(e.filename, e.is_active() ? active_list_ : inactive_list_);
    item->setData(Qt::UserRole, e.is_active() ? e.active_path : e.inactive_path);
  }
...
void EspsTab::on_activate() {
  ...
  while (root.exists(new_name)) {
    ...
    const QString entered = QInputDialog::getText(
        this, tr("File Exists"),
        tr("A file with that name exists. Please enter a new one:"), QLineEdit::Normal,
        e.filename, &ok);
    if (!ok)
      return;
    if (!entered.trimmed().isEmpty())
      new_name = entered.trimmed();
  }

  if (!root.rename(e.inactive_path, new_name)) {
    QMessageBox::warning(this, tr("Activate Plugin"),
                         tr("Failed to move \"%1\" to the Data root.").arg(e.filename));
    return;
  }
...
void EspsTab::on_deactivate() {
  ...
  if (!root.rename(e.active_path, new_name)) {
    QMessageBox::warning(this, tr("Deactivate Plugin"),
                         tr("Failed to move \"%1\" to optional/.").arg(e.filename));
    return;
  }
