...
namespace {
  ...
  void br_tags_to_newlines(QString &s) {
    ...
    const QRegularExpression kBrTrail(QStringLiteral("<\\s*br\\s*/?\\s*>\n"),
    ...
    // pattern's "\n" + "<br>" shape.
    const QRegularExpression kBrLead(QStringLiteral("\n<\\s*br\\s*/?\\s*>"),
                                     QRegularExpression::CaseInsensitiveOption);
    s.replace(kBrLead, QStringLiteral("\n"));
    // A lone <br> with no adjacent newline still gets converted.
    const QRegularExpression kBrAlone(QStringLiteral("<\\s*br\\s*/?\\s*>"),
                                      QRegularExpression::CaseInsensitiveOption);
  ...
  constexpr std::array<QStringView, 5> kAllowedSchemes = {
      QStringView(u"http://"), QStringView(u"https://"), QStringView(u"mailto:"),
      QStringView(u"ftp://"), QStringView(u"ftps://")};
  ...
  void linkify_mentions(QString &s) {
    ...
    while (search_from < s.size()) {
      ...
      const QString replacement = QStringLiteral("[url=%1]@%2[/url]").arg(href, name);
...
void set_bbcode_html_async(DescriptionRenderer *renderer, const QString &desc,
  ...
  const unsigned token =
      request_token != nullptr ? request_token->load(std::memory_order_relaxed) : 0u;
