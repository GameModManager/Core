...
namespace {
  ...
  QStringList supported_image_suffixes() {
    ...
    for (const auto &f : formats)
      suffixes << QLatin1Char('.') + f.toLower();
    if (!suffixes.contains(QStringLiteral(".dds")))
      suffixes << QStringLiteral(".dds");
    return suffixes;
  }
...
ImagesTab::ImagesTab(QWidget *parent) : ModInfoTab(parent) {
  ...
  connect(thumbnails_, &QListWidget::currentItemChanged, this,
          [this](QListWidgetItem *current, QListWidgetItem *) {
            select_image(current ? current->data(Qt::UserRole).toString() : QString());
          });
...
void ImagesTab::set_mod(const ModInfoData &data) {
  ...
  if (root.exists()) {
    ...
    while (it.hasNext()) {
      ...
      if (!suffix.isEmpty() && suffixes.contains(QLatin1Char('.') + suffix)) {
        files_.push_back({full, root.relativeFilePath(full)});
      }
    }
  }

  std::sort(files_.begin(), files_.end(), [](const ImageFile &a, const ImageFile &b) {
...
void ImagesTab::first_activation() {
  ...
  for (int i = 0; i < thumbnails_->count(); ++i) {
    ...
    if (native.isValid() && !native.isEmpty() &&
        (native.width() > kThumb.width() || native.height() > kThumb.height())) {
      QSize target = native;
    ...
    if (img.width() > kThumb.width() || img.height() > kThumb.height())
      img = img.scaled(kThumb, Qt::KeepAspectRatio, Qt::SmoothTransformation);
