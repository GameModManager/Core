...
ConflictsInfoTab::ConflictsInfoTab(QWidget *parent) : ModInfoTab(parent) {
  ...
  for (auto *group : {&wins_, &loses_, &no_conflict_}) {
    connect(group->filter, &QLineEdit::textChanged, this, [this, group]() {
      apply_filter(*group);
    });
...
void ConflictsInfoTab::show_menu(Group &group, const QPoint &pos) {
  ...
  QObject::connect(hide, &QAction::triggered, this, [this, &group, hidden]() {
    on_hide(group, !hidden);
  });
...
void ConflictsInfoTab::on_hide(Group &group, bool hide) {
  ...
  if (abs.isEmpty())
    return;
  if (!current().hide_file || !current().refresh_conflicts)
    return;
