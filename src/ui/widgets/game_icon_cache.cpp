...
void GameIconFetcher::run(const QString &game_id, const QString &url) {
  std::string error;
  bool ok = engine::ensure_icon_cached(game_id.toStdString(), url.toStdString(), error);
  if (!ok) {
    engine::Logger::instance().warn("Icon fetch failed for " + game_id.toStdString() +
                                    ": " + error);
  }
...
namespace {
  ...
  QPixmap enforce_size(const QPixmap &src, int target) {
    ...
    if (src.width() == target && src.height() == target)
      return src;
    QPixmap scaled =
        src.scaled(target, target, Qt::KeepAspectRatio, Qt::SmoothTransformation);
    if (scaled.width() == target && scaled.height() == target)
      return scaled;
    QPixmap canvas(target, target);
    canvas.fill(Qt::transparent);
    QPainter p(&canvas);
    p.drawPixmap((target - scaled.width()) / 2, (target - scaled.height()) / 2, scaled);
    return canvas;
  }
...
GameIconCache::GameIconCache() {
  ...
  connect(worker_, &GameIconFetcher::finished, this, &GameIconCache::on_fetch_finished);
  thread_->start();
}
...
QIcon GameIconCache::icon_for(const QString &game_id, const QString &name, int size) {
  std::string gid = game_id.toStdString();

  std::string url = knowledge_ ? engine::icon_url_for(*knowledge_, gid) : std::string();
  if (!url.empty()) {
    auto cached = engine::cached_icon_path(gid);
    std::error_code ec;
    bool cached_ok =
        fs::is_regular_file(cached, ec) && !ec && fs::file_size(cached, ec) > 0;
    if (cached_ok) {
      QPixmap pm(QString::fromStdString(cached.string()));
      if (!pm.isNull())
        return QIcon(enforce_size(pm, size));
    }
...
void GameIconCache::request_fetch(const std::string &game_id, const std::string &url) {
  if (pending_.count(game_id))
...
void GameIconCache::on_fetch_finished(const QString &game_id, bool ok) {
  ...
  if (ok)
    emit icon_ready(game_id);
}

QIcon GameIconCache::make_placeholder(const QString &game_id, const QString &name,
                                      int size) {
  auto hash = std::hash<std::string>{}(game_id.toStdString());
  QColor base;
  switch (hash % 8) {
  case 0:
    base = QColor(100, 149, 237);
    break;  // cornflower blue
  case 1:
    base = QColor(220, 80, 80);
    break;  // red
  case 2:
    base = QColor(80, 180, 100);
    break;  // green
  case 3:
    base = QColor(200, 160, 60);
    break;  // gold
  case 4:
    base = QColor(160, 100, 200);
    break;  // purple
  case 5:
    base = QColor(60, 180, 200);
    break;  // teal
  case 6:
    base = QColor(220, 140, 60);
    break;  // orange
  case 7:
    base = QColor(120, 120, 180);
    break;  // slate
  }
