...
FlagsDelegate::FlagsDelegate(int flag_icons_role, int flag_tooltips_role,
                             QWidget *parent)
    : QStyledItemDelegate(parent), flag_icons_role_(flag_icons_role),
...
void FlagsDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option,
  ...
  const int y0 = option.rect.top() + std::max(0, (option.rect.height() - block_h) / 2);
  for (int i = 0; i < icons.size(); ++i) {
    const QRect target(option.rect.left() + (i % per_line) * step_x,
                       y0 + (i / per_line) * cell.height(), cell.width(),
                       cell.height());
...
QSize FlagsDelegate::sizeHint(const QStyleOptionViewItem &option,
  ...
  if (cell_width <= 0)
    cell_width = 80;  // default Flags column width
...
bool FlagsDelegate::helpEvent(QHelpEvent *event, QAbstractItemView *view,
  ...
  if (event->type() == QEvent::ToolTip && flag_tooltips_role_ != 0) {
    const QStringList tips = index.data(flag_tooltips_role_).value<QStringList>();
    if (!tips.isEmpty()) {
      const QList<QIcon> icons = flags_for_index(index, flag_icons_role_);
...
IndentDelegate::IndentDelegate(int indent_depth_role, int is_last_child_role,
    ...
    : QStyledItemDelegate(parent), indent_depth_role_(indent_depth_role),
      is_last_child_role_(is_last_child_role), is_separator_role_(is_separator_role) {}
...
void IndentDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option,
  ...
  if (checkboxWidth > 0) {
    ...
    checkOpt.state &= ~(QStyle::State_On | QStyle::State_Off | QStyle::State_NoChange);
    ...
    style->drawPrimitive(QStyle::PE_IndicatorItemViewItemCheck, &checkOpt, painter,
                         widget);
  }
...
void ModMarkingScrollBar::paintEvent(QPaintEvent *event) {
  ...
  if (!view_ || !view_->model())
    return;

  QStyleOptionSlider style_option;
  initStyleOption(&style_option);
  const QRect groove = style()->subControlRect(QStyle::CC_ScrollBar, &style_option,
  ...
  for (int r = 0; r < row_count; ++r) {
    if (!view_->isRowHidden(r, QModelIndex()))
  ...
  for (int i = 0; i < visible_rows.size(); ++i) {
    ...
    if (!color_variant.canConvert<QColor>())
      continue;
...
ModView::ModView(QWidget *parent) : QTreeView(parent) {
  ...
  setItemDelegateForColumn(ModList::Name,
                           new IndentDelegate(ModList::kIndentDepthRole,
                                              ...
                                              ModList::kIsSeparatorRole, this));
}

void ModView::apply_scrollbar_policy() {
  setVerticalScrollBarPolicy(Settings::instance().color_separator_scrollbar()
...
void ModView::setHeader(QHeaderView *header) {
  ...
  connect(header, &QHeaderView::sectionResized, this, [this](int logical, int, int) {
    if (logical == ModList::Conflicts || logical == ModList::Flags)
...
bool ModView::is_under_overwrite(const QString &path) const {
  ...
  if (canon.isEmpty() || ow_canon.isEmpty())
    return false;

  // Equal to or strictly inside the overwrite dir.
  return canon == ow_canon || canon.startsWith(ow_canon + QLatin1Char('/'));
}
