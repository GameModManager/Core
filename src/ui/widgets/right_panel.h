...
public:
  ...
  // "Data" is always shown. Other tabs only appear if the game registers that
  // capability.
  ...
  // nullptr when the capability is not in the tab bar. Use these when the
  // caller needs to push data or wire signals; the plain getters above
  // only peek and return nullptr while the tab is still a placeholder.
  DownloadsTab *ensure_downloads_tab();
...
signals:
  ...
  // A placeholder tab was replaced with its real content (first selection
  // or an ensure_* call). Controllers wire the fresh tab here - the wiring
  // that used to run eagerly after set_game. Emitted at most once per tab
  // instance (a game switch deletes the tabs, so the next materialization
  // re-emits for the rebuilt tab).
  void tab_materialized(const QString &capability);
...
private:
  ...
  // Replace the capability's placeholder with its real content and emit
  // tab_materialized. No-op (returns the existing tab) when already built;
  // returns nullptr when the capability is not in the tab bar.
  QWidget *materialize(const std::string &capability);
  void apply_filter();
  ...
  // Placeholders (Workspace-j6ty): capability -> "Loading..." label shown
  // until the real tab is built on first selection. tab_labels_ keeps the
  // display label so materialize() can re-insert the real tab in place.
  std::unordered_map<std::string, QWidget *> placeholders_;
  std::unordered_map<std::string, QString> tab_labels_;
  // True while materialize() swaps a placeholder for its real tab; the
  // remove/insert fires currentChanged for intermediate states, which the
  // handler skips so only the final selection is filtered and persisted.
