...
struct ModEntry {
  ...
  bool redundant =
      false;  // every file this mod provides is overridden by a higher-priority owner
  ...
  bool no_metadata =
      false;  // no manager metadata file in the folder (not a managed install)
  ...
  QString content_dir;
  // Mirror/backup state (Workspace-0pi5). is_mirrored: the mod's meta.ini
  // carries [Mirror]. mirror_source_missing: the recorded sourcePath is no
  // longer on disk (deleted by Steam etc.) - the row shows the warning
  // badge and starts unchecked; re-enabling uses the backup files.
  // mirror_source_path: the original external folder (for display).
...
inline bool is_phantom_row(const ModEntry& e) {
  return e.is_overwrite || e.is_merged || e.is_game_native ||
         e.id == QLatin1String(kOverwriteModId) || e.id == QLatin1String(kMergedModId);
}
...
public:
  ...
  void add_mod(const QString& id, const QString& name, const QString& version,
               int priority = -1, bool is_game_native = false, qint64 install_ts = 0,
               qint64 changed_ts = 0);
  ...
  // Mirror/backup state (Workspace-0pi5). Refreshes the Flags badge and
  // tooltip; the caller drives the enable checkbox separately via
  // set_mod_enabled.
  ...
  [[nodiscard]] bool has_visible_descendant(int row,
  ...
  [[nodiscard]] bool is_conflict_order_reversed() const {
    return conflict_order_reversed_;
  }
  ...
  [[nodiscard]] const QMap<QString, ConflictPairs>& conflict_pairs() const {
    return conflict_pairs_;
  }
...
private:
  ...
  QIcon invalid_icon_;
  QIcon empty_icon_;
  // Mirror/backup badges (Workspace-0pi5): merge (mirrored, source present)
