...
PipelineContentWidget::PipelineContentWidget(QWidget *parent) : QWidget(parent) {
  ...
  connect(refresh_timer_, &QTimer::timeout, this, &PipelineContentWidget::refresh);
  refresh_timer_->start(2000);
}

PipelineContentWidget::~PipelineContentWidget() {
  if (refresh_timer_)
    refresh_timer_->stop();
}

QString PipelineContentWidget::status_color(engine::TraceStatus status) {
  switch (status) {
  case engine::TraceStatus::Running:
    return "#1e88e5";
  case engine::TraceStatus::Completed:
    return "#2e7d32";
  case engine::TraceStatus::Failed:
    return kFailColor;
  case engine::TraceStatus::Skipped:
    return "#757575";
  case engine::TraceStatus::NotImplemented:
  case engine::TraceStatus::Pending:
    return "#9e9e9e";
  }
  return "#9e9e9e";
}

QString PipelineContentWidget::status_text(engine::TraceStatus status) {
  switch (status) {
  case engine::TraceStatus::Running:
    return "Running";
  case engine::TraceStatus::Completed:
...
QString PipelineContentWidget::format_duration(int64_t ms) {
  if (ms <= 0)
    return QString();
  if (ms < 1000)
...
void PipelineContentWidget::show_placeholder(FlowTab &tab, const QString &text) {
  tab.scene->clear();
  tab.cards.clear();
...
void PipelineContentWidget::rebuild_cards(
    FlowTab &tab, const std::vector<engine::TraceStage> &stages) {
  tab.scene->clear();
  tab.cards.clear();
  tab.connectors.clear();
  tab.stage_names.clear();
  ...
  for (size_t i = 0; i + 1 < tab.cards.size(); ++i) {
    ...
    conn.head  = tab.scene->addPolygon(QPolygonF(), QPen(Qt::NoPen), QBrush());
...
void PipelineContentWidget::update_cards(
    FlowTab &tab, const std::vector<engine::TraceStage> &stages) {
  for (size_t i = 0; i < tab.cards.size() && i < stages.size(); ++i) {
    ...
    bool failed = stage.status == engine::TraceStatus::Failed && !stage.reason.empty();
...
void PipelineContentWidget::relayout(FlowTab &tab) {
  if (tab.cards.empty())
    return;
  ...
  for (auto &conn : tab.connectors) {
    ...
    QPolygonF tri;
    tri << QPointF(x2 - a_size, y - a_size) << QPointF(x2 - a_size, y + a_size)
        << QPointF(x2, y);
...
void PipelineContentWidget::refresh() {
  for (auto &[flow_id, tab] : tabs_by_flow_) {
    ...
    tab.header->setText(
        QString("%1: %2").arg(QString::fromStdString(snap->title)).arg(state));
