...
QVariant ModList::data(const QModelIndex &index, int role) const {
  ...
  if (role == kScrollMarkRole) {
    if (mod.is_separator) {
      ...
      if (!Settings::instance().color_separator_scrollbar())
        return {};
      QColor bg(mod.separator_color.isEmpty() ? "#888888"
                                              : mod.separator_color);
      return bg;
    }
  ...
  if (role == Qt::DisplayRole && index.column() == Fold) {
    const bool nestable_mod = !mod.is_separator && !mod.is_overwrite &&
  ...
  if (mod.is_separator) {
    ...
    if (role == Qt::ForegroundRole) {
      ...
      QColor bg(mod.separator_color.isEmpty() ? "#888888"
                                              : mod.separator_color);
      int l = static_cast<int>(0.299 * bg.red() + 0.587 * bg.green() +
                               0.114 * bg.blue());
      return QBrush(l > 128 ? QColor(0, 0, 0) : QColor(255, 255, 255));
    }
    if (role == Qt::FontRole &&
        (index.column() == Name || index.column() == Fold)) {
      QFont f;
      f.setBold(true);
      return f;
    }
    if (role == Qt::DisplayRole) {
      ...
      case Version:
        return QString();
      case Flags:
        return QString(); // icons come via kFlagIconsRole
      case Priority:
        return mod.priority;
      }
    }
  ...
  if (role == Qt::DisplayRole || role == Qt::EditRole) {
    ...
    case Name:
      return mod.name;
    case Conflicts:
      return QString(); // icon via kFlagIconsRole
    case Flags:
      return QString(); // icon via kFlagIconsRole
    case Category:
      return mod.category;
  ...
  if (role == Qt::DecorationRole && index.column() == Source &&
      !mod.is_separator) {
    return source_icon(mod.source_type);
  }
  // Conflicts tooltip: what this mod wins/loses against.
  if (role == Qt::ToolTipRole && index.column() == Conflicts &&
      !mod.is_separator &&
  ...
  if (role == Qt::ToolTipRole && index.column() == Source &&
      !mod.source_type.isEmpty())
...
bool ModList::setData(const QModelIndex &index, const QVariant &value,
                      int role) {
  if (!index.isValid() || index.row() >= mods_.size())
    return false;
...
Qt::DropActions ModList::supportedDropActions() const { return Qt::MoveAction; }

Qt::DropActions ModList::supportedDragActions() const { return Qt::MoveAction; }

QStringList ModList::mimeTypes() const { return {kModListMimeType}; }
...
bool ModList::dropMimeData(const QMimeData *data, Qt::DropAction action,
  ...
  if (action != Qt::MoveAction)
    return false;
  if (!data->hasFormat(kModListMimeType))
    return false;
  ...
  for (int r : sourceRows) {
    if (r < 0 || r >= mods_.size())
      continue;
    if (mods_[r].is_overwrite || mods_[r].is_merged || mods_[r].is_game_native)
      continue;
    if (validSources.contains(r))
      continue;
    validSources.append(r);
    if (nesting_enabled_) {
      for (int j = 0; j < mods_.size(); ++j)
        if (j != r && !validSources.contains(j) &&
            is_descendant_of(j, mods_[r].id))
  ...
  if (on_item && nesting_enabled_ && drop_parent_row >= 0 &&
    ...
    if (tgt_nestable) {
      ...
      if (all_same_kind && !target_in_block)
    ...
    if (tgt.is_separator) {
      for (int j = drop_parent_row + 1; j < mods_.size(); ++j) {
        if (mods_[j].is_overwrite || mods_[j].is_merged)
          break;
        if (mods_[j].is_separator && !is_descendant_of(j, new_parent_id))
          break;
        last_desc = j;
      }
    } else {
  ...
  if (targetRow < 0)
    targetRow = 0;
  if (targetRow > mods_.size())
    targetRow = mods_.size();
...
bool ModList::moveRows(const QModelIndex &srcParent, int srcRow, int count,
  ...
  if (srcRow < 0 || srcRow + count > mods_.size())
    return false;
  if (dstRow < 0 || dstRow > mods_.size())
    return false;
  if (count != 1)
    return false;

  if (mods_[srcRow].is_overwrite || mods_[srcRow].is_merged ||
      mods_[srcRow].is_game_native)
    return false;
  ...
  beginMoveRows(srcParent, srcRow, srcRow, srcParent,
                dest + (dest >= srcRow ? 1 : 0));
...
void ModList::add_mod(const QString &id, const QString &name,
                      const QString &version, int priority, bool is_game_native,
                      qint64 install_ts, qint64 changed_ts) {
...
void ModList::rename_mod_in_place(int row, const QString &new_id,
  ...
  emit dataChanged(index(row, Name), index(row, Version),
...
void ModList::remove_mod(const QString &id) {
  ...
  for (int i = 0; i < mods_.size(); ++i) {
    if (mods_[i].id == id && !mods_[i].is_overwrite && !mods_[i].is_merged &&
        !mods_[i].is_game_native) {
...
void ModList::move_mod(const QString &id, int new_row) {
  ...
  if (src < 0 || src == new_row)
    return;
  if (mods_[src].is_game_native)
    return;
...
void ModList::set_source_info(const QString &id, const QString &source_type,
  ...
  for (int i = 0; i < mods_.size(); ++i) {
    if (mods_[i].id == id) {
      if (mods_[i].source_type != source_type ||
        ...
        emit dataChanged(
            index(i, Source), index(i, SourceId),
...
void ModList::set_timestamps(const QString &id, qint64 install_ts,
                             qint64 changed_ts) {
  for (int i = 0; i < mods_.size(); ++i) {
    if (mods_[i].id == id && (mods_[i].installation_ts != install_ts ||
...
void ModList::renumber_priorities() {
  for (int i = 0; i < mods_.size(); ++i) {
  ...
  // Fold column.
  if (!mods_.isEmpty())
    emit dataChanged(index(0, Fold), index(mods_.size() - 1, Fold),
                     {Qt::DisplayRole});
}
...
int ModList::overwrite_row() const {
  for (int i = 0; i < mods_.size(); ++i) {
...
bool ModList::is_overwrite(int row) const {
...
int ModList::nesting_depth(int row) const {
  if (!nesting_enabled_)
    return 0;
  if (row < 0 || row >= mods_.size())
    return 0;
...
bool ModList::has_content(int row) const {
  ...
  if (!nesting_enabled_) {
    ...
    for (int i = row + 1; i < mods_.size(); ++i) {
      if (mods_[i].is_separator || mods_[i].is_overwrite)
        return false;
      return true;
    }
    return false;
  ...
  for (int j = 0; j < mods_.size(); ++j) {
    if (j != row && is_descendant_of(j, m.id))
      return true;
  }
  if (m.is_separator) {
    for (int i = row + 1; i < mods_.size(); ++i) {
      ...
      if (mods_[i].is_separator) {
        if (is_descendant_of(i, m.id))
          continue;   // inside the scope
...
QVector<bool> ModList::compute_fold_hidden() const {
...
bool ModList::is_row_fold_hidden(int row) const {
  if (row < 0 || row >= mods_.size())
    return false;
  return compute_fold_hidden()[row];
}

void ModList::sanitize_parent_links() {
  bool changed = false;
  for (int i = 0; i < mods_.size(); ++i) {
...
bool ModList::has_visible_descendant(int row,
                                     const QVector<bool> &visible) const {
  if (row < 0 || row >= mods_.size() || !nesting_enabled_)
    return false;
  for (int j = 0; j < mods_.size(); ++j)
    if (j != row && visible.value(j, false) &&
        is_descendant_of(j, mods_[row].id))
      return true;
  return false;
}
...
QIcon ModList::source_icon(const QString &source_type) const {
  ...
  const QString key = QString::fromStdString(
      engine::vendor_icon_key(source_type.toStdString()));
  if (key.isEmpty())
    return {};
...
void ModList::pin_pinned_rows() {
  ...
  if (pinned.isEmpty())
    return; // nothing to pin; keep caller order
...
void ModList::set_conflict_order_reversed(bool reversed) {
  if (conflict_order_reversed_ == reversed)
    return;
  conflict_order_reversed_ = reversed;
}

QString ModList::compute_separator_flags(int row) const {
  bool has_wins = false, has_losses = false;
  for (int i = row + 1; i < mods_.size(); ++i) {
    if (mods_[i].is_separator)
      break;
    if (mods_[i].conflict_wins > 0)
      has_wins = true;
    if (mods_[i].conflict_losses > 0)
      has_losses = true;
  }
  if (has_wins && has_losses)
    return QString("\u00B1");
  if (has_wins)
    return QString("+");
  if (has_losses)
...
void ModList::set_overwrite_path(const QString &path) {
  ...
  int ow_row = overwrite_row();
  if (ow_row >= 0)
    emit dataChanged(index(ow_row, Name), index(ow_row, Name),
                     {Qt::ForegroundRole});
}
...
void ModList::ensure_overwrite_present() {
  ...
  // When called from the constructor the model is not yet connected to any
  // view, so beginInsertRows/endInsertRows is unnecessary and can trigger a
  // segfault in the QList insert path (Qt6 QVector<QList> internals for
  // non-trivially-relocatable types).  Directly append to the backing
  // storage and let the view pick up the row when rowCount()/data() are
  // next queried after the model is set.
  if (mod_view_) {
...
int ModList::merged_row() const {
  for (int i = 0; i < mods_.size(); ++i) {
    if (mods_[i].is_merged)
      return i;
  }
  return -1;
}

int ModList::native_band_first() const {
  for (int i = 0; i < mods_.size(); ++i) {
    if (mods_[i].is_game_native)
      return i;
  }
  return mods_.size();
}

int ModList::native_band_last() const {
  for (int i = mods_.size() - 1; i >= 0; --i) {
    if (mods_[i].is_game_native)
      return i;
  }
...
bool ModList::is_merged(int row) const {
  return row >= 0 && row < mods_.size() && mods_[row].is_merged;
}

void ModList::set_uses_merged(bool on) {
  if (uses_merged_ == on)
    return;
  uses_merged_ = on;
  if (on) {
...
void ModList::ensure_merged_present() {
