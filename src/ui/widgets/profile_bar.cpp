...
ProfileBar::ProfileBar(QWidget *parent) : QWidget(parent) {
  ...
  auto add_folder    = [this, folders_menu](const QString &text, FolderKind kind) {
    ...
    connect(action, &QAction::triggered, this, [this, kind]() {
      emit open_folder_requested(kind);
    });
    return action;
  };
  ...
  connect(export_action, &QAction::triggered, this,
          &ProfileBar::export_modlist_clicked);
  connect(import_action, &QAction::triggered, this,
          &ProfileBar::import_modlist_clicked);
  ...
  connect(create_menu->actions()[0], &QAction::triggered, this,
          &ProfileBar::create_separator_clicked);
  connect(create_menu->actions()[1], &QAction::triggered, this,
          &ProfileBar::create_empty_mod_clicked);
