...
void ListDialog::apply_filter() {
    ...
    for (int i = 0; i < choices_.size(); ++i) {
        if (filter.isEmpty() ||
            choices_[i].contains(filter, Qt::CaseInsensitive)) {
            newChoices << choices_[i];
            if (i < data_.size()) newData << data_[i];
        }
    }

    for (int i = 0; i < newChoices.size(); ++i) {
        auto* item = new QListWidgetItem(newChoices[i], list_);
        if (i < newData.size()) item->setData(Qt::UserRole, newData[i]);
    }
    ...
    auto* box = buttons_;
    if (box) box->button(QDialogButtonBox::Ok)->setEnabled(list_->count() > 0);
}
