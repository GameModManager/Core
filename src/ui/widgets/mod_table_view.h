...
inline int flag_icon_at(const QList<QIcon> &icons, const QRect &cell_rect,
                        const QPoint &local) {
  if (icons.isEmpty() || !cell_rect.contains(local))
    return -1;
  const QSize cell = flags_cell_size(icons);
  const int per_line =
  ...
  const QRect icon_rect(cell_rect.left() + (idx % per_line) * step_x,
                        y0 + (idx / per_line) * cell.height(), cell.width(),
                        cell.height());
...
public:
  explicit IndentDelegate(int indent_depth_role, int is_last_child_role,
                          int is_separator_role, QWidget *parent = nullptr);
...
private:
  ...
  static constexpr int kCenterOffset =
