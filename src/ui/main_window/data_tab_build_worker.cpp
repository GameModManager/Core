...
resolve_mod_file(const std::string &mod_id, const std::string &rel_path,
...
DataTabRow build_data_row(const std::string &key_path, const std::string &display_path,
                          ...
                          const std::filesystem::path &game_mods_dir,
                          engine::DeploySpace space, const std::string &deploy_prefix,
  ...
  auto winner = conflict_reversed ? std::min_element(owners.begin(), owners.end(),
  ...
  std::error_code ec;
  const auto winner_real =
      resolve_mod_file(winner->first, key_path, mods_dir, game_mods_dir);
  if (!winner_real.empty()) {
    ...
    if (!ec)
      row.size = static_cast<qint64>(sz);
  }
...
static bool is_reserved_root_dir(const std::string &name,
                                 const std::string &mods_subpath) {
  auto lname = name;
  for (char &c : lname)
    c = static_cast<char>(std::tolower(static_cast<unsigned char>(c)));
  std::unordered_set<std::string> reserved = {"overwrite", "merged", ".merged",
                                              ".gmm_staging"};
  auto lsub                                = mods_subpath;
  for (char &c : lsub)
    c = static_cast<char>(std::tolower(static_cast<unsigned char>(c)));
  if (!lsub.empty())
    reserved.insert(std::move(lsub));
  else
    reserved.insert("data");
...
static bool dirs_first_less(const QString &ap, const QString &bp,
...
std::vector<DataTabRow> build_data_tab_rows(const DataTabBuildRequest &request) {
  ...
  engine::parallel::for_each(entries.size(), [&](size_t i) {
    ...
    const auto cls =
        engine::classify_registry_path(key, owners, root_override_mods, deploy_prefix);
    ...
    rows[i] = build_data_row(key, cls.display_path, owners, display_names,
                             conflict_reversed, mods_dir, game_mods_dir, cls.space,
                             deploy_prefix, deploy_include_mod_id, root_override_mods);
  });
  ...
  std::sort(rows.begin(), rows.end(),
            [&dirs](const DataTabRow &a, const DataTabRow &b) {
              if (a.path != b.path)
                return dirs_first_less(a.path, b.path, dirs);
