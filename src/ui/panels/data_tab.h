...
signals:
  ...
  void preview_requested(const QString &file_path, const QStringList &provider_paths,
  ...
  void add_executable_requested(const QString &file_path, const QString &default_name,
...
private:
  ...
  std::vector<DataTabRow>
      applied_rows_;  // last fully applied row set (path-keyed no-op)
