...
DataTab::DataTab(QWidget *parent) : QWidget(parent) {
  ...
  connect(tree_, &QTreeWidget::itemDoubleClicked, this,
          &DataTab::on_item_double_clicked);
  ...
  connect(build_thread_->worker(), &DataTabBuildWorker::finished, this,
          &DataTab::on_build_finished);
}
...
void DataTab::show_data(
  ...
  const bool inputs_changed =
      registry != stored_registry_ || !same_mod_list(stored_mods_, all_mods) ||
      stored_conflict_reversed_ != conflict_reversed || stored_mods_dir_ != mods_dir ||
      stored_game_mods_dir_ != game_mods_dir ||
      stored_game_root_dir_ != game_root_dir || stored_mods_subpath_ != mods_subpath ||
...
void DataTab::on_build_finished(DataTabBuildResult result, quint64 generation) {
  if (generation != build_generation_)
    return;  // a newer build superseded it
  build_pending_ = false;
...
void DataTab::apply_build_result(DataTabBuildResult result) {
  ...
  // is exact).
  if (result.rows == applied_rows_)
    return;

  // Incremental path: on a mod reorder only the winners of conflicted files
  // change - the path set is identical and most rows compare equal. Updating
  // just the changed rows in place avoids tree_->clear() + recreating all
  // 25k items (the visible reorder stall). Added/removed paths (mod
  // install/uninstall) and small winner-only deltas both go through here;
  // only large structural changes fall through to the chunked full rebuild.
  if (!applied_rows_.empty() && !result.rows.empty()) {
    ...
    for (auto it = old_eff.constBegin(); it != old_eff.constEnd(); ++it) {
      ...
      if (nit == new_eff.constEnd())
        removed.push_back(it.key());
      else if (*(*nit) != *it.value())
        changed.push_back(*nit);
    }
    for (auto it = new_eff.constBegin(); it != new_eff.constEnd(); ++it) {
      if (!old_eff.contains(it.key()))
        added.push_back(it.value());
    }

    const std::size_t delta = removed.size() + added.size() + changed.size();
    const auto base =
        static_cast<std::size_t>(std::max(old_eff.size(), new_eff.size()));
    // Hidden rows share their display slot with a visible copy (the worker
    // sorts the visible copy first and upsert_data_row skips a hidden row
    // whose slot is taken). A delta that flips a slot between hidden and
    ...
    // Nav-row expectation for the current view: a mismatch (view switch
    ...
    const bool want_nav = view_ == View::Root || !stored_game_root_dir_.empty();
    if (has_nav != want_nav) {
      ...
      for (const auto &path : removed) {
        ...
        delete item;
        // Prune directories left empty by the removal (never the
        // invisible root, file-backed items, or the nav row).
        ...
        while (cur != nullptr && cur != root_item && cur->childCount() == 0 &&
          ...
          delete cur;
          cur         = grand;
          const int s = dir_path.lastIndexOf('/');
      ...
      if (!added.empty() || !removed.empty())
        sort_dirs_first(root_item);
      tree_->setUpdatesEnabled(true);
      applied_rows_ = std::move(result.rows);
      pending_rows_.clear();
      pending_pos_ = 0;
      engine::Logger::instance().debug(
          "Data tab updated incrementally (" +
          std::string(view_ == View::Root ? "root" : "data") + " view): " +
          std::to_string(changed.size()) + " changed, " + std::to_string(added.size()) +
          " added, " + std::to_string(removed.size()) + " removed");
      return;
    }
  }
  ...
  if (!result.rows.empty()) {
    if (view_ == View::Root) {
      ...
      tree_->topLevelItem(0)->setToolTip(0, tr("Open the %1 folder").arg(label));
  ...
  if (pending_rows_.empty()) {
    applied_rows_.clear();
    engine::Logger::instance().debug(
        "Data tab populated (" + std::string(view_ == View::Root ? "root" : "data") +
        " view): 0 files");
    return;
  }
...
void DataTab::apply_chunk_step() {
  ...
  engine::Logger::instance().debug(
      "Data tab populated (" + std::string(view_ == View::Root ? "root" : "data") +
      " view): " + std::to_string(applied_rows_.size()) + " files");
...
void DataTab::apply_mod(
  ...
  for (const auto &[path, owners] : registry) {
    ...
    if (!provides)
      continue;
    const auto cls =
        engine::classify_registry_path(path, owners, root_mods, deploy_prefix);
    if (root_view && cls.space != engine::DeploySpace::Root)
      continue;
    if (!root_view && cls.space != engine::DeploySpace::Data)
      continue;
    any = true;
    upsert_data_row(tree_,
                    build_data_row(path, cls.display_path, owners, display_names,
                                   conflict_reversed, mods_dir, game_mods_dir,
                                   cls.space, deploy_prefix, deploy_include_mod_id,
                                   root_mods),
                    item_index_);
  }
...
void DataTab::on_item_double_clicked(QTreeWidgetItem *item, int column) {
  ...
  if (!item->data(0, DataNavRole).isNull()) {
    switch_view(item->data(0, DataNavRole).toInt() == kNavTargetRoot ? View::Root
                                                                     : View::Data);
    return;
  }
