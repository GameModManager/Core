...
int main(int argc, char *argv[]) {
  ...
  if (parser.isSet(helpOpt) || parser.isSet(helpShort)) {
    ...
    fprintf(stdout,
            ...
            O, D, B, D);
    fprintf(stdout,
            "  gamemodmanager %s--launch%s %s--instance%s %s<path>%s %s--exe%s "
            "%s<path>%s\n",
            O, D, O, D, B, D, O, D, B, D);
    fprintf(stdout,
    ...
    fprintf(
        stdout,
        "  %s--exe%s %s<path>%s      Executable path relative to game dir\n", O,
        D, B, D);
    fprintf(stdout,
            "  %s--handle-nxm%s %s<url>%s  Handle an nxm:// download link\n", O,
            D, B, D);
    fprintf(stdout,
            "  %s--handle-gmm%s %s<url>%s  Handle a gmm:// download link\n", O,
            D, B, D);
    return 0;
  }
  ...
  // loading so the masterlist_url hooks are registered. Fetches happen
  // synchronously at startup; the plugin reads the cached file later.
  for (const auto &game_id : plugin_loader.knowledge().registered_games()) {
    std::string url =
        engine::masterlist_url_for(plugin_loader.knowledge(), game_id);
    if (url.empty())
      continue;
  ...
  if (headless) {
    ...
    if (steam_appid == 0) {
      auto id_str =
          plugin_loader.knowledge().get(inst.info().game_id, "steam_appid", "");
      if (!id_str.empty()) {
        try {
  ...
  if (handle_nxm || handle_gmm) {
    ...
    if (engine::send_nxm_to_running_instance(
            QString::fromStdString(pending_url))) {
    ...
    // ourselves Match the nexus_domain to a game_id via plugins
        ...
        [&](const ui::GameEntry &entry) {
          ...
          if (!pending_url.empty()) {
            main_window->handle_nxm_download(
                engine::NxmRouter::parse(pending_url));
