...
Application::Application(int& argc, char** argv)
  ...
  if (handle_count > 1) {
    ...
    fprintf(stderr, "GameModManager: cannot combine --handle-nxm/--handle-gmm/"
  ...
  // Prefetch masterlists for all registered games on a background thread so
  // startup never blocks on the network. This runs after plugin loading so
  // the masterlist_url hooks are registered. Consumers read through
  // read_cached_masterlist(), which returns empty until the fetch lands - a
  // missing cache is always "not yet available", never an error. The thread
  // is detached: it only touches the cache files and the thread-safe logger,
  // so outliving the Application during shutdown is harmless (worst case a
...
int Application::run() {
  ...
  if (args.headless) {
    ...
    if (args.handle_nxm || args.handle_gmm || args.handle_modl) {
      engine::Logger::instance().error(
          ...
          "--handle-modl.");
      fprintf(stderr, "GameModManager: --launch is for game launch only. Drop the "
    ...
    if (inst.info().game_dir.empty()) {
      engine::Logger::instance().error("game_dir not set in instance.toml for " +
                                       instance_root.string());
      return 1;
    }
    ...
    auto exec_path =
        (ce || canon_game.empty() ? inst.info().game_dir : canon_game) / exe_rel;
    ...
    if (steam_appid == 0) {
      auto id_str =
          plugin_loader_->knowledge().get(inst.info().game_id, "steam_appid", "");
      if (!id_str.empty()) {
        try {
  ...
  if (args.handle_nxm || args.handle_gmm) {
    ...
    if (matched_game_id.empty()) {
      engine::Logger::instance().error("No game plugin supports Nexus domain: " +
                                       link.nexus_domain);
    ...
    if (engine::send_url_to_running_instance(QString::fromStdString(pending_url_))) {
    ...
    if (target_instance.empty()) {
      ...
      engine::Logger::instance().error("No instance exists for " + display_name + " (" +
                                       matched_game_id + ")");
      fprintf(stderr,
    ...
    engine::Logger::instance().debug("Download URL: resolved to instance " +
                                     target_instance + " (game=" + matched_game_id +
                                     ")");
  } else if (args.handle_modl) {
    ...
    if (link.game_id != "other") {
      ...
      if (matched_game_id.empty()) {
        engine::Logger::instance().error("No game plugin supports modl host: " +
                                         link.game_id);
        fprintf(stderr, "GameModManager: no game supports modl host '%s'\n",
                link.game_id.c_str());
        return 1;
      }
    }
    ...
    if (engine::send_url_to_running_instance(QString::fromStdString(pending_url_))) {
      engine::Logger::instance().info("modl:// URL forwarded to running instance");
      return 0;
    }
    ...
    if (target_instance.empty() || !fs::exists(engine::default_instances_dir() /
                                               target_instance / "instance.toml")) {
      engine::Logger::instance().error(
          "No instance available for modl:// link "
          "(open GameModManager and create an instance first)");
      fprintf(stderr, "GameModManager: no instance available for modl:// link. "
                      "Open GameModManager and create an instance first.\n");
      return 1;
    }
    ...
    engine::Logger::instance().debug(
        ...
        (matched_game_id.empty() ? " (other)" : " (game=" + matched_game_id + ")"));
  }
  ...
  if (!instance_name.isEmpty()) {
    ...
    if (active_instance.empty() || !fs::exists(engine::default_instances_dir() /
                                               active_instance / "instance.toml")) {
  ...
  if (!active_instance.empty()) {
    ...
    auto temp_inst =
        engine::Instance::installed(active_instance, engine::default_instances_dir());
    temp_inst.read_toml();
    ...
    window.setWindowTitle(QCoreApplication::translate("main", "GameModManager - %1")
