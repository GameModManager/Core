...
static fs::path temp_dir(const char *tag) {
  fs::path p = fs::temp_directory_path() / ("gmm_loverslab_test_" + std::string(tag) +
...
TEST_CASE("loverslab auth", "[engine]") {
  ...
  // ---- LoversLabAuth with injected keyring ----------------------
  {
    ...
    CHECK_MSG(auth.has_cookie(), "cookie present after set");
    CHECK_MSG(auth.get_cookie() == clean_cookie,
              "get returns the sanitized cookie (cf/__cf cookies dropped)");
    CHECK_MSG(fs::exists(primary_dir / "keyring_loverslab_cookie.dat"),
              "cookie stored in injected keyring");
    auth.clear_cookie();
    CHECK_MSG(!auth.has_cookie(), "cookie gone after clear");
  }
  ...
  // ---- LoversLabAuth fallback to internal file storage ----------
  {
    ...
    CHECK_MSG(auth.get_cookie() == "fallback=xyz", "fallback get returns cookie");
    CHECK_MSG(fs::exists(config / "GameModManager" / "keyring_loverslab_cookie.dat"),
              "fallback wrote its file");
    auth.clear_cookie();
    CHECK_MSG(!auth.has_cookie(), "fallback cleared");
  }
  ...
  {
    CHECK_MSG(engine::download::parse_content_disposition_filename(
    ...
    CHECK_MSG(engine::download::parse_content_disposition_filename("").empty(),
              "empty header -> empty");
    CHECK_MSG(engine::download::percent_decode("a%20b%2Fc") == "a b/c",
              "percent decode");
    CHECK_MSG(engine::download::percent_decode("plain") == "plain",
  ...
  // ---- sanitize_cookie() ----------------------------------------
  {
    ...
    CHECK_MSG(S::sanitize_cookie("ips4_member_id\t\"3430506\"\n"
    ...
    CHECK_MSG(S::sanitize_cookie("a=\"quoted\"; b=2") == "a=quoted; b=2",
              "quoted values unquoted");
    CHECK_MSG(S::sanitize_cookie("a=1; cf_clearance=xyz; b=2") == "a=1; b=2",
              "cf_clearance dropped");
    CHECK_MSG(S::sanitize_cookie("a=1; __cf_bm=xyz; b=2") == "a=1; b=2",
              "__cf_* dropped");
    CHECK_MSG(S::sanitize_cookie("malformed; a=1; =2; ;b=3") == "a=1; b=3",
              "malformed / empty tokens skipped");
    CHECK_MSG(S::sanitize_cookie("  a=1  ;  b = 2  ") == "a=1; b=2",
              "surrounding whitespace trimmed");
    CHECK_MSG(S::sanitize_cookie("") == "", "empty stays empty");
    CHECK_MSG(S::sanitize_cookie("   \n  ") == "", "whitespace-only stays empty");
    CHECK_MSG(S::sanitize_cookie("ips4_dataLayerLogin=eyJ0IjoxfQ==; a=b") ==
    ...
    CHECK_MSG(S::sanitize_cookie(json) ==
    ...
    const std::string json_cf =
        ...
        "{\"name\": \"ips4_member_id\", \"value\": \"3430506\"} ]";
    CHECK_MSG(S::sanitize_cookie(json_cf) == "ips4_member_id=3430506",
              "JSON form drops cf_clearance");
    const std::string json_no_match =
        "[ {\"name\": \"ips4_member_id\", \"domain\": \"loverslab.com\"} ]";
    CHECK_MSG(S::sanitize_cookie(json_no_match).empty(),
