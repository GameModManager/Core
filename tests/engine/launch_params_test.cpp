...
static void check_deploy(const fs::path &root, bool include_mod_id,
  ...
  const auto params = engine::prepare_launch_params(
      root, game_dir, exe, make_knowledge(include_mod_id), "testgame", 12345, true);
  ...
  require(params.extra_lowerdirs.size() == 1, label + ": exactly one extra lowerdir");
  require(params.extra_lowerdirs.front() == root / ".gmm_staging",
          label + ": lowerdir is <root>/.gmm_staging");
  require(fs::is_directory(root / ".gmm_staging"), label + ": .gmm_staging created");

  // Enabled mod file deployed as a symlink back into the mods folder.
  const fs::path deployed =
      include_mod_id ? root / ".gmm_staging" / "Data" / "EnabledMod" / "RaceMenu.esp"
  ...
  const fs::path disabled =
      include_mod_id ? root / ".gmm_staging" / "Data" / "DisabledMod" / "SkyUI.esp"
...
static void check_deploy_request(const fs::path &root, const std::string &label) {
  ...
  const auto params =
      engine::prepare_launch_params(req, [&progress](int done, int total) {
  ...
  require(params.environment.size() == 2 && params.environment[0] == "WINEDEBUG=+file",
          label + ": environment order preserved");
  require(params.args == req.args, label + ": args forwarded to LaunchParams");
  require(params.args.size() == 2 && params.args[1] == "bar baz",
          label + ": args order and quoting preserved");
  require(params.cwd == req.cwd, label + ": cwd forwarded to LaunchParams");
...
static void check_ci_deploy(const fs::path &root, const std::string &label) {
  ...
  const auto params =
      engine::prepare_launch_params(root, game_dir, exe, k, "testgame", 12345, true);
...
static void check_deploy_direct(const fs::path &root, bool include_mod_id,
  ...
  const auto params = engine::prepare_launch_params(
      root, game_dir, exe, make_knowledge(include_mod_id, "symlink"), "testgame", 12345,
      true);
...
// game_dir.
static void check_direct_winner_change(const fs::path &root, const std::string &label) {
  ...
  auto deploy = [&]() {
    return engine::deploy_all_enabled_mods_direct(mods, game_dir, "Data", false,
                                                  "disabled.txt", true, ledger);
  };
  ...
  require(!fs::exists(dunique), label + ": disabled mod's file unlinked from game_dir");
...
static void check_direct_executable_copy(const fs::path &root,
  ...
  require(engine::deploy_all_enabled_mods_direct(mods, game_dir, "Data", false,
                                                 "disabled.txt", true, ledger),
          label + ": deploys");
...
static void check_direct_backup_restore(const fs::path &root,
  ...
  auto deploy = [&]() {
    return engine::deploy_all_enabled_mods_direct(
        mods, game_dir, "Data", false, "disabled.txt", true, ledger, originals);
  };
  ...
  require(fs::is_regular_file(backup),
          label + ": original backed up into Original_Files");
  {
  ...
  require(fs::is_regular_file(backup),
          label + ": post-removal deploy re-backs-up the original");
}
...
static void check_merged_view(const fs::path &base) {
  ...
  if (!ec) {
    ...
    require(
        engine::merged_view_file_resolve(game_dir, staging, via_link) ==
  ...
  const fs::path mods_exe = base / "mods" / "skse64_2_02_06" / "skse64_loader.exe";
...
static void check_overlay_unavailable(const fs::path &root) {
  ...
  const auto params = engine::prepare_launch_params(
      root, game_dir, exe, make_knowledge(false), "testgame", 12345, true);
...
TEST_CASE("launch params", "[engine]") {
  ...
  check_deploy_direct(base / "instances" / "DirectByMod", true,
                      "direct include-mod-id deploy");
  check_direct_winner_change(base / "instances" / "DirectWinner",
                             "direct winner change");
  check_direct_executable_copy(base / "instances" / "DirectExe",
                               "direct executable copy");
  check_direct_backup_restore(base / "instances" / "DirectBackup",
