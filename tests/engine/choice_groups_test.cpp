...
const GroupVerdict &verdict_for(const ChoiceValidation &validation,
                                const std::string &id) {
...
TEST_CASE("Unknown groups and non-member picks are reported, not counted",
          "[collection][choice]") {
  const auto groups = single_group("textures", ChoiceMode::ExactlyOne);
...
TEST_CASE("Prior pick is remembered when membership is identical",
          "[collection][choice]") {
  const auto groups = single_group("textures", ChoiceMode::ExactlyOne);
...
TEST_CASE("Prior pick is dropped when the group no longer exists",
          "[collection][choice]") {
...
TEST_CASE("Explicit current pick wins over the remembered pick",
          "[collection][choice]") {
  const auto groups = single_group("textures", ChoiceMode::ExactlyOne);
  ...
  const auto effective =
      reconcile_prior_choices(groups, make_prior(prior_picks, membership), current);

  REQUIRE(effective.at("textures") == std::vector<std::string>{"tex-c"});
}

TEST_CASE("Prior pick without a membership snapshot is dropped",
          "[collection][choice]") {
  const auto groups = single_group("textures", ChoiceMode::ExactlyOne);
