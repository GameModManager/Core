...
TEST_CASE("nxm router parse", "[engine]") {
  ...
  expect_link("nxm://skyrimspecialedition/mods/184625/files/"
              "781833?key=WPsTiCS-cJMsRv29vXJX4g&expires=1785695383&user_id=44196692",
              "skyrimspecialedition", 184625, 781833, "WPsTiCS-cJMsRv29vXJX4g",
              1785695383);
...
TEST_CASE("modl router parse", "[engine]") {
  ...
  expect_modl("modl://falloutnv/?url=https%3A%2F%2Fcdn.mod.pub%2Ffile.7z", "falloutnv",
              "https://cdn.mod.pub/file.7z");
  expect_modl("modl://skyrimse/?url=https%3A%2F%2Fexample.com%2Fpath%2FMod%20Name.7z",
              "skyrimse", "https://example.com/path/Mod Name.7z");

  // MO2 alias normalization (case-insensitive on input).
  expect_modl("modl://FalloutNV/?url=https%3A%2F%2Fa.b%2Fc", "falloutnv",
              "https://a.b/c");
  expect_modl("modl://SkyrimSpecialEdition/?url=https%3A%2F%2Fa.b%2Fc", "skyrimse",
              "https://a.b/c");
  expect_modl("modl://newvegas/?url=https%3A%2F%2Fa.b%2Fc", "falloutnv",
              "https://a.b/c");
  expect_modl("modl://other/?url=https%3A%2F%2Fa.b%2Fc", "other", "https://a.b/c");
  ...
  expect_modl("modl://starfield/?url=https%3A%2F%2Fa.b%2Fc", "starfield",
              "https://a.b/c");
  ...
  const auto empty_host =
      engine::Source::Router::parse_modl("modl:///?url=https%3A%2F%2Fa.b%2Fc");
  ...
  expect_modl("modl://falloutnv/?url=https%3A%2F%2Fa.b%2Fpath%23frag", "falloutnv",
              "https://a.b/path");
  ...
  const std::string too_long_url =
      "modl://falloutnv/?url=https%3A%2F%2Fa.b%2F" + std::string(15000, 'a');
  ...
  const auto wrong_scheme =
      engine::Source::Router::parse_modl("modl://falloutnv/?url=http%3A%2F%2Fa.b%2Fc");
  REQUIRE_FALSE(wrong_scheme.valid());

  const auto no_scheme =
      engine::Source::Router::parse_modl("modl://falloutnv/?url=javascript%3Aalert(1)");
  ...
  const auto unknown =
      engine::Source::Router::parse_modl("modl://NotAGame/?url=https%3A%2F%2Fa.b%2Fc");
  REQUIRE(unknown.valid());
  REQUIRE(unknown.game_id == "notagame");
  REQUIRE(unknown.direct_url == "https://a.b/c");
}

TEST_CASE("Router::derive_source_from_direct_url - modl transport source attribution",
          "[engine]") {
  ...
  }
  {
    ...
    const auto d =
        Router::derive_source_from_direct_url("https://www.mod.pub/skyrim-se/22.zip");
    REQUIRE(d.source_type == "modpub");
  }
  {
    // Case-insensitive host match: HTTPS://MOD.PUB/...
    const auto d =
        Router::derive_source_from_direct_url("HTTPS://MOD.PUB/skyrim-se/22.zip");
    REQUIRE(d.source_type == "modpub");
  }
  {
  ...
  }
  {
    ...
    const auto d =
        Router::derive_source_from_direct_url("https://mod.pub:443/skyrim-se/22.zip");
    REQUIRE(d.source_type == "modpub");
    const auto d8443 =
        Router::derive_source_from_direct_url("https://mod.pub:8443/skyrim-se/22.zip");
    REQUIRE(d8443.source_type == "modpub");
  }
  {
    // mod.pub.evil.com MUST NOT match (subdomain-attack guard).
    const auto d =
        Router::derive_source_from_direct_url("https://mod.pub.evil.com/x.zip");
    REQUIRE(d.source_type.empty());
  }
  {
    // evil.mod.pub MUST NOT match either.
    const auto d = Router::derive_source_from_direct_url("https://evil.mod.pub/x.zip");
    REQUIRE(d.source_type.empty());
  }
  {
    // Generic host -> no source_type (manual fallback).
    const auto d =
        Router::derive_source_from_direct_url("https://example.com/path/Mod.7z");
    REQUIRE(d.source_type.empty());
  ...
  }
  {
    ...
    const auto d =
        Router::derive_source_from_direct_url("https://example.com:8443/path/Mod.7z");
    REQUIRE(d.source_type.empty());
  }
  {
    ...
    const auto d =
        Router::derive_source_from_direct_url("https://cdn.example.org/files/22.zip");
    REQUIRE(d.source_type.empty());
  }
  {
