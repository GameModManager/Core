...
TEST_CASE("mod state - hidden and disabled", "[engine]") {
  ...
  require(wh.hidden_files[0] == "meshes/secret.nif", "first hidden file correct");
  require(wh.hidden_files[1] == "textures/hidden.dds", "second hidden file correct");

  // Toggle off.
  tracker.set_hidden("mod_hidden", false);
  require(tracker.entry("mod_hidden").hidden == false, "hidden toggled off");
...
TEST_CASE("mod state - deploy at root", "[engine]") {
  ...
  require(tracker.entry("mod_a").deploy_at_root == true, "deploy_at_root set to true");

  // Another mod stays false.
  tracker.set_position("mod_b", 1);
  require(tracker.entry("mod_b").deploy_at_root == false, "other mod unaffected");

  // Clear flag.
  tracker.set_deploy_at_root("mod_a", false);
  require(tracker.entry("mod_a").deploy_at_root == false, "deploy_at_root cleared");
...
TEST_CASE("mod state - comprehensive roundtrip", "[engine]") {
  ...
  // Verify file exists.
  require(fs::exists(root / "mod_state.json"), "mod_state.json created");
...
TEST_CASE("mod state - hidden_files edge cases", "[engine]") {
  ...
  require(tracker.entry("mod_a").hidden_files.empty(), "empty hidden_files vector");
  ...
  require(reload.load(), "reload succeeds");
  auto &ra = reload.entry("mod_a");
  require(ra.hidden_files.empty(), "empty hidden_files stays empty");
