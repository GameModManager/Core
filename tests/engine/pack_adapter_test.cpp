...
public:
  ...
  [[nodiscard]] bool can_handle_source(const std::string &url_or_path) const override {
    return engine::Pack::detect_source_kind(url_or_path) == kind_;
  }

  [[nodiscard]] engine::Pack::Outcome<engine::Pack::PackManifest>
  resolve_pack(const std::string &url_or_path) override {
    ...
    choices_     = {{"c1",
                     ...
                     "2K"}};
    resolved_    = true;
    return engine::Pack::Outcome<engine::Pack::PackManifest>::success(manifest_);
  }

  [[nodiscard]] engine::Pack::Outcome<engine::Pack::ResolvedMod>
  resolve_mod(const engine::Pack::ModEntry &entry) override {
    if (entry.id.empty()) {
  ...
  [[nodiscard]] engine::Pack::DownloadInfo
  get_download_info(const engine::Pack::ResolvedMod &resolved) const override {
  ...
  [[nodiscard]] std::vector<engine::Pack::IniEdit> get_ini_edits() const override {
    return resolved_ ? ini_edits_ : std::vector<engine::Pack::IniEdit>{};
  }
  [[nodiscard]] std::vector<engine::Pack::ExecEntry> get_executables() const override {
    return resolved_ ? executables_ : std::vector<engine::Pack::ExecEntry>{};
  }
  [[nodiscard]] std::vector<engine::Pack::ChoiceGroup>
  get_choice_groups() const override {
...
TEST_CASE("pack detect_source_kind routes files and nxm urls", "[engine]") {
  ...
  REQUIRE(engine::Pack::detect_source_kind("/packs/foo.gmmpack") == SourceKind::File);
  REQUIRE(engine::Pack::detect_source_kind("/packs/FOO.GMMpack") == SourceKind::File);
  REQUIRE(engine::Pack::detect_source_kind("/packs/plain-dir") == SourceKind::File);
...
TEST_CASE("pack outcome carries success and failure", "[engine]") {
  auto ok = engine::Pack::Outcome<engine::Pack::PackManifest>::success({"Pack"});
  REQUIRE(ok.ok);
  REQUIRE(ok.value.name == "Pack");
  REQUIRE(ok.error.empty());
  auto err = engine::Pack::Outcome<engine::Pack::PackManifest>::failure("nope");
  REQUIRE_FALSE(err.ok);
  REQUIRE(err.error == "nope");
}

TEST_CASE("pack file adapter drives the full installer contract", "[engine]") {
  ...
  auto empty = adapter.resolve_mod(engine::Pack::ModEntry{});
  ...
  REQUIRE(choices[0].mode == engine::Pack::ChoiceGroup::Mode::ExactlyOne);
...
TEST_CASE("pack nxm adapter serves api-based collections", "[engine]") {
  ...
  auto pack =
      adapter.resolve_pack("nxm://skyrimspecialedition/collections/abc/revisions/1");
  REQUIRE(pack.ok);
  REQUIRE(pack.value.mods.size() == 2);
...
TEST_CASE("pack registry routes references to adapters", "[engine]") {
  ...
  reg.register_adapter(std::make_unique<FakeAdapter>("nexus-collection",
                                                     engine::Pack::SourceKind::NxmApi));
