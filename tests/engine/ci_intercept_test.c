...
int main(void) {
  ...
  if (mkdir(meshes, 0755) != 0) {
    perror("mkdir meshes");
    return 2;
  }
  char nested_upper[4096];
  snprintf(nested_upper, sizeof nested_upper, "%s/Meshes/Behaviors", base);
  if (mkdir(nested_upper, 0755) != 0) {
    perror("mkdir nested");
    return 2;
  }
  char deployed_upper[4096];
  snprintf(deployed_upper, sizeof deployed_upper,
           "%s/Meshes/Behaviors/0_Master.hxk", base);
  write_file(deployed_upper, "X");

  char lower_nested[4096];
  snprintf(lower_nested, sizeof lower_nested,
           "%s/meshes/behaviors/0_master.hxk", base);
   ...
   * at `/` and query
   * `<base>/dosdevices/z:<base>/meshes/behaviors/0_master.hxk`. The kernel
   * follows `z:` -> `/`, so the raw path is `<base>/...` and the rest of the
   * query is the lowercase nested path -> ENOENT without the shim; with the
   * shim it must resolve through the prefix strip. */
  char dd[4096];
  snprintf(dd, sizeof dd, "%s/dosdevices", base);
  if (mkdir(dd, 0755) != 0) {
    perror("mkdir dosdevices");
    return 2;
  }
  char dd_z[4096];
  ...
  char dd_query[4096];
  snprintf(dd_query, sizeof dd_query,
           "%s/dosdevices/z:%s/meshes/behaviors/0_master.hxk", base, base);
  ...
  if (child_ok && child2_ok && child3_ok && parent_failed && dd_parent_failed &&
      mode_ok) {
    printf("ALL CHECKS PASSED\n");
    return 0;
  }
  fprintf(stderr,
          "child_ok=%d child2_ok=%d child3_ok=%d parent_failed=%d "
          "dd_parent_failed=%d mode_ok=%d\n",
          child_ok, child2_ok, child3_ok, parent_failed, dd_parent_failed,
          mode_ok);
  if (!parent_failed)
    fprintf(stderr,
            "FAIL: parent stat'ed lowercase WITHOUT shim (fs is CI?)\n");
  if (!dd_parent_failed)
    fprintf(stderr,
            "FAIL: parent stat'ed dosdevices query WITHOUT shim (fs is CI?)\n");
  return 1;
}