...
static engine::DirectDeployStrategy make_strategy(const fs::path &base,
...
TEST_CASE("direct strategy deploy_all and queries", "[engine]") {
  ...
  check(fs::read_symlink(deployed) == base / "mods" / "EnabledMod" / "RaceMenu.esp",
  ...
  check(!fs::exists(base / "game" / "Data" / "SkyUI.esp"), "disabled mod not deployed");
...
TEST_CASE("direct strategy undeploy restores originals", "[engine]") {
  ...
  check(read_file(original_target) == "original", "undeploy leaves original content");
  ...
  check(!fs::exists(base / ".gmm_deploy_ledger"), "ledger dropped after full removal");
  ...
  check(fs::is_regular_file(backup), "post-undeploy deploy re-backs-up the original");
