...
static void test_fomod_claim_via_bridge() {
  ...
  PluginLoader loader;
  check(loader.load_plugin(GMM_FOMOD_PLUGIN_PATH), "Fomod stage-claim plugin loads");
  ...
  pipeline.add_stage(
      std::make_unique<PluginClaimStage>("Fomod", claim->plugin_id, claim->handler));
  ...
  check(mod.name == "Bridge Installed Name", "wizard-edited name lands on the mod");
  ...
  check(!fs::exists(staging / "Patches" / "Lite.esp"), "unselected plugin file pruned");
  ...
  check(lines.size() == 3 &&
            lines[1].find("\"final_name\":\"Bridge Installed Name\"") !=
                std::string::npos,
        "outcome carries the final mod folder name");
  check(lines.size() == 3 && lines[1].find("\"choices\":") != std::string::npos &&
