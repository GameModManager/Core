...
Manifest manifest_with(std::vector<ModEntry> mods,
                       const std::string& id = "pack-uuid",
                       int64_t revision = 2) {
    Manifest m;
...
::engine::Mod installed_mod(const std::string& id,
...
{
    Manifest incoming =
        manifest_with({nexus_entry("a", "1.0"), nexus_entry("b", "2.0")});
...
TEST_CASE("diff ignores untracked and foreign-collection mods",
          "[collection][sync]")
{
    Manifest incoming = manifest_with({nexus_entry("a", "1.0")});
    ...
    ::engine::Mod foreign =
        installed_mod("foreign-mod", "1.0", "other-pack", 5);
...
TEST_CASE("diff update scenario: add + update + remove + keep",
          "[collection][sync]")
{
...
TEST_CASE("Resolver stamps collection tracking on resolve",
          "[collection][sync]")
{
...
TEST_CASE("ModMeta collection keys default to untracked",
          "[collection][sync]")
{
    engine::ModMeta meta;
