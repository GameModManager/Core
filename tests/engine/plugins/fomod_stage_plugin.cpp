...
static int fomod_stage_handler(GmmModHandle mod, GmmInstanceHandle instance,
                               ...
                               GmmProfileHandle profile, void *user_data) {
  (void)conflicts;
  (void)profile;
  (void)user_data;
  ...
  snprintf(line, sizeof line, "stage_claimed game=%s files=%zu root=%s", gid, n, root);
  log_line(line);
  ...
  if (strstr(json, "\"outcome\":\"not_fomod\""))
    return 1;  // pass through
  if (strstr(json, "\"outcome\":\"canceled\""))
    return 0;  // wizard cancel
  if (strstr(json, "\"outcome\":\"failed\""))
