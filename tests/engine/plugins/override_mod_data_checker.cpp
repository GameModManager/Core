...
static void on_test_event(const char *event_id, const char *json_payload,
                          void *user_data) {
  (void)user_data;
...
extern "C" void gmm_register_v1(GmmRegistrationCtx *ctx) {
  if (ctx->register_game_feature) {
    ctx->register_game_feature(
        ctx, "SkyrimSpecialEdition", /* game_id — override Skyrim's checker */
        "mod_data_checker", 100,     /* priority — above the game's own (0) */
        OVERRIDE_DIRS, sizeof(OVERRIDE_DIRS) / sizeof(OVERRIDE_DIRS[0]), OVERRIDE_EXTS,
        sizeof(OVERRIDE_EXTS) / sizeof(OVERRIDE_EXTS[0]));

    ctx->register_game_feature(
        ctx, "SkyrimSpecialEdition", /* game_id — override Skyrim's band */
        "game_plugins", 100,         /* priority — above the game's own (0) */
        OVERRIDE_PLUGINS, sizeof(OVERRIDE_PLUGINS) / sizeof(OVERRIDE_PLUGINS[0]), NULL,
        0);
  }
  if (ctx->register_game_feature_data) {
    ctx->register_game_feature_data(
        ctx, "SkyrimSpecialEdition", /* game_id — override Skyrim's SKSE */
        "script_extender", 100,      /* priority — above the game's own (0) */
        OVERRIDE_SE_KEYS, OVERRIDE_SE_VALS,
        sizeof(OVERRIDE_SE_KEYS) / sizeof(OVERRIDE_SE_KEYS[0]));

    ctx->register_game_feature_data(
        ctx, "SkyrimSpecialEdition", /* game_id — Skyrim registers none */
        "bsa_invalidation", 100,     /* priority — only registration */
        OVERRIDE_BSA_KEYS, OVERRIDE_BSA_VALS,
