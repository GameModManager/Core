...
static const std::string kElf     = "\x7f"
                                    "ELF\x02\x01\x01\x00";
...
TEST_CASE("deploy binary", "[engine]") {
  ...
  check(engine::is_executable_binary(tmp / "B.ELF"),
        ".ELF is a binary (case-insensitive)");
  ...
  check(engine::is_executable_binary(tmp / "e"),
        "extensionless shebang script is a binary");
  check(!engine::is_executable_binary(tmp / "f"),
        "extensionless plain data is not a binary");
  ...
  check(!engine::is_executable_binary(tmp / "i.exe.notes"),
        "fake .exe extension look-alike is not a binary");
  ...
  const bool ok = engine::deploy_all_enabled_mods(base / "mods", staging, "Data",
  ...
  const bool ok2 = engine::deploy_all_enabled_mods(base / "mods", staging, "Data",
                                                   /*deploy_include_mod_id=*/false, "");
  ...
  check(fs::is_regular_file(game / "launcher.sh") &&
            !fs::is_symlink(game / "launcher.sh"),
        "SymlinkStrategy .sh is a real file");
  check(fs::is_symlink(game / "notes.txt"), "SymlinkStrategy .txt stays a symlink");
