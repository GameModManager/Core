...
TEST_CASE("Resolver before rule emits warning for unknown mod",
          "[collection][resolver]") {
  Manifest m;
...
TEST_CASE("Resolver mixed providers", "[collection][resolver]") {
  ...
  direct_mod.phase = 0;
  SourceDirect ds;
  ds.url            = "https://example.com/dl.zip";
  direct_mod.source = ds;
  m.mods.push_back(direct_mod);

  ModEntry steam_mod;
  steam_mod.id    = "steam-mod";
  steam_mod.phase = 0;
  SourceSteamWorkshop ss;
  ss.workshop_item_id = 999;
  steam_mod.source    = ss;
