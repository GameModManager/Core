...
static std::string write_tmp_ini_dir() {
  static int n = 0;
  fs::path dir = fs::temp_directory_path() / ("gmm_ini_test_" + std::to_string(++n));
...
static IniEditFile make_file(const std::string &target, std::vector<IniTweak> tweaks) {
...
TEST_CASE("parse_ini_content handles sections, comments, blanks", "[ini]") {
  const std::string content = "; top comment\n"
                              "# hash comment\n"
                              "\n"
                              "[Display]\n"
...
TEST_CASE("parse_ini_content duplicate key last-wins with warning", "[ini]") {
  std::string err;
  std::vector<std::string> warnings;
  auto edits = parse_ini_content("[S]\nk=first\nk=second\n", &err, &warnings);
  REQUIRE(edits.has_value());
...
TEST_CASE("merge groups case-insensitive targets", "[ini]") {
  auto a = make_file("Skyrim.ini", {make_tweak("t1", "[Display]\niSize W=2560\n")});
  auto b = make_file("skyrim.INI",
                     {make_tweak("t2", "[General]\nsLanguage=ENGLISH\n",
                                 TweakStatus::Recommended, true, std::string("M"))});
...
TEST_CASE("merge: pack-author beats mod-attributed, last wins ties", "[ini]") {
  auto a = make_file("x.ini",
                     {make_tweak("mod-k", "[S]\nk=mod-val\n", TweakStatus::Recommended,
                                 true, std::string("SomeMod")),
                      make_tweak("j-first", "[S]\nj=first\n", TweakStatus::Recommended,
                                 true, std::string("A")),
                      make_tweak("j-second", "[S]\nj=second\n",
                                 TweakStatus::Recommended, true, std::string("B"))});
  auto b = make_file("x.ini", {make_tweak("author-k", "[S]\nk=author-val\n",
...
TEST_CASE("merge: disabled tweaks are filtered", "[ini]") {
  auto f      = make_file("x.ini", {make_tweak("on", "[S]\nk=kept\n"),
                                    make_tweak("off", "[S]\nj=dropped\n",
...
TEST_CASE("apply updates value preserving layout and comments", "[ini]") {
  const std::string ini = "; top comment\n"
                          "\n"
                          "[Display]\n"
                          "iSize W=1920 ; screen width\n"
                          "bFull Screen=1\n"
...
TEST_CASE("apply appends missing key and section", "[ini]") {
  ...
  auto f =
      make_file("x.ini", {make_tweak("t1", "[Display]\nbBorderless=1\n"
...
TEST_CASE("apply flags user-modified keys instead of overwriting", "[ini]") {
  ...
  auto merged           = single_target(
      "x.ini", {{{"Display", "iSize W", "2560", std::nullopt, std::nullopt}}});
  ...
  auto merged2 = single_target(
      "x.ini", {{{"Display", "iSize W", "3440", std::nullopt, std::nullopt}}});
...
TEST_CASE("apply keeps original prior across re-applies", "[ini]") {
  ...
  auto m1 =
      single_target("x.ini", {{{"S", "k", "v1", std::nullopt, std::string("t1")}}});
  auto m2 =
      single_target("x.ini", {{{"S", "k", "v2", std::nullopt, std::string("t1")}}});
...
TEST_CASE("retract restores prior value and removes added keys", "[ini]") {
  ...
  auto merged           = single_target(
      "x.ini", {{{"S", "k", "v1", std::string("ModA"), std::string("t1")},
                 {"S", "added", "yes", std::string("ModA"), std::string("t1")},
                 {"S", "other", "kept", std::string("ModB"), std::string("t2")}}});
  auto applied = apply_ini_edits(ini, merged);
  auto ret     = retract_ini_edits(applied.text, applied.applied, std::string("ModA"));
  CHECK(ret.text.find("k=orig\n") != std::string::npos);
...
TEST_CASE("retract leaves user-changed values and reports them", "[ini]") {
  const std::string ini = "[S]\nk=orig\n";
  auto merged =
      single_target("x.ini", {{{"S", "k", "v1", std::string("M"), std::string("t1")}}});
  auto applied        = apply_ini_edits(ini, merged);
  std::string changed = applied.text;
  changed.replace(changed.find("v1"), 2, "zz");
  auto ret = retract_ini_edits(changed, applied.applied, std::string("M"));
...
TEST_CASE("retract_ini_tweak leaves user-changed values and reports them", "[ini]") {
  const std::string ini = "[S]\nk=orig\n";
  auto f                = make_file("x.ini", {make_tweak("t1", "[S]\nk=v1\n")});
  auto merged           = merge_ini_edits({f});
  auto applied          = apply_ini_edits(ini, merged[0]);
...
TEST_CASE("applied state JSON round-trips with tweak ids", "[ini]") {
  std::vector<AppliedEdit> state = {
      {"Skyrim.ini", "Display", "iSize W", true, "1920", "2560", std::nullopt,
       std::string("aniso")},
...
TEST_CASE("applied state JSON loads pre-tweak files without tweakId", "[ini]") {
...
TEST_CASE("CRLF documents keep CRLF", "[ini]") {
  const std::string ini = "[S]\r\nk=orig\r\n";
  auto merged =
      single_target("x.ini", {{{"S", "k", "new", std::nullopt, std::string("t1")}}});
