...
static void deploy_and_merge(const fs::path &base, bool case_sensitive,
  ...
  const bool ok          = engine::deploy_all_enabled_mods(
      mods, staging, "Data", /*deploy_include_mod_id=*/false, "", case_sensitive);
  ...
  if (case_sensitive) {
    ...
    check(!fs::is_symlink(staging / "data") && !fs::exists(staging / "data"),
    ...
    const fs::path upper_alias =
        real_upper ? staging / "Data" / "MESHES" : staging / "Data" / "MESHES";
    check(fs::is_symlink(upper_alias),
          (l + ": UPPERCASE alias exists for the merged dir").c_str());
    check(fs::exists(upper_alias / "a.nif") && fs::exists(upper_alias / "b.nif") &&
