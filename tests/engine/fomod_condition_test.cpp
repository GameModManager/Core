...
std::unique_ptr<ModuleConfiguration> parse_config(const std::string &xml) {
  const auto path =
      std::filesystem::temp_directory_path() / "gmm_fomod_condition_test.xml";
...
std::shared_ptr<GroupViewModel> first_group(const std::shared_ptr<FomodViewModel> &vm,
                                            int stepIndex) {
...
void test_most_recent_flag_wins() {
  ...
  REQUIRE(follower->getCurrentPluginType() ==
          PluginTypeEnum::Optional);  // only "early" is set
...
void test_first_match_pattern_wins() {
  ...
  REQUIRE(dual->getCurrentPluginType() ==
          PluginTypeEnum::Recommended);  // first pattern still wins

  vm->togglePlugin(group, setA1, false);
  REQUIRE(dual->getCurrentPluginType() ==
          PluginTypeEnum::Required);  // only second pattern matches
