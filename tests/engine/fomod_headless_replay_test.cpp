...
std::unique_ptr<ModuleConfiguration> parse_config(const std::string &xml) {
  const auto path =
      std::filesystem::temp_directory_path() / "gmm_headless_replay_test.xml";
...
std::shared_ptr<PluginViewModel> find_plugin(const std::shared_ptr<FomodViewModel> &vm,
...
TEST_CASE("Headless replay with empty selections preserves defaults",
          "[fomod][headless]") {
  auto vm = make_view_model(kTwoStepXml);
...
TEST_CASE("Headless replay on stepless FOMOD returns empty diagnostics",
          "[fomod][headless]") {
...
TEST_CASE("Headless replay partial match reports only mismatches",
          "[fomod][headless]") {
  auto vm = make_view_model(kTwoStepXml);
...
TEST_CASE("Choice groups exactly-one happy path", "[fomod][headless][choices]") {
  const std::vector<Collection::ChoiceGroup> groups = {
      make_group("texture-pack", Collection::ChoiceMode::ExactlyOne,
                 {"smi", "sky202x"}),
  };
...
TEST_CASE("Choice groups exactly-one empty pick is a violation",
...
TEST_CASE("Choice groups exactly-one double pick keeps first",
          "[fomod][headless][choices]") {
  const std::vector<Collection::ChoiceGroup> groups = {
      make_group("texture-pack", Collection::ChoiceMode::ExactlyOne,
                 {"smi", "sky202x"}),
  ...
  const auto result =
      resolve_choice_groups(groups, {{"texture-pack", {"smi", "sky202x"}}});
...
TEST_CASE("Choice groups report unknown group and member",
          "[fomod][headless][choices]") {
  const std::vector<Collection::ChoiceGroup> groups = {
      make_group("texture-pack", Collection::ChoiceMode::ExactlyOne,
                 {"smi", "sky202x"}),
