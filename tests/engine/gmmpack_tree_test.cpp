...
static gmmpack::SeparatorNode make_sep(
    const std::string& name, std::vector<gmmpack::TreeNode> children = {},
...
TEST_CASE("parse applies enabled/collapsed defaults", "[tree_parser]") {
    ...
    REQUIRE(mod->enabled);
    auto* sep =
        std::get_if<gmmpack::SeparatorNode>(&tree.nodes[1].data);
    REQUIRE(sep != nullptr);
    REQUIRE_FALSE(sep->collapsed);
}
...
TEST_CASE("serialize nested separator structure", "[tree_parser]") {
    ...
    tree.nodes.push_back(sep(
        "Outer", {mod("x"), sep("Inner", {mod("y")})}, true));
    auto j = gmmpack::serialize_tree(tree);
...
TEST_CASE("round-trip: serialize then parse produces same tree",
          "[tree_parser]") {
    gmmpack::TreeRoot original;
    original.nodes.push_back(sep(
        "Group",
        {mod("a"), sep("Inner", {mod("b"), mod("c", false)}), mod("d")},
        true));
...
TEST_CASE("validate valid tree passes", "[tree_parser]") {
    gmmpack::TreeRoot tree;
    tree.nodes.push_back(
        sep("Core", {mod("skyui"), sep("ENB", {mod("enb-preset")})}));
