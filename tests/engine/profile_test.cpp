...
TEST_CASE("settings.ini preserves unknown keys and sections", "[engine]") {
  ...
  write_text(dir / "settings.ini", "LocalSaves=false\n"
                                   "CustomRootKey=hello\n"
                                   "[Game]\n"
                                   "bSomething=1\n");
  engine::profile::ProfileManager profile(dir);
...
TEST_CASE("repair creates all missing required files with defaults", "[engine]") {
  ...
  REQUIRE(std::find(generated.begin(), generated.end(), "settings.ini") !=
          generated.end());
  REQUIRE(std::find(generated.begin(), generated.end(), "modlist.txt") !=
          generated.end());
  REQUIRE(std::find(generated.begin(), generated.end(), "archives.txt") !=
          generated.end());
...
TEST_CASE("repair is a no-op when all required files exist", "[engine]") {
  ...
  REQUIRE(read_text(dir / "archives.txt").find("Skyrim - Textures.bsa") !=
          std::string::npos);
}
...
TEST_CASE("modlist.txt parses enabled/disabled/foreign", "[engine]") {
  ...
  write_text(dir / "modlist.txt", "# comment\r\n"
                                  "+ModA\r\n"
                                  "-ModB\r\n"
                                  "*DLC1\r\n"
                                  "\r\n"
...
TEST_CASE("DelayedFileWriter debounces into a single write", "[engine]") {
  ...
  {
    engine::profile::DelayedFileWriter writer(
        [&] {
          calls.fetch_add(1);
        },
        40ms);
...
TEST_CASE("plugins/loadorder/lockedorder/archives round-trip", "[engine]") {
  ...
  REQUIRE(profile.read_plugins() ==
          std::vector<std::string>({"Skyrim.esm", "Update.esm"}));

  REQUIRE(profile.write_load_order({"Update.esm", "Skyrim.esm"}));
  REQUIRE(profile.read_load_order() ==
          std::vector<std::string>({"Update.esm", "Skyrim.esm"}));
  ...
  REQUIRE(profile.read_archives() ==
          std::vector<std::string>({"Skyrim - Textures.bsa"}));
}

TEST_CASE("lockedorder skips malformed lines", "[engine]") {
  auto dir = make_temp_dir("locked_malformed");
  write_text(dir / "lockedorder.txt",
...
TEST_CASE("remove cancels a pending modlist write", "[engine]") {
  ...
  {
    engine::profile::ProfileManager profile(dir,
                                            10s);  // long debounce: nothing flushes
    profile.refresh_mod_status({"ModA"});
    profile.set_mod_enabled("ModA", false);  // schedules a delayed write
    REQUIRE(profile.remove() == engine::profile::ProfileRemoveResult::Removed);
