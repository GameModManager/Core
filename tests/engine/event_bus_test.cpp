...
TEST_CASE("event bus", "[engine]") {
  ...
  // --- unsubscribe stops delivery; token is idempotent ---
  {
    int hits = 0;
    uint64_t tok =
        bus.subscribe(events::kModMoved, [&](const std::string &, const std::string &) {
          ++hits;
        });
  ...
  {
    int nested = 0;
    bus.subscribe(events::kModRemoved, [&](const std::string &, const std::string &) {
      ++nested;
    });
    uint64_t self_tok = 0;
    self_tok          = bus.subscribe(
        events::kModStateChanged, [&](const std::string &, const std::string &) {
