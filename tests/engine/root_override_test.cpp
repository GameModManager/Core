...
TEST_CASE("root override", "[engine]") {
  ...
  auto flat = classify_registry_path("meshes/test.nif",
                                     Owners{{"ModA", 1}, {"ModB", 2}}, no_root, "Data");
  check(flat.space == DeploySpace::Data, "no root owner -> Data space");
  check(flat.display_path == "meshes/test.nif", "no root owner -> path unchanged");

  // Root-flagged owner + Data/ prefix -> Data space, prefix stripped.
  auto data = classify_registry_path("Data/SKSE/Plugins/foo.dll", Owners{{"SKSE", 5}},
                                     with_root, "Data");
  check(data.space == DeploySpace::Data, "root owner + Data/ prefix -> Data space");
  ...
  auto root = classify_registry_path("skse64_loader.exe", Owners{{"SKSE", 5}},
                                     with_root, "Data");
  check(root.space == DeploySpace::Root, "root owner, no prefix -> Root space");
  ...
  check(exact.display_path.empty(), "root owner, exact prefix -> empty display path");

  // Case-insensitive prefix segment (Windows game dirs).
  auto ci = classify_registry_path("data/skse64_loader.exe", Owners{{"SKSE", 5}},
                                   with_root, "Data");
  ...
  auto lookalike =
      classify_registry_path("Datax/tool.dll", Owners{{"SKSE", 5}}, with_root, "Data");
  ...
  check(mixed.space == DeploySpace::Root, "any root owner -> Root space");
  ...
  auto no_prefix =
      classify_registry_path("resources/gfx/a.png", Owners{{"SKSE", 5}}, with_root, "");
  check(no_prefix.space == DeploySpace::Root,
        "empty deploy_prefix -> Root space for a root owner");
  auto no_prefix_flat =
      classify_registry_path("resources/gfx/a.png", Owners{{"ModA", 1}}, no_root, "");
