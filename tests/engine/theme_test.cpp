...
TEST_CASE("theme", "[engine]") {
  ...
  write(root / "app" / "themes" / "Dark" / "theme.json",
        "{ \"base_style\": \"Fusion\" }");
  write(root / "app" / "themes" / "Nord" / "nord.qss", "QWidget { color: $fg; }");
  write(root / "app" / "themes" / "Shared" / "shared.qss", "FROM_APP");
  write(root / "themes" / "Shared" / "shared.qss", "FROM_SUBMODULE");
  ...
  {
    write(root / "app" / "themes" / "Shared" / "theme.json", "{ \"author\": \"gmm\" }");
