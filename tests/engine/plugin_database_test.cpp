...
void run_synthetic_fixture() {
  ...
  // --- Apply a LOOT-style sorted order (PLAN.md §7.1) ---
  {
    ...
    const std::vector<std::string> loot_order = {"Lights.esl", "Patch.esp",
                                                 "SkyUI_SE.esp"};
  ...
  std::string err3;
  require(!db3.set_enabled("SkyUI_SE.esp", false, &err3),
          "SkyUI disable blocked by Patch");
  ...
  // profile round-trip (MO2 lockedorder.txt parity).
  {
    ...
    PluginDatabase dbl;
    require(dbl.refresh(g, m, "", "Skyrim.esm,Update.esm"), "refresh lock fixture");
    dbl.sort_load_order();
    dbl.set_all_enabled();
    ...
    std::string errl;
    require(!dbl.set_locked("Skyrim.esm", true, &errl), "core plugin cannot be locked");
    ...
    require(!dbl.move_plugin(a_row, b_row, &errl), "cannot drop onto a locked row");
    ...
    PluginDatabase dbm;
    require(dbm.refresh(g, m, "", "Skyrim.esm,Update.esm"), "refresh lock handwrite");
    dbm.sort_load_order();
    ...
    require(!dbl.plugins()[order_of(dbl, "ModB.esp")].locked, "locked flag cleared");
    dbl.save_profile(pf, "Default");
    require(!contains(file_contents(pf / "Default" / "lockedorder.txt"), "ModB.esp|"),
  ...
  require(PluginDatabase::write_plugins_txt_for_launch(game, inst_root, "skyrim",
                                                       489830, knowledge, &platform),
  ...
  require(!PluginDatabase::write_plugins_txt_for_launch(game, inst_root, "isaac",
                                                        250900, no_plugins, &platform),
  ...
  // absent or was disabled by the user.
  {
    ...
    write_esp(m / "NoMaster" / "NoMaster.esp", false,
              {"Skyrim.esm", "AbsentMaster.esm"});
  ...
  // for the tooltip (MO2 addInformation parity).
  {
    ...
    require(dxps[0].messages.size() == 2 && dxps[0].messages[0] == "hello-alpha" &&
                dxps[0].messages[1] == "bye-alpha",
            "provider messages parsed into GamePlugin::messages");
    require(dxps[1].messages.size() == 1 && dxps[1].messages[0] == "hello-beta",
  ...
  // not a duplicate native.
  {
    ...
    PluginDatabase dsh;
    require(dsh.refresh(g, m, "", "Skyrim.esm"), "refresh shadow fixture");
    dsh.sort_load_order();
    require(dsh.plugins().size() == 1,
            "re-cased mod copy shadows the game Data plugin (one row)");
    require(order_of(dsh, "Skyrim.esm") == 0, "mod copy wins the name conflict");
...
void run_disabled_mod_fixture() {
  ...
  {
    PluginDatabase db;
    require(db.refresh(game, mods, "", "Skyrim.esm"), "refresh with empty mechanism");
  ...
  // Correct: the sentinel is honored -> disabled mod contributes nothing.
  {
    ...
    require(db.find("Disabled.esp") == nullptr, "disabled mod's plugin is excluded");
    require(db.find("Enabled.esp") != nullptr, "enabled mod present");
  }
...
// MO2 hover-parity data fixture (Workspace-stpm, Phase A): master_unset
// semantics (enabled-only, absent OR present-but-disabled), skip-disabled
// mod indexes, whole-VFS archive scope, and LOOT report attach/clear.
void run_hover_parity_fixture() {
  using engine::PluginDatabase;

  const fs::path base = "/tmp/gmm_plugin_hover_fixture";
  std::error_code ec;
  fs::remove_all(base, ec);
  const fs::path game     = base / "game";
  const fs::path mods     = base / "mods";
  ...
  write_esp(game / "Data" / "Native.esm", true, {});
  // Alpha carries a folder-scoped INI; its texture archive lives in a
  // DIFFERENT mod folder (whole-VFS basename-prefix scope, MO2 parity).
  fs::create_directories(mods / "Alpha", ec);
  write_esp(mods / "Alpha" / "Alpha.esp", false, {"Native.esm"});
  {
    std::ofstream ini(mods / "Alpha" / "Alpha.ini");
    ini << "[General]\n";
  }
  fs::create_directories(mods / "Tex", ec);
  {
    std::ofstream bsa(mods / "Tex" / "Alpha - Textures.bsa", std::ios::binary);
    bsa.write("BSA", 3);
    std::ofstream other(mods / "Tex" / "Unrelated.bsa", std::ios::binary);
    other.write("BSA", 3);
  }
  // Beta depends on Alpha (present-but-disabled once the profile below loads).
  fs::create_directories(mods / "Beta", ec);
  write_esp(mods / "Beta" / "Beta.esp", false, {"Native.esm", "Alpha.esp"});

  PluginDatabase db;
  require(db.refresh(game, mods, "", "Native.esm"), "hover refresh");
  db.sort_load_order();
  db.set_all_enabled();
  ...
  require(!beta->has_ini, "no INI without a same-folder file");
  {
    ...
    require(std::find(archives.begin(), archives.end(), "Unrelated.bsa") ==
                archives.end(),
            "non-matching archive ignored");
  }

  // Profile starring only Beta: Alpha stays disabled while Beta loads, so
  // Beta's master Alpha is present-but-disabled (MO2 testMasters parity).
  fs::create_directories(profiles / "BetaOnly", ec);
  {
    std::ofstream plugins_txt(profiles / "BetaOnly" / "plugins.txt");
    plugins_txt << "# test profile\n*Beta.esp\nAlpha.esp\n";
  }
  require(db.load_profile(profiles, "BetaOnly"), "BetaOnly profile loads");
  db.set_missing_masters();
  db.generate_mod_indexes();

  alpha = db.find("Alpha.esp");
  beta  = db.find("Beta.esp");
  require(!alpha->enabled && beta->enabled, "profile enable states applied");
  require(alpha->master_unset.empty(), "disabled plugin never flagged");
  require(beta->master_unset.size() == 1 && beta->master_unset[0] == "Alpha.esp",
          "present-but-disabled master is unset");
  require(alpha->mod_index_text.empty(), "disabled plugin has no mod index");
  require(beta->mod_index_text == "01", "Beta counts only enabled predecessors");

  // LOOT reports attach case-insensitively and clear cleanly.
  {
    engine::LootReport report;
    report.messages.push_back({"warning", "A <b>warning</b>"});
    report.missing_masters.push_back("GoneMaster.esm");
    db.set_loot_reports({{"beta.esp", report}});
    beta  = db.find("Beta.esp");
    alpha = db.find("Alpha.esp");
    require(beta->loot_report.messages.size() == 1 &&
                beta->loot_report.messages[0].level == "warning",
            "LOOT report attached by case-insensitive name");
