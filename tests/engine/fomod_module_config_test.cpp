...
TEST_CASE("fomod module config", "[engine]") {
  ...
  for (const auto &fix : kFixtures) {
    ...
    if (fix.step_count >= 0) {
      REQUIRE(static_cast<int>(config.installSteps.installSteps.size()) ==
              fix.step_count);
    }
  ...
  // and the TK Dodge typeDescriptor (file dependencies, order, priority).
  {
    ...
    const auto &tkDodge = config.installSteps.installSteps[0]
                              ...
                              .plugins.plugins[0];
    REQUIRE(tkDodge.typeDescriptor.dependencyType.defaultType ==
    ...
    REQUIRE(pattern.dependencies.fileDependencies[0].state ==
            FileDependencyTypeEnum::Active);
    REQUIRE(pattern.dependencies.fileDependencies[1].file == "TKDodge.esp");
    REQUIRE(pattern.dependencies.fileDependencies[1].state ==
            FileDependencyTypeEnum::Inactive);
    REQUIRE(pattern.dependencies.fileDependencies[2].file == "UltimateCombat.esp");
    REQUIRE(pattern.dependencies.fileDependencies[2].state ==
            FileDependencyTypeEnum::Active);
    REQUIRE(pattern.dependencies.fileDependencies[3].file == "UltimateCombat.esp");
    REQUIRE(pattern.dependencies.fileDependencies[3].state ==
            FileDependencyTypeEnum::Inactive);
  ...
  // Base Object Swapper (test_moduleconf_bos.cpp): version dependency.
  {
    ...
    REQUIRE(firstPlugin.typeDescriptor.dependencyType.defaultType ==
            PluginTypeEnum::Optional);
    ...
    REQUIRE(patterns.front().dependencies.gameDependencies.front().version ==
            "1.6.1130.0");
