...
void write_test_zip(const std::filesystem::path& path,
    ...
    const std::uint32_t cd_size =
        static_cast<std::uint32_t>(out.size()) - cd_offset;
...
struct TempFile {
    ...
    explicit TempFile(const std::string& suffix)
        : path(std::filesystem::temp_directory_path() /
               ("gmm_pack_detect_" + std::to_string(g_temp_counter++) +
                suffix)) {
        std::error_code ec;
...
TEST_CASE("pack source ids stay in sync with the adapter interface",
          "[engine]") {
...
TEST_CASE("pack source detects .gmmpack extension", "[engine]") {
    ...
    expect_format("https://cdn.example.com/packs/a.gmmpack?dl=1",
                  PackFormat::Gmmpack, "gmmpack");
    expect_format("https://cdn.example.com/packs/a.gmmpack#frag",
                  PackFormat::Gmmpack, "gmmpack");
    // Missing files are trusted - the reference may be validated before
    // anything is downloaded.
    const std::string missing =
        (std::filesystem::temp_directory_path() / "gmm_no_such_pack.gmmpack")
            .string();
...
TEST_CASE("pack source detects nexusmods collection page URLs", "[engine]") {
    expect_format(
        "https://www.nexusmods.com/skyrimspecialedition/collections/5777",
        PackFormat::NexusCollection, "nexus-collection");
    ...
    expect_format(
        ...
        PackFormat::NexusCollection, "nexus-collection");
    expect_format(
        "http://www.nexusmods.com/fallout4/collections/123#revisions",
        PackFormat::NexusCollection, "nexus-collection");
    // Nexus pages that are NOT collections are not packs.
    expect_format("https://www.nexusmods.com/skyrimspecialedition/mods/184625",
                  PackFormat::Unknown, "");
    expect_format("https://www.nexusmods.com/skyrimspecialedition",
                  PackFormat::Unknown, "");
    expect_format("https://www.nexusmods.com/", PackFormat::Unknown, "");
    // Lookalike hosts must not match.
    expect_format("https://evilinexusmods.com/g/collections/1",
                  PackFormat::Unknown, "");
    expect_format("https://nexusmods.com.evil.com/g/collections/1",
                  PackFormat::Unknown, "");
...
TEST_CASE("pack source rejects non-zip .gmmpack files", "[engine]") {
    ...
    write_test_zip(real.path, {{"manifest.json", "{}"},
                               {"mods/skyui.json", "{}"}});
...
TEST_CASE("pack source sniffs misnamed pack zips", "[engine]") {
    ...
    write_test_zip(renamed.path, {{"manifest.json", "{}"},
                                  {"tree.json", "{}"},
                                  {"mods/skyui.json", "{}"}});
    ...
    REQUIRE(detect_pack_source(garbage.path.string()).format ==
            PackFormat::Unknown);
    TempFile empty(".bin");
    write_text(empty.path, "");
    REQUIRE(detect_pack_source(empty.path.string()).format ==
            PackFormat::Unknown);
}
...
TEST_CASE("pack source rejects junk input", "[engine]") {
    ...
    expect_format("modl://skyrimse/?url=https%3A%2F%2Fmod.pub%2Fx",
                  PackFormat::Unknown, "");  // transport, not a pack
