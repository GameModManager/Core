...
TEST_CASE("deploy hidden", "[engine]") {
  ...
  const fs::path dir =
      fs::current_path() / ("gmm_hide_utils_" + std::to_string(getpid()));
  ...
  check(fs::exists(dir / "mo2.txt"), "no-op unhide leaves the file in place");
  fs::remove_all(dir);
  ...
  const bool ok = engine::deploy_all_enabled_mods(root / "mods", staging, "Data",
                                                  /*deploy_include_mod_id=*/false, "");
  check(ok, "deploy_all_enabled_mods succeeds");

  check(fs::exists(staging / "Data" / "SkyUI.esp"), "visible file is deployed");
  ...
  const bool ok2          = engine::deploy_all_enabled_mods(
      root / "mods", staging2, "Data", /*deploy_include_mod_id=*/false, ".gmmdisabled");
