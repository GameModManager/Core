...
struct TempDir {
  ...
  TempDir() {
    ...
    root =
        fs::temp_directory_path() / ("gmm_filetree_test_" + std::to_string(::getpid()) +
...
static std::vector<ShapeEntry>
collect_shape(const std::shared_ptr<const engine::FileTree> &tree) {
  std::vector<ShapeEntry> out;
  tree->walk([&out](const std::string &prefix,
  ...
  std::sort(out.begin(), out.end(), [](const ShapeEntry &a, const ShapeEntry &b) {
    if (a.path != b.path)
...
TEST_CASE("filetree", "[engine]") {
  ...
  // --- 2. entry API over a directory tree --------------------------------
  {
    ...
    check(noext != nullptr && noext->suffix().empty(), "extension-less suffix empty");
    ...
    check(tree->find_directory("a.txt") == nullptr, "find_directory rejects files");

    // path_from with an ancestor tree
    auto top = sub->find("b.bin");
    if (top)
      check(top->path_from(sub) == "b.bin", "path_from subtree");

    // walk CONTINUE visits everything
    std::vector<std::string> visited;
    tree->walk(
        [&visited](const std::string &prefix, const FileTree::const_reference &entry) {
  ...
  // --- 4. walk STOP / SKIP ------------------------------------------------
  {
    ...
    auto tree   = FileTree::make_tree_from_directory(env.root);
    int stopped = 0;
    tree->walk([&stopped](const std::string &, const FileTree::const_reference &) {
      ++stopped;
    ...
    tree->walk(
        [&kept](const std::string &prefix, const FileTree::const_reference &entry) {
  ...
  // --- 5. shape equivalence: archive tree == folder tree -------------------
  {
    ...
    std::vector<ZipItem> items = {
        {"meshes/foo.nif", "foo"}, {"meshes/bar.nif", "bar"},
        {"textures/tex.dds", "t"}, {"SKSE/Plugins/version.dll", "v"},
        {"docs/readme.txt", "r"},  {"empty/", "", true},
        {"top.txt", "top"},
    };
    ...
    if (atree && ftree) {
      assert_same_shape(atree, ftree, "archive and folder trees have the same shape");
      check(atree->exists("empty"), "archive tree has empty dir");
      auto empty = atree->find_directory("empty");
      check(empty != nullptr && empty->empty(), "empty dir populated as empty");
    }
    std::printf("PASS: archive/folder shape equivalence\n");
  }

  // --- 6. analyze_staging_layout on trees --------------------------------
  auto run_scenario = [](const char *what, const std::vector<ZipItem> &items,
    ...
    check(av.fomod == expect_fomod && av.simple == expect_simple &&
              ...
              av.peeled_folder_hint == expect_hint && av.peel_chain == expect_chain,
          what);
  };

  run_scenario("peel a non-data wrapper", {{"Wrapper/SKSE/Plugins/x.dll", "x"}}, false,
               true, false, "Wrapper", {"Wrapper"});
  run_scenario("merge a lone Data+readme top layer",
               {{"Data/meshes/foo.nif", "foo"}, {"readme.txt", "r"}}, false, true, true,
               "", {});
  ...
  run_scenario("deep wrapper chain peels every level",
               {{"A/B/SKSE/Plugins/x.dll", "x"}}, false, true, false, "A", {"A", "B"});
  ...
  // ModDataChecker direct check (MO2 GamebryoModDataChecker port)
  {
    ...
    check(!ModDataChecker::data_looks_valid(p), "ModDataChecker rejects junk");
