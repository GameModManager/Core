...
TEST_CASE("proton tools", "[engine]") {
  ...
  // 2. Override resolvable through the platform -> override wins.
  {
    ...
    require(engine::resolve_proton_runner(req) == fs::path("/steam/Proton 10.0/proton"),
  ...
  // 5. Empty path.
  {
    engine::ProtonToolRequest req;
    req.platform = &platform;
    require(engine::run_proton_exe(req, fs::path()) == -1, "empty exe path refused");
  }
  ...
  //    The fake `proton` script exits immediately; we only assert the fork.
  {
    ...
    // 8. Same spawn with launch args (Workspace-pqkh): args ride along
    //    after the exe path. The fake runner exits immediately; assert
    //    the fork still succeeds with a non-empty args vector.
    auto pid_args = engine::run_proton_exe(req, exe_path, {"-windowed", "foo bar"});
    require(pid_args > 0, "real exe with args spawns detached child");
