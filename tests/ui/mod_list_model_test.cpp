...
TEST_CASE("mod list model", "[ui]") {
  ...
  check(row_with_id(model, "Skyrim.esm") == 0 &&
            row_with_id(model, "Update.esm") == 1 && row_with_id(model, "SkyUI") == 2 &&
  ...
  // name for name-based lookups.
  {
    ...
    if (skyui >= 0) {
      QVariant fold =
          model.data(model.index(skyui, ui::ModList::Fold), Qt::DisplayRole);
      check(fold.toString().isEmpty(), "regular mod Fold cell is empty");
      check(!model.has_content(skyui), "regular mod reports no fold content");
    }
  }
  ...
  // disk and every reopen/relaunch reset the separator states (user report).
  {
    ...
    QObject::disconnect(&model, &ui::ModList::mod_list_changed, nullptr, nullptr);
  }
  ...
  // band ends at Overwrite has nothing to hide.
  {
    ...
    const QVariant fold_open = m2.data(m2.index(0, ui::ModList::Fold), Qt::DisplayRole);
    ...
    const QVariant name_disp = m2.data(m2.index(0, ui::ModList::Name), Qt::DisplayRole);
    ...
    const QVariant align_sep =
        m2.data(m2.index(0, ui::ModList::Fold), Qt::TextAlignmentRole);
    check(align_sep.isValid() && align_sep.toInt() == static_cast<int>(Qt::AlignCenter),
          "separator Fold cell centered");
    const QVariant align_mod =
        m2.data(m2.index(1, ui::ModList::Fold), Qt::TextAlignmentRole);
    check(align_mod.isValid() && align_mod.toInt() == static_cast<int>(Qt::AlignCenter),
    ...
    check(!m3.has_content(0), "separator directly above Overwrite has no content");
  }
  ...
  check(row_with_id(model, "Skyrim.esm") == 0 && row_with_id(model, "Update.esm") == 1,
  ...
  check(model.dropMimeData(&drop, Qt::MoveAction, 0, 0, {}), "user-mod drop accepted");
  check(row_with_id(model, "Skyrim.esm") == 0 &&
            row_with_id(model, "Update.esm") == 1 && row_with_id(model, "SkyUI") == 2 &&
            row_with_id(model, "Enemy NPCs") == 3,
  ...
  QMimeData bad;
  bad.setData(QLatin1String(ui::kModListMimeType),
              QByteArrayLiteral("0"));  // Skyrim.esm
  check(!model.dropMimeData(&bad, Qt::MoveAction, 3, 0, {}),
        "native-only drop rejected");
  check(row_with_id(model, "Skyrim.esm") == 0, "band intact after rejected drop");
  ...
  // --- Separators above the game-native band (fold hides the native mods) ---
  {
    ...
    check(row_with_id(m2, "Testing_separator") == 0 &&
              row_with_id(m2, "Skyrim.esm") == 1 && row_with_id(m2, "Update.esm") == 2,
    ...
    check(m2.dropMimeData(&dmixed, Qt::MoveAction, 0, 0, {}), "mixed drag accepted");
    check(row_with_id(m2, "Testing_separator") != 0,
  ...
  // --- Center text on separators (Theme > Design, default on) ---
  {
    ...
    const QVariant on_name =
        m3.data(m3.index(sep_row, ui::ModList::Name), Qt::TextAlignmentRole);
    check(on_name.isValid() && on_name.toInt() == static_cast<int>(Qt::AlignCenter),
          "separator Name centered with the setting on");
    const QVariant on_prio =
        m3.data(m3.index(sep_row, ui::ModList::Priority), Qt::TextAlignmentRole);
    check(on_prio.isValid() && on_prio.toInt() == static_cast<int>(Qt::AlignCenter),
    ...
    const QVariant off_name =
        m3.data(m3.index(sep_row, ui::ModList::Name), Qt::TextAlignmentRole);
    check(!off_name.isValid(), "separator Name left-aligned with the setting off");
    const QVariant off_prio =
        m3.data(m3.index(sep_row, ui::ModList::Priority), Qt::TextAlignmentRole);
    check(off_prio.isValid() && off_prio.toInt() == static_cast<int>(Qt::AlignCenter),
          "separator Priority stays centered regardless");

    // Regular mods are never centered by the separator setting.
    const QVariant mod_align =
        m3.data(m3.index(mod_row, ui::ModList::Name), Qt::TextAlignmentRole);
  ...
  // --- Plugin-selected mod highlight (MO2 "mod contains selected file") ---
  {
    ...
    const QVariant mark = model.data(model.index(hl_row, ui::ModList::Name),
                                     ui::ModList::kScrollMarkRole);
    check(mark.canConvert<QColor>() && mark.value<QColor>() == hl,
          "scroll mark for highlighted mod");
    const QVariant no_mark = model.data(model.index(other_row, ui::ModList::Name),
                                        ui::ModList::kScrollMarkRole);
    ...
    const QVariant bg =
        model.data(model.index(hl_row, ui::ModList::Flags), Qt::BackgroundRole);
    ...
    const QVariant conflict_bg =
        model.data(model.index(hl_row, ui::ModList::Flags), Qt::BackgroundRole);
    check(conflict_bg.canConvert<QBrush>() && conflict_bg.value<QBrush>().color() == hl,
  ...
  // extended across multi-selection.
  {
    ...
    enemy.wins_against << QStringLiteral(
        "Update.esm");  // Enemy NPCs overwrites Update.esm
    ui::ConflictPairs skyrim;
    skyrim.loses_to << QStringLiteral(
        "Update.esm");  // Update.esm overwrites Skyrim.esm
    model.set_conflict_pairs({
    ...
    const QVariant red_mark = model.data(model.index(skyui_row, ui::ModList::Name),
                                         ui::ModList::kScrollMarkRole);
    check(red_mark.canConvert<QColor>() &&
              red_mark.value<QColor>() ==
                  Settings::instance().modlist_overwriting_loose(),
          "scroll mark: red for the mod overwriting the selection");
    const QVariant green_mark = model.data(model.index(update_row, ui::ModList::Name),
                                           ui::ModList::kScrollMarkRole);
    check(green_mark.canConvert<QColor>() &&
              green_mark.value<QColor>() ==
                  Settings::instance().modlist_overwritten_loose(),
          "scroll mark: green for the mod the selection overwrites");
    const QVariant none_mark = model.data(model.index(skyrim_row, ui::ModList::Name),
                                          ui::ModList::kScrollMarkRole);
    ...
    const QVariant bg =
        model.data(model.index(skyui_row, ui::ModList::Flags), Qt::BackgroundRole);
    check(bg.canConvert<QBrush>() &&
              bg.value<QBrush>().color() ==
                  Settings::instance().modlist_overwriting_loose(),
          "background tint shares the union color");

    // Multi-select union: partners of EITHER selected mod are marked.
    model.set_selected_mods(
        {QStringLiteral("Enemy NPCs"), QStringLiteral("Skyrim.esm")});
    ...
    const QVariant union_mark = model.data(model.index(update_row, ui::ModList::Name),
                                           ui::ModList::kScrollMarkRole);
    check(union_mark.canConvert<QColor>() &&
              union_mark.value<QColor>() ==
                  Settings::instance().modlist_overwriting_loose(),
          "red beats green when a row wins one pair and loses another");
    const QVariant skyui_union = model.data(model.index(skyui_row, ui::ModList::Name),
                                            ui::ModList::kScrollMarkRole);
    check(skyui_union.canConvert<QColor>() &&
              skyui_union.value<QColor>() ==
                  Settings::instance().modlist_overwriting_loose(),
          "union marks a partner of the other selection too");
    const QVariant sel_self = model.data(model.index(skyrim_row, ui::ModList::Name),
                                         ui::ModList::kScrollMarkRole);
    ...
    model.set_highlighted_mods({QStringLiteral("SkyUI")});
    const QVariant hl_mark = model.data(model.index(skyui_row, ui::ModList::Name),
                                        ui::ModList::kScrollMarkRole);
    ...
    model.set_selected_mods({});
    model.set_highlighted_mods({});
    const QVariant cleared = model.data(model.index(skyui_row, ui::ModList::Name),
                                        ui::ModList::kScrollMarkRole);
  ...
  // no mark, while a highlighted mod's mark is independent of it.
  {
    ...
    model.set_highlighted_mods({QStringLiteral("SkyUI")});
    const QVariant hl_mark =
        model.data(model.index(row_with_id(model, "SkyUI"), ui::ModList::Name),
                   ui::ModList::kScrollMarkRole);
    check(hl_mark.canConvert<QColor>() &&
              hl_mark.value<QColor>() == Settings::instance().modlist_contains_file(),
  ...
  // --- MO2-parity inline rename (EditRole) + separator colors ---
  {
    ...
    check(sep_row >= 0 && mod_row >= 0 && ow_row >= 0, "rows located for rename tests");
    ...
    check(!model.setData(model.index(ow_row, ui::ModList::Name), QStringLiteral("x"),
                         Qt::EditRole),
    ...
    check(!model.setData(model.index(sep_row, ui::ModList::Name), Qt::Checked,
                         Qt::CheckStateRole),
  ...
  // rename_mod_in_place: keeps the row where it is (id + name updated).
  {
    ...
    model.rename_mod_in_place(before, QStringLiteral("SkyUI2"),
                              QStringLiteral("SkyUI2"));
  ...
  // set_mod_color / clear_mod_color drive the separator BackgroundRole.
  {
    ...
    const QVariant bg =
        model.data(model.index(sep_row, ui::ModList::Name), Qt::BackgroundRole);
  ...
  {
    model.add_mod(QStringLiteral("GhostMod"), QStringLiteral("GhostMod"), QString(),
                  -1);
    ...
    QList<QIcon> icons =
        model.data(model.index(gr, ui::ModList::Flags), ui::ModList::kFlagIconsRole)
            .value<QList<QIcon>>();
    check(icons.size() == 1, "invalid-data adds one flag icon");
    const QString tip =
        model.data(model.index(gr, ui::ModList::Flags), Qt::ToolTipRole).toString();
    check(tip.contains(QStringLiteral("No valid game data")), "invalid-data tooltip");
    const QVariant fnt = model.data(model.index(gr, ui::ModList::Name), Qt::FontRole);
    check(fnt.canConvert<QFont>() && fnt.value<QFont>().italic(),
          "invalid-data name is italic");
    const QVariant fg =
        model.data(model.index(gr, ui::ModList::Name), Qt::ForegroundRole);
    ...
    icons = model.data(model.index(gr, ui::ModList::Flags), ui::ModList::kFlagIconsRole)
                .value<QList<QIcon>>();
    ...
    const QVariant fnt2 = model.data(model.index(gr, ui::ModList::Name), Qt::FontRole);
...

  {
    ...
    const char *labels[] = {
        "",          "Name",    "Conflicts",    "Flags",   "Category", "Source",
        "Source ID", "Version", "Installation", "Changed", "Priority"};
    for (int c = 0; c < 11; ++c) {
    ...
    const QVariant cat =
        model.data(model.index(tr, ui::ModList::Category), Qt::DisplayRole);
    ...
    const QVariant inst =
        model.data(model.index(tr, ui::ModList::Installation), Qt::DisplayRole);
    const QVariant chg =
        model.data(model.index(tr, ui::ModList::Changed), Qt::DisplayRole);
    ...
    const QVariant inst0 =
        model.data(model.index(tr, ui::ModList::Installation), Qt::DisplayRole);
    check(inst0.toString().isEmpty(), "zero timestamps render empty cells");
    ...
    const QVariant sid =
        model.data(model.index(tr, ui::ModList::SourceId), Qt::DisplayRole);
    ...
    const QVariant ver =
        model.data(model.index(tr, ui::ModList::Version), Qt::DisplayRole);
    check(ver.isValid() && ver.toString() == QLatin1String("1.0"),
          "Version renders after column reorder");
    const QVariant prio =
        model.data(model.index(tr, ui::ModList::Priority), Qt::DisplayRole);
    check(prio.isValid() && prio.toInt() == 7, "Priority renders after column reorder");
  }

  {
    ...
    check(hdr->count() == model.columnCount(), "header follows the model column count");
    ...
    QTimer::singleShot(100, [&]() {
      ...
      for (int i = 0; i < actions.size() && i < 3; ++i)
        menu_state.append({actions.at(i)->isEnabled(), actions.at(i)->isChecked()});
      if (actions.size() > 1)
    ...
    auto *cme =
        new QContextMenuEvent(QContextMenuEvent::Mouse, QPoint(5, 5), QPoint(5, 5));
    QCoreApplication::postEvent(hdr->viewport(), cme);
    QApplication::processEvents();
    check(menu_state.size() == 3, "context menu offers one action per column");
    check(menu_state.size() == 3 && !menu_state.at(0).first && menu_state.at(0).second,
    ...
    check(hdr->isSectionHidden(1), "user toggle hides the unlocked section");
    check(!hdr->isSectionHidden(0), "locked section stays visible");
  ...
  // children (parent_id + indentation + a fold arrow that hides the subtree).
  {
    ...
    auto rid = [&](const char *id) {
      return row_with_id(m, id);
    };
    ...
    check(m.data(m.index(rid("ModA"), ui::ModList::Fold), Qt::DisplayRole).toString() ==
              QStringLiteral("\u25BC"),
  ...
  // the children in reverse drop order).
  {
    ...
    auto srid = [&](const char *id) {
      return row_with_id(s, id);
    };

    QMimeData sd1;
    sd1.setData(QLatin1String(ui::kModListMimeType), QByteArrayLiteral("1"));
    check(
        s.dropMimeData(&sd1, Qt::MoveAction, -1, 0, s.index(srid("SepP_separator"), 0)),
        "separator nest drop #1 accepted");
    QMimeData sd2;
    sd2.setData(QLatin1String(ui::kModListMimeType), QByteArrayLiteral("2"));
    check(
        s.dropMimeData(&sd2, Qt::MoveAction, -1, 0, s.index(srid("SepP_separator"), 0)),
    ...
    check(s.mods()[srid("SepC_separator")].parent_id ==
                  QLatin1String("SepP_separator") &&
              s.mods()[srid("SepD_separator")].parent_id ==
                  QLatin1String("SepP_separator"),
  ...
  // (below its mods) and takes nothing with it.
  {
    ...
    QVector<ui::ModEntry> e;
    for (const char *id :
         {"P_separator", "S1_separator", "ModM1", "ModM2", "S2_separator"}) {
      ui::ModEntry ent;
  ...
  // dropped onto a separator fell back to a flat move.
  {
    ...
    auto rid2 = [&](const char *id) {
      return row_with_id(m2, id);
    };
    QMimeData d2;
    ...
    auto rid3 = [&](const char *id) {
      return row_with_id(m3, id);
    };
    QMimeData d3;
  ...
  // their bands are swallowed by the parent's fold.
  {
    ...
    check(m.nesting_depth(rid("SepN_separator")) == 1, "nested separator depth 1");
    ...
    check(rid("SepN_separator") == rid("SepP_separator") + 1,
          "moving the nested separator above the band places it directly under the "
          "parent");
    check(m.has_content(rid("SepN_separator")),
    ...
    check(m.is_row_fold_hidden(rid("ModZ")), "folding SepQ hides the mods in its band");
  ...
  // unpinnable targets (Overwrite/MERGED/game-native) never nest.
  {
    ...
    check(
        m.dropMimeData(&dmod, Qt::MoveAction, -1, 0, m.index(rid("SepS_separator"), 0)),
    ...
    check(m.dropMimeData(&dow, Qt::MoveAction, -1, 0, m.index(rid("__overwrite__"), 0)),
    ...
    QMimeData dlink;
    dlink.setData(QLatin1String(ui::kModListMimeType), QByteArray::number(rid("ModQ")));
    ...
    check(m.nesting_depth(rid("ModQ")) == 0, "gate-off drops indentation to 0");
    check(!m.has_content(rid("ModP")), "gate-off hides mod content/arrows");
  ...
  // drop clamps below the band as usual.
  {
    ...
    check(m.native_band_last() == 0, "native band intact after the drop");
  }
  ...
  // valid chains survive.
  {
    ui::ModList m;
    QVector<ui::ModEntry> e;
    for (const char *id : {"ModA", "ModB", "ModC", "ModD", "ModE", "ModF", "ModG",
                           "ModH", "ModI", "ModJ"}) {
      ui::ModEntry mod;
      mod.id      = QString::fromLatin1(id);
      mod.name    = mod.id;
    ...
    check(
        !(pid("ModH") == QLatin1String("ModI") && pid("ModI") == QLatin1String("ModH")),
  ...
  // path), remove_mod subtree detach, rename_mod_in_place link cascade.
  {
    ...
    check(pid("ModD").isEmpty(), "restore_parent_links sanitizes dangling links");

    // remove_mod detaches the subtree instead of cascade-deleting it.
    m.remove_mod(QStringLiteral("ModA"));
    check(pid("ModB").isEmpty(), "remove_mod detaches children (no orphaned link)");
  ...
  // rows sit between them; gate-off degrades to the flat single-row move.
  {
    ...
    for (const auto &mod : m.mods())
      order.append(mod.id);
    check(order ==
              (QVector<QString>{QStringLiteral("P"), QStringLiteral("C"),
    ...
    for (const auto &mod : m.mods())
      order.append(mod.id);
    check(order ==
              (QVector<QString>{QStringLiteral("C"), QStringLiteral("G"),
  ...
  // is correct (the connector lines confirm the visual nesting).
  {
    ...
    rm.dropMimeData(&rd, Qt::MoveAction, -1, 0, rm.index(row_with_id(rm, "Parent"), 0));
    ...
    if (cell0.isValid() && cell1.isValid() && !shot.isNull()) {
      ...
      check(!c0.isNull() && !c1.isNull(), "nested Name cells render without error");
    } else {
  ...
  // to start clearly further right at each successive depth.
  {
    ...
    auto nest_onto = [&](const char *child, const char *parent) {
      ...
      sm.dropMimeData(&d, Qt::MoveAction, -1, 0, sm.index(row_with_id(sm, parent), 0));
    };
    nest_onto("B", "A");
    nest_onto("C", "B");
    nest_onto("D", "C");
    ...
    auto first_content_x = [&](const QImage &cell) -> int {
      ...
      for (int x = 0; x < cell.width(); ++x)
        for (int y = y0; y <= y1; ++y) {
          ...
          if (qAbs(qRed(p) - qRed(bg)) > 40 || qAbs(qGreen(p) - qGreen(bg)) > 40 ||
    ...
    for (const char *id : {"A", "B", "C", "D", "E"}) {
      const QRect cell =
          sv.visualRect(sm.index(row_with_id(sm, id), ui::ModList::Name));
      if (!cell.isValid()) {
        geom_ok = false;
        break;
      }
    ...
    if (geom_ok && starts.size() == 5) {
      ...
      for (int i = 1; i < starts.size(); ++i) {
        if (starts[i] <= starts[i - 1])
          increasing = false;
        if (starts[i] - starts[i - 1] < 14)
          clear_step = false;
      }
      check(increasing, "nested separator text starts further right at every depth");
      check(clear_step, "each nesting depth indents the separator name by a clear, "
  ...
  // checkbox width, so the child lands a full step right of the parent.
  {
    ...
    mm.dropMimeData(&md, Qt::MoveAction, -1, 0, mm.index(row_with_id(mm, "P"), 0));
    check(mm.nesting_depth(row_with_id(mm, "C")) == 1, "nested mod renders at depth 1");
    ui::ModView mv;
    mv.setModel(&mm);
    ...
    if (pte >= 0 && cte >= 0)
      check(cte > pte, "first nested mod's text sits right of its parent's text "
...
TEST_CASE("mod list model unparent via flat drop", "[ui]") {
  ...
  check(m.nesting_depth(rid("ModA")) == 0, "ModA is top-level after flat drop");
  int pos = rid("ModA");
...
TEST_CASE("mod list model multi-level nesting drop", "[ui]") {
  ...
  auto add = [&](const char *id, const char *parent) {
    ...
    mod.enabled = true;
    if (parent)
      mod.parent_id = QString::fromLatin1(parent);
    e.append(mod);
  };
  ...
  check(m.nesting_depth(rid("ModB")) == 0, "ModB is at depth 0 after unparenting");
  ...
  check(m.nesting_depth(rid("ModSub")) == 2, "ModSub stays at depth 2 after reorder");
  ...
  QMimeData d;
  d.setData(QLatin1String(ui::kModListMimeType), QByteArray::number(rid("ModB")));
...
TEST_CASE("mod list model move to separator lands last", "[ui]") {
  ...
  // From above: Top lands after M2 (last in SepA), not after SepA.
  {
    ui::ModList m;
    build(m);
    m.move_to_separator(QStringLiteral("Top"), QStringLiteral("SepA_separator"));
    auto rid = [&](const char *id) {
      return row_with_id(m, id);
    };
    ...
    check(m.mods()[rid("Top")].separator_id == QLatin1String("SepA_separator"),
          "sent mod joins the separator group");
    check(m.mods()[rid("Top")].parent_id.isEmpty(),
          "sent mod stays top-level in the band");
    check(m.nesting_depth(rid("Top")) == 0, "sent mod renders flat in the band");
  }
  ...
  // From below: M3 lands after M2 (last in SepA).
  {
    ui::ModList m;
    build(m);
    m.move_to_separator(QStringLiteral("M3"), QStringLiteral("SepA_separator"));
    auto rid = [&](const char *id) {
      return row_with_id(m, id);
    };
  ...
  // From within: M1 moves after M2 (last).
  {
    ui::ModList m;
    build(m);
    m.move_to_separator(QStringLiteral("M1"), QStringLiteral("SepA_separator"));
    auto rid = [&](const char *id) {
      return row_with_id(m, id);
    };
  ...
  // Already last: no-op.
  {
    ...
    m.move_to_separator(QStringLiteral("M2"), QStringLiteral("SepA_separator"));
  ...
  // Empty band: the mod lands directly below the separator.
  {
    ...
    m.move_to_separator(QStringLiteral("Top"), QStringLiteral("SepE_separator"));
    auto rid = [&](const char *id) {
      return row_with_id(m, id);
    };
    ...
    check(m.nesting_depth(rid("Top")) == 0, "empty band: mod renders flat");
  }
  ...
  // A parent mod rides its subtree along and still lands last.
  {
    ...
    auto add = [&](const char *id, const char *parent, bool sep) {
      ...
      ent.is_separator = sep;
      if (parent)
        ent.parent_id = QString::fromLatin1(parent);
      e.append(ent);
    };
    add("P", nullptr, false);
    add("C", "P", false);
    ...
    m.move_to_separator(QStringLiteral("P"), QStringLiteral("SepA_separator"));
    auto rid = [&](const char *id) {
      return row_with_id(m, id);
    };
  ...
  // Unknown ids are no-ops.
  {
    ui::ModList m;
    build(m);
    m.move_to_separator(QStringLiteral("ghost"), QStringLiteral("SepA_separator"));
    m.move_to_separator(QStringLiteral("Top"), QStringLiteral("ghost_separator"));
...
TEST_CASE("mod list model separator boundary resets indent", "[ui]") {
  qputenv("QT_QPA_PLATFORM", "offscreen");
  qputenv("TZ", "UTC");
  tzset();
  const std::filesystem::path cfg = "/tmp/gmm_mod_list_model_sep_indent/config";
  std::filesystem::remove_all("/tmp/gmm_mod_list_model_sep_indent");
  std::filesystem::create_directories(cfg);
  qputenv("XDG_CONFIG_HOME", cfg.c_str());
  int test_argc     = 1;
  char test_argv0[] = "test";
  ...
  // The reported layout: A(0), CrashLogger child of A (1), SepS (2),
  // Water first inside the band (3, stale link to CrashLogger), M2 (4).
  auto build_stale = [](ui::ModList &m) {
    m.set_nesting_enabled(true);
    QVector<ui::ModEntry> e;
    auto add = [&](const char *id, const char *parent, bool sep) {
      ui::ModEntry ent;
    ...
    add("A", nullptr, false);
    add("CrashLogger", "A", false);
    add("SepS_separator", nullptr, true);
    add("Water", "CrashLogger", false);  // stale cross-boundary link
    add("M2", nullptr, false);
    ui::ModEntry ow;
    ow.id           = ui::kOverwriteModId;
    ow.name         = ui::kOverwriteModName;
    ow.enabled      = true;
    ow.is_overwrite = true;
    e.append(ow);
    m.reset_with_order(e);  // sanitize keeps the same-kind link
  };

  // The stale link survives load (same kind, no cycle) but renders flat.
  {
    ui::ModList m;
    build_stale(m);
    auto rid = [&](const char *id) {
    ...
    check(m.mods()[rid("Water")].parent_id == QLatin1String("CrashLogger"),
          "stale cross-boundary link survives sanitize");
    check(m.nesting_depth(rid("Water")) == 0,
          "first mod inside the separator renders flat despite the link");
    check(
        m.data(m.index(rid("Water"), ui::ModList::Name), ui::ModList::kIndentDepthRole)
                .toInt() == 0,
        "indent role is 0 for the first mod inside the separator");
    check(m.nesting_depth(rid("CrashLogger")) == 1,
          "in-band nesting above the separator is unaffected");
  }

  // Deeper stale chain: the reset is total, not partial.
  {
    ui::ModList m;
    m.set_nesting_enabled(true);
    QVector<ui::ModEntry> e;
    ...
    add("CrashLogger", "A", false);
    add("SepS_separator", nullptr, true);
    add("Water", "CrashLogger", false);  // stale: depth would be 2
    ui::ModEntry ow;
    ow.id           = ui::kOverwriteModId;
    ow.name         = ui::kOverwriteModName;
    ow.enabled      = true;
    ...
    check(m.nesting_depth(rid("Water")) == 0,
          "depth-2 stale chain across a separator renders flat");
  }

  // Last-child scan stops at separators: P(0), C1 child of P (1), S (2),
  // C2 child of P (3, stale). C1 is the last in-band child.
  {
    ui::ModList m;
    m.set_nesting_enabled(true);
    QVector<ui::ModEntry> e;
    ...
    add("C1", "P", false);
    add("SepS_separator", nullptr, true);
    add("C2", "P", false);  // stale cross-boundary sibling
    ui::ModEntry ow;
    ow.id           = ui::kOverwriteModId;
    ow.name         = ui::kOverwriteModName;
    ow.enabled      = true;
    ...
    check(m.data(m.index(rid("C1"), ui::ModList::Name), ui::ModList::kIsLastChildRole)
              .toBool(),
          "in-band child is last once the scan stops at the separator");
    check(m.nesting_depth(rid("C2")) == 0, "cross-boundary sibling renders flat");
    check(m.nesting_depth(rid("C1")) == 1, "in-band child keeps its depth");
  }

  // Moving a mod to first-in-band never parents it under the separator.
  {
    ui::ModList m;
    m.set_nesting_enabled(true);
    QVector<ui::ModEntry> e;
    ...
    e.append(ow);
    m.reset_with_order(e);
    // Rows: Top(0), SepS(1), M1(2), overwrite(3).
    auto rid = [&](const char *id) {
      return row_with_id(m, id);
    };
    // move_mod() takes post-removal coordinates: removing Top(0) shifts
    // the band up, so first-in-band is index 1, not rid(SepS) + 1 (= 2).
    m.move_mod(QStringLiteral("Top"), 1);
    check(rid("Top") == rid("SepS_separator") + 1, "mod lands first in the band");
    check(m.mods()[rid("Top")].parent_id.isEmpty(),
          "move below a separator never links under it");
    check(m.nesting_depth(rid("Top")) == 0, "moved mod renders flat");
  }

  // Between-row drops below a separator stay flat too.
  {
    ui::ModList m;
    m.set_nesting_enabled(true);
    QVector<ui::ModEntry> e;
    ...
    QMimeData d;
    d.setData(QLatin1String(ui::kModListMimeType), QByteArrayLiteral("0"));
    check(m.dropMimeData(&d, Qt::MoveAction, rid("SepS_separator") + 1, 0, {}),
          "between-row drop below a separator accepted");
    check(m.mods()[rid("Top")].parent_id.isEmpty(),
          "dropped mod never links under the separator");
    check(m.nesting_depth(rid("Top")) == 0, "dropped mod renders flat");
  }

  // Positive control: genuine in-band nesting still indents.
  {
    ui::ModList m;
    m.set_nesting_enabled(true);
    QVector<ui::ModEntry> e;
    ...
    e.append(ow);
    m.reset_with_order(e);
    // Rows: SepS(0), M1(1), M2(2), overwrite(3).
    auto rid = [&](const char *id) {
      return row_with_id(m, id);
    };
    QMimeData d;
    d.setData(QLatin1String(ui::kModListMimeType), QByteArray::number(rid("M2")));
    check(m.dropMimeData(&d, Qt::MoveAction, -1, 0, m.index(rid("M1"), 0)),
          "in-band mod-on-mod nest drop accepted");
    check(m.mods()[rid("M2")].parent_id == QLatin1String("M1"),
          "in-band nesting still links");
...
TEST_CASE("mod list phantom row predicate", "[ui]") {
  ...
  sep.is_separator = true;
  check(!ui::is_phantom_row(sep), "separator is a real folder, persists meta");
  ...
  check(ui::is_phantom_row(ow_id_only), "overwrite id alone never persists meta");
