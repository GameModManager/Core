...
TEST_CASE("settings sources tab", "[ui]") {
  ...
  // --- Dialog #1: initial disconnected state. ---
  {
    ...
    if (steam)
      for (auto *gb : steam->findChildren<QGroupBox *>())
        if (gb->title() == "Steam Web API Rate Limit")
          steam_group = true;
    ...
    if (nexus)
      for (auto *gb : nexus->findChildren<QGroupBox *>()) {
        if (gb->title() == "Nexus Account")
          acc = true;
        if (gb->title() == "Statistics")
          stats = true;
        if (gb->title() == "Nexus Connection")
          conn = true;
        if (gb->title() == "Options")
          opts = true;
        if (gb->title() == "Servers")
    ...
    check(acc && stats && conn && opts && srv, "Nexus page has all five MO2 boxes");
    ...
    check(uid && nm && acc_lbl && daily && hourly, "account/statistics labels present");
    check(uid && uid->text() == "N/A" && nm && nm->text() == "N/A" && acc_lbl &&
              acc_lbl->text() == "N/A" && daily && daily->text() == "N/A" && hourly &&
              hourly->text() == "N/A",
    ...
    check(connect_btn && manual_btn && disconnect_btn, "connection buttons present");
    ...
    if (nexus)
      for (auto *cb : nexus->findChildren<QCheckBox *>()) {
        if (cb->text() == "Endorsement Integration")
          endorse = cb;
        if (cb->text() == "Tracked Integration")
          track = cb;
        if (cb->text() == "Apply Nexus category mappings")
          cat = cb;
        if (cb->text() == "Hide API Request Counter")
          counter = cb;
        if (cb->text() == "Queue downloads (one at a time)")
          queue = cb;
      }
    check(endorse && track && cat && counter && queue,
          "all five option checkboxes present");
    check(endorse && endorse->isChecked() && track && track->isChecked() && counter &&
              !counter->isChecked() && queue && queue->isChecked(),
          "options initial states (endorse + track + queue on, counter off)");
    check(cat && !cat->isEnabled() && cat->toolTip().contains("Work in progress"),
    ...
    check(known && preferred && known->dragDropMode() == QAbstractItemView::DragDrop &&
  ...
  check(servers.known().size() == 1 && servers.known()[0].average_speed() == 1048576,
  ...
  // --- Dialog #2: populated lists + drag&drop persistence. ---
  {
    ...
    app.processEvents();
    {
      ...
      for (const auto &srv : known_v)
        if (srv.name == "CDN")
          known_cdn = true;
      check(pref_v.size() == 1 && pref_test && known_v.size() == 1 && known_cdn,
  ...
  // --- Dialog #3: the reordered preference survives a reopen. ---
  {
    ...
    auto *preferred =
        nexus ? nexus->findChild<QListWidget *>("preferredServersList") : nullptr;
    check(known && known->count() == 1 && known->item(0)->text() == "CDN (automatic)",
        ...
        "user info roundtrips through disk");
  {
    ...
    auto *nm      = nexus ? nexus->findChild<QLabel *>("nexusName") : nullptr;
    auto *acc_lbl = nexus ? nexus->findChild<QLabel *>("nexusAccount") : nullptr;
    check(uid && uid->text() == "12345" && nm && nm->text() == "TestUser" && acc_lbl &&
              acc_lbl->text() == "Supporter",
  ...
  // the ~1.5MB/s cap, so parallel Nexus downloads are useful).
  {
    ...
    check(q && !q->isChecked(), "Premium account: queueing defaults OFF while unset");
  }
  // Supporter keeps the free-account throttle -> ON.
  {
    ...
    auto dlg = make_dialog();
    auto *q  = queue_box_of(nexus_page_of(dlg));
    check(q && q->isChecked(), "Supporter account: queueing defaults ON while unset");
  }
  // Regular -> ON.
  {
    ...
    auto dlg = make_dialog();
    auto *q  = queue_box_of(nexus_page_of(dlg));
    check(q && q->isChecked(), "Regular account: queueing defaults ON while unset");
  }
  ...
  // apply_nexus_queue_default() may override it.
  {
    ...
    auto dlg = make_dialog();
    auto *q  = queue_box_of(nexus_page_of(dlg));
    check(q && !q->isChecked(), "manual OFF choice shown even for a Supporter login");
  ...
  check(auth.has_api_key(), "api key persisted (file fallback)");
  {
    ...
    check(connect_btn && !connect_btn->isEnabled() && disconnect_btn &&
          ...
          "Connect stays disabled (validation crashes), Disconnect enabled once a key "
          "is stored");
    check(log_list && log_list->count() == 1 &&
