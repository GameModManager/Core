...
//
// Covers: table populated from engine::Category::Factory (ID | Name |
// ParentID), Add inserting a row with the next free ID, Remove deleting the
// selected row, ParentID validation (0 or an existing id, never self),
// duplicate-ID rejection, and commit_changes applying the table to the factory
