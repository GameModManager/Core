...
static void seed_synthetic_fomod(engine::PluginLoader &loader) {
  ...
  fomod.settings = {{"Restore previous choices", "1"}, {"Show FOMOD images", "1"}};
...
TEST_CASE("fomod plugin settings integration", "[ui]") {
  ...
  for (const auto &p : loader.plugins()) {
    ...
    if (basename.find("FomodInstaller") != std::string::npos ||
      ...
      check(p.category == "Installer", "FOMOD plugin category is Installer");
  ...
  if (has_fomod) {
    ...
    if (table) {
      ...
      rows_ok       = table->columnCount() == 2 && table->rowCount() == 2 && restore &&
                      images && (restore->flags() & Qt::ItemIsUserCheckable) &&
                      restore->checkState() == Qt::Checked &&
                      (images->flags() & Qt::ItemIsUserCheckable) &&
                      images->checkState() == Qt::Checked;
    }
    check(rows_ok, "FOMOD settings render as checked checkbox rows in the table");
      ...
      for (int c = 0; c < group->childCount() && !restored; ++c) {
        if (group->child(c)->text(0) == fomod_display_name) {
          ...
          if (auto *t = page2->findChild<QTableWidget *>()) {
            ...
            restored = restore && images && restore->checkState() == Qt::Unchecked &&
                       images->checkState() == Qt::Checked;
          }
        }
      }
