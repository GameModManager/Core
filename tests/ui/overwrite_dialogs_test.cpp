...
TEST_CASE("overwrite dialogs", "[ui]") {
  ...
  }
  {
    ...
    replace->click();
    check(dlg.action() == engine::OverwriteAction::Replace,
          "Replace sets Replace action");
  }
  {
    ...
    cancel->click();
    check(dlg.action() == engine::OverwriteAction::Cancel,
          "Cancel keeps Cancel action");
  ...
  // ---- SyncOverwriteDialog --------------------------------------------------
  {
    ...
    for (const auto &[name, combo] : rows) {
      if (name == "foo.nif")
        foo_owner = combo->currentData().toString().toStdString();
      if (name == "bar.dds")
        bar_owner = combo->currentData().toString().toStdString();
      if (name == "shared.nif")
        shared_owner = combo->currentData().toString().toStdString();
    ...
    check(shared_owner == "ModB",
          "conflict defaults to the higher-priority owner ModB");
    ...
    for (const auto &t : targets) {
      if (t.overwrite_rel == "Data/meshes/foo.nif" && t.mod_folder == "ModA")
        has_foo = true;
      if (t.overwrite_rel == "Data/textures/bar.dds" && t.mod_folder == "ModB")
        has_bar = true;
      if (t.overwrite_rel == "Data/meshes/shared.nif" && t.mod_folder == "ModB")
        has_shared = true;
    }
    check(has_foo && has_bar && has_shared,
          "targets() maps each owned file to its owner mod");
    ...
    for (const auto &[name, combo] : rows2) {
      if (name == "shared.nif") {
        for (int i = 0; i < combo->count(); ++i) {
          if (combo->itemData(i).toString() == "ModA")
  ...
  // ---- OverwriteInfoDialog ------------------------------------------------
  {
    ...
    const auto data_idx = model->index(QString::fromStdString((ow / "Data").string()));
    ...
    const auto isaac_root_idx = isaac_model->index(QString::fromStdString(ow.string()));
