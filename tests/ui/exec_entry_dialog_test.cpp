// Offscreen GUI regression test for Executables::Entry + the output-to-mod path
// matcher.
...
TEST_CASE("exec entry dialog", "[ui]") {
  ...
  // ---- Executables::Entry JSON round-trip ------------------------------------------
  {
    ...
    check(back.path == e.path && back.title == e.title &&
              back.output_mod == "Bodyslide OUT",
  ...
  // ---- output_mod_for_path -------------------------------------------------
  {
    ...
    check(
        ui::Executables::output_mod_for_path(
            entries, game, qstr(game / "Data/CalienteTools/BodySlide/BodySlide.exe")) ==
            "Bodyslide OUT",
    ...
    check(ui::Executables::output_mod_for_path(
              entries, game, qstr(game / "SKSE64_Loader.exe")) == "SKSE Scripts",
    ...
    // Empty inputs.
    check(ui::Executables::output_mod_for_path(
              {}, game, qstr(game / "Data/CalienteTools/BodySlide/BodySlide.exe"))
              .isEmpty(),
  ...
  // ---- environment_for_path ------------------------------------------------
  {
    ...
    check(ui::Executables::environment_for_path(
              entries, game,
              qstr(game /
                   "Data/Pandora Behaviour Engine+/Pandora Behaviour Engine+.exe")) ==
              pandora.environment,
    ...
    check(
        ui::Executables::environment_for_path(
            {}, game,
  ...
  // ---- symlink-canonicalized game dir --------------------------------------
  {
    ...
    if (!ec) {
      check(ui::Executables::output_mod_for_path(
                {e}, link, qstr(game / "Data/CalienteTools/BodySlide/BodySlide.exe")) ==
                "Bodyslide OUT",
            "symlinked game dir resolves against canonical entries");
      check(ui::Executables::output_mod_for_path(
                {e}, game, qstr(link / "Data/CalienteTools/BodySlide/BodySlide.exe")) ==
                "Bodyslide OUT",
