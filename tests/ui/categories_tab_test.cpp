...
//
// The tab must render the instance-scoped engine::CategoryFactory registry —
...
// (the registry is driven through CategoryFactory::load() exactly like
...
QVector<NodeInfo> nodes_of(const ui::CategoriesTab& tab) {
    ...
    if (tree) collect_nodes(tree->invisibleRootItem(), 0, out);
    return out;
}
...
void switch_game(const std::filesystem::path& dat) {
    engine::CategoryFactory::instance().load(dat);
}
...
TEST_CASE("categories tab", "[ui]") {
    ...
    SECTION("assignment persists ids with ancestors auto-checked") {
        ...
        const auto meta = engine::ModMeta::load(base / "instances/Isaac/meta",
                                                "ModA");
