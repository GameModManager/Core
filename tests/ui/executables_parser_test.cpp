...
TEST_CASE("executables parser", "[ui]") {
  ...
  const std::string content = "executables = [\n"
  ...
  check(contains(entries, "skse64_loader.exe"), "first entry present");
  ...
  const std::string quoted = "executables = [{ path = \"a.exe\", args = \"-x ]\" },"
                             "{ path = \"b.exe\" }]\n";
  ...
  check(contains(q, "b.exe"), "bracket inside a quoted string does not truncate");

  // Legacy JSON-style inline tables (pre-toml++ migration) are repaired and
  // parsed: {"path":"..."} -> { path = "..." }.
  const std::string legacy  = "executables = [\n"
                              "  {\"path\":\"skse64_loader.exe\",\"title\":\"SKSE\","
                              "\"env\":[\"WINEDEBUG=+file\"]},\n"
                              "  {\"path\":\"Data/Tools/Tool.exe\",\"title\":\"Tool\"}\n"
                              "]\n";
  ...
  check(contains(legacy_entries, "skse64_loader.exe"), "legacy first entry present");
  check(contains(legacy_entries, "Data/Tools/Tool.exe"),
        "legacy second entry present after nested env array");
  check(contains(legacy_entries, "WINEDEBUG=+file"), "legacy env value preserved");
  ...
  check(contains(plain_entries, "skse64_loader.exe"), "plain string wrapped with path");
