...
TEST_CASE("list dialog", "[ui]") {
  ...
  // ---- choices / selection ------------------------------------------------
  {
    ...
    check(list && list->item(0)->text() == "Alpha" && list->item(2)->text() == "Gamma",
  ...
  // ---- filter -------------------------------------------------------------
  {
    ...
    check(list->styleSheet().isEmpty(), "cleared filter removes the red border");
  }
  ...
  // ---- double-click accepts ----------------------------------------------
  {
    ...
    if (list) {
      ...
      QMouseEvent release(QEvent::MouseButtonRelease, center, center, Qt::LeftButton,
                          Qt::LeftButton, Qt::NoModifier);
