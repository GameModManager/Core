...
TEST_CASE("settings plugins tab", "[ui]") {
  ...
  const std::filesystem::path cfg = "/tmp/gmm_settings_plugins_tab/config";
  std::filesystem::remove_all("/tmp/gmm_settings_plugins_tab");
  ...
  const std::filesystem::path root = "/tmp/gmm_settings_plugins_tab/instances/Test";
  ...
  for (int g = 0; g < tree->topLevelItemCount(); ++g) {
    ...
    if (group->text(0) == "Game Support" && f.bold())
      has_game_support = true;
    if (group->text(0) == "Tool" && f.bold())
      has_tool = true;
  ...
  for (int g = 0; g < tree->topLevelItemCount(); ++g)
    if (tree->topLevelItem(g)->flags() & Qt::ItemIsSelectable)
      headers_unselectable = false;
        ...
        if (t->columnCount() == 2 && t->rowCount() > 0) {
          ...
          for (int r = 0; r < t->rowCount(); ++r) {
            ...
            if (k->text() == "masterlist_url")
              has_masterlist = true;
            else if (k->text() == "auto_sort_on_load")
              has_auto = true;
          }
  ...
  check(
      opt_masterlist && opt_masterlist->text().isEmpty() &&
          !(opt_masterlist->flags() & Qt::ItemIsUserCheckable) && opt_auto &&
          opt_auto->checkState() == Qt::Checked &&
  ...
  app.processEvents();
  check(Settings::instance().plugin_setting(options_basename, "auto_sort_on_load",
                                            "1") == "0",
  ...
  // No "Fixture Settings" tab may exist anymore — settings render inline.
  bool has_fixture_tab = false;
  for (int i = 0; tabs && i < tabs->count(); ++i)
    if (tabs->tabText(i) == "Fixture Settings")
      has_fixture_tab = true;
  ...
  if (fixture_table) {
    ...
    rows_ok = fixture_table->rowCount() == 5 && previews && threads && prefix && mode &&
              ...
              prefix->text() == "mod_" && mode->text() == "Full" &&
              legacy->text() == "legacy_value";
  }
  ...
  if (paths_page) {
    for (auto *le : paths_page->findChildren<QLineEdit *>()) {
      ...
      if (ph == "$BASE_DIRECTORY/mods")
        mods_edit = le;
      else if (ph == "$BASE_DIRECTORY/downloads")
        downloads_edit = le;
      else if (ph == "$BASE_DIRECTORY/cache")
        cache_edit = le;
      else if (ph == "$BASE_DIRECTORY/profiles")
        profiles_edit = le;
      else if (ph == "$BASE_DIRECTORY/overwrite")
        overwrite_edit = le;
    }
  }
  ...
  const QString custom_mods = "/tmp/gmm_settings_plugins_tab/custom/mods";
  if (mods_edit) {
  ...
  for (int g = 0; tree2 && g < tree2->topLevelItemCount(); ++g)
    if (tree2->topLevelItem(g)->text(0) == "Sources")
      has_sources2 = true;
  ...
  check(persisted_value, "edited option values persisted across dialog reopen");
  ...
  // --- P1.5 reopen: the edited inline settings survive into a new dialog.
