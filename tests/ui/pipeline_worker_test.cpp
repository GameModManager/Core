...
public:
  ...
  bool fetch(const engine::Mod &, engine::PipelineContext &ctx,
    ...
    if (barrier_.load()) {
      ...
      if (!barrier_cv_.wait_for(lk, std::chrono::seconds(5), [this] {
            return barrier_go_.load();
          })) {
  ...
  bool wait_entered(int n, int timeout_ms) {
    ...
    return entry_cv_.wait_for(lk, std::chrono::milliseconds(timeout_ms), [this, n] {
      return entered_.load() >= n;
    });
  }
...
TEST_CASE("pipeline worker", "[ui]") {
  ...
  QObject::connect(
      &worker, &ui::PipelineWorker::download_complete, &app,
      [&](const std::string &id, bool ok, const std::string &, const std::string &) {
        completions.emplace_back(id, ok);
      });
  QObject::connect(&worker, &ui::PipelineWorker::paused, &app,
                   [&](const std::string &id) {
  ...
  check(wait_until(
            [&] {
              return has_completion("a") && has_completion("b");
            },
            5000),
  ...
  check(std::filesystem::exists(partial) && std::filesystem::file_size(partial) > 0,
  ...
  start("r1");
  start("r2");
  check(fake->wait_entered(2, 5000), "queue-pause test: both slots occupied");
  start("r3");
  QThread::msleep(150);
  ...
  check(wait_until(
            [&] {
              return fake->completed() == 7;
            },
            5000),
        "the two running downloads complete");
  check(wait_until(
            [&] {
              return !has_completion("r3");
            },
            300),
  ...
  check(wait_until(
            [&] {
              return nfake->entered() >= 1;
            },
            5000),
        "queueing: first nexus download starts");
  QThread::msleep(150);
  check(nfake->started() == 1, "queueing: second nexus download stays QUEUED "
                               "(1-at-a-time) even with a free slot");
  nfake->release_barrier();
  check(wait_until(
            [&] {
              return nfake->completed() == 1;
            },
            5000),
        "queueing: first nexus download completes");
  check(wait_until(
            [&] {
              return nfake->started() == 2;
            },
            5000),
  ...
  check(wait_until(
            [&] {
              return nfake->completed() == 2;
            },
            5000),
        "queueing: both nexus downloads complete");
  check(nfake->max_active() == 1, "queueing: nexus transfers never overlapped");
  check(wait_until(
            [&] {
              return has_completion("n1") && has_completion("n2");
            },
            5000),
  ...
  check(wait_until(
            [&] {
              return has_completion("l1") && has_completion("n3");
            },
            5000),
  ...
  check(wait_until(
            [&] {
              return has_completion("n4") && has_completion("n5");
            },
            5000),
