...
public:
  ...
  fs::path config_dir() const override {
    return "/tmp/gmm_instance_options_widget_config";
  }
  fs::path cache_dir() const override {
    return "/tmp/gmm_instance_options_widget_cache";
  }
  ...
  fs::path find_proton_named(const std::string &name) const override {
    if (name == "Proton 10.0")
      return "/steam/Proton 10.0/proton";
    if (name == "Proton - Experimental")
      return "/steam/Proton - Experimental/proton";
    return {};
  }
...
TEST_CASE("instance options widget", "[ui]") {
  ...
  ui::InstanceOptionsWidget widget(
      ...
      engine::kDefaultDeployStrategy, engine::DeployConfig{});
  ...
  for (auto *combo : widget.findChildren<QComboBox *>()) {
    for (int i = 0; i < combo->count(); ++i) {
      ...
      if (text == "Automatic (Steam default)")
        found_auto = true;
      if (text == "Proton 10.0")
        found_10 = true;
      if (text == "Proton - Experimental")
        found_exp = true;
    }
  }
  ...
  for (auto *btn : widget.findChildren<QPushButton *>()) {
    if (strip_mnemonic(btn->text()) == "Save")
      btn->click();
  }
  require(saved, "Save emits save_requested");
  for (auto *btn : widget.findChildren<QPushButton *>()) {
    if (strip_mnemonic(btn->text()) == "Close")
      btn->click();
  }
...
TEST_CASE("instance options widget survives tab close during deploy task", "[ui]") {
  ...
  const auto tmp =
      std::filesystem::temp_directory_path() / "gmm_instance_options_uaf_test";
  ...
  auto *widget = new ui::InstanceOptionsWidget(
      &platform, &loader, "SkyrimSpecialEdition", "Skyrim Special Edition",
      config.game_dir, 489830, tmp, "", engine::kDefaultDeployStrategy, config);
