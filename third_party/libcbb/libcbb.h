...
static int cbb_eq(const char *a, int alen, const char *b) {
  ...
  for (int i = 0; i < alen; ++i)
    if (cbb_lo((unsigned char)a[i]) != cbb_lo((unsigned char)b[i]))
      return 0;
  return 1;
}
...
static const cbb_tag_def *cbb_lookup(const char *name, int nlen) {
  for (int i = 0; i < CBB_N_TAGS; ++i)
    if (cbb_eq(name, nlen, cbb_tags[i].name))
      return &cbb_tags[i];
  return NULL;
}
...
static void cbb_puts(cbb_buf *b, const char *s) {
  cbb_putsn(b, s, strlen(s));
}
...
static void cbb_pop_to(cbb_buf *o, cbb_stk *s, int to) {
  for (int j = s->len - 1; j > to; --j)
    cbb_emit_close(o, &s->data[j]);
  cbb_emit_close(o, &s->data[to]);
  for (int j = to; j < s->len; ++j)
    free(s->data[j].arg);
  s->len = to;
}
...
static int cbb_find_li(const cbb_stk *s) {
  for (int i = s->len - 1; i >= 0; --i)
    if (s->data[i].tag && cbb_eq("*", 1, s->data[i].tag->name))
      return i;
  return -1;
}
  ...
  for (int i = 0; i < n && !out.oom; ++i) {
    ...
    if (noparse > 0) {
      if (t->kind == T_CLOSE && stack.len > 0) {
        ...
        int top_is_code =
            (top->tag && cbb_eq(top->tag->name, (int)strlen(top->tag->name), "code"));
        int close_is_noparse = cbb_eq(t->name, t->name_len, "noparse");
        int close_is_code    = cbb_eq(t->name, t->name_len, "code");
        if ((top_is_noparse && close_is_noparse) || (top_is_code && close_is_code)) {
          if (top_is_code) {
    ...
    if (def && cbb_eq(t->name, t->name_len, "code")) {
      if (stack.len > 0 && stack.data[stack.len - 1].tag &&
          cbb_eq(stack.data[stack.len - 1].tag->name,
                 (int)strlen(stack.data[stack.len - 1].tag->name), "code")) {
        if (in_code > 0) {
          cbb_puts(&out, "</code></pre>");
          in_code--;
        }
